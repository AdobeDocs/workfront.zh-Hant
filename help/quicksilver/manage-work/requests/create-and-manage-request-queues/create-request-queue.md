...
1. 指定下列資訊：
   ...
   * **誰可以新增要求至此佇列：** 選取哪些使用者有權將請求新增至此佇列。 您可以讓下列群組的人在全域導覽列的「要求」區域中看到「要求佇列」：
     ...
     | 專案群組人員 | 屬於與此專案相關聯的群組的使用者可以檢視請求並將其新增到此佇列。 如果有與專案相關聯的群組，則該群組的名稱會列在此設定後的括弧中。 |
   ...
   * **來自相同公司的人員將針對所有要求繼承相同權限。：** 選取後，所有提交至佇列的請求對同一家公司的使用者可見。 使用者可以在所有請求區段（位於請求區域）中檢視這些請求。 啟用或停用此設定時，將會影響所有未來的請求；而不會回溯影響資訊。
   ...
   * **預設路由**：將路由規則與此請求佇列相關聯。 使用「遞送規則」，自動將提交至「請求佇列」的新問題指派給正確的資源（使用者、工作角色或團隊），並指派給正確的專案。 提交至此佇列的所有問題都將與此路由規則相關聯。 您必須先設定路由規則，才能將其與請求佇列關聯。\
   ...
   * **新問題欄位：** 在向所有使用者顯示以下已選取的欄位區段中，選取您希望向專案提交請求或向專案或任務新增問題的所有使用者可見的任何欄位。

     >[!TIP]
     >
     在佇列詳細資料區段中選取的新問題欄位也會與新增到專案或問題區段中的任務的任何新問題相關聯。
   ...
   * **將所有選取和未選取的欄位顯示給：** 選取您想要檢視表單上所有欄位的使用者。 以下選項可控制對表單上欄位的存取。
   ...
   * **自訂Forms**：選取要與請求佇列相關聯的自訂表單。 在此下拉式選單中僅問題自訂Forms可供選取。 提交至「請求佇列」的所有問題都會有與其關聯的選定表單。\
     ...
     >[!TIP]
     >
     新增到佇列詳細資料區段的自訂表單還與新增到專案的任何新問題或問題區段中的任務相關聯。
