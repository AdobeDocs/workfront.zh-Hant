...
<table border="1" cellspacing="15"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>百分比（p或pe）</p> </td> 
   <td> <p>延遲以完成前置任務的估計時間的百分比表示。 </p> <p>例如，如果10天前置任務存在20%延遲的完成 — 開始依賴項，則系統將計算多少天代表20%的前置任務持續時間，並將其用作延遲。 在此情況下，工作完成後2天。 </p>

<p><b>附註</b></p> 百分比的最大延遲限制為2000%。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>星期(w) </p> </td> 
   <td> <p>兩個任務之間的延遲通過指示包含前一任務的計畫完成日期的一週中的天數來衡量。</p> <p>對於此延遲類型，一週中的每一天都與一個數字相關聯：</p> 
    <ul> 
     ...
     <li>星期五=6</li> 
     <li>星期六=7</li> 
    </ul> <p>如果要指明繼承者的計畫起始日期應為當周的星期二，並且星期二早於前一個的計畫完成日期，則您將使用以下公式為您的繼承者編碼： </p> <p><code style="font-style: normal;">4fs-3w</code> </p>

<p><b>附註</b></p>

如果星期二是前一任務的計畫完成日期的一週，則後續任務的計畫起始日期是該周的第一個可用工作日。 </p> <p>如果您想要指出延遲應落在當周的星期六，而星期六晚於前一個的計畫完成日期，您將使用下列公式為您的繼任者編碼：</p> <p>4fs+7w</code> </p> <p>如果星期六是非工作日，則選擇星期六之後的下一個可用日（以表示正滯後）作為後續的計畫起始日期。 </p>

<p>這不適用於排程例外。 如果日期也是計畫例外，而後繼項的「開始日期」計算為該日，則系統會嘗試查找最接近的可用日期，即在前置表達式中指定的一週中的某天。</p>

<p>例如，如果「開始日期」計算為某個星期二，而該日是計畫例外，而前一個日期的延遲是正數，則它將選擇下星期二（如果也是工作日）作為後繼日期的開始日期。 如果延遲為負，則系統會選擇前一個星期二作為開始日期。</p>

<p>若要指出過去或未來的周數，您可以在延遲類型的日數前面新增數字。 </p> <p>例如，若要指出10週前的星期一，您可以使用此程式碼來指出您繼任者的前身：</p> <p><code>4fs-102w</code> </p> <p>10表示10週前，2是指派給星期一的數字。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p>星期非零(k)</p> </td> 
