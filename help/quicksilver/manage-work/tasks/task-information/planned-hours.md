...
* 計畫時數主要與Adobe Workfront中的工作專案（任務和問題）相關聯。 工作專案中的計畫時數累計至其專案的計畫時數。
* 依預設，Workfront會將任務和問題計畫時數平均分發給任務或問題期間中的所有天。
* 將使用者和角色指派給任務和問題時，任務和問題的規劃時數會與使用者或角色指派相關聯。
* 如果要使用Workfront中的資源管理工具，您必須為任務和問題定義規劃時數的值。
* 您只能為某些期間型別修改任務的計畫時數值。

  有關修改任務中與任務期間型別相關的計畫時數的更多資訊，請參閱小節 [根據期間型別更新任務計畫時數](#update-task-planned-hours-based-on-duration-type) 本文章內容。

* 您可以隨時修改問題的計畫時數值。
* 您無法修改專案或父系任務的計畫時數值，因為它們是其所有任務和子任務的所有計畫時數計算的總和。
* 使用資源管理工具管理使用者配置可能會變更任務、問題、專案以及與工作專案相關聯的指派的計畫時數。

## 任務上的計畫時數與專案上的計畫時數 {#planned-hours-on-tasks-vs-planned-hours-on-projects}

從任務累計至專案計畫時數的計畫時數。 問題的計畫時數並不總是累計到專案的計畫時數。

本節說明任務和專案計畫時數之間的差異。 它也說明您可以在何處檢視累計至專案的問題計畫時數。

* [任務的計畫時數](#planned-hours-on-tasks)
* [專案上的計畫時數](#planned-hours-on-projects)

### 任務的計畫時數 {#planned-hours-on-tasks}

任務的計畫時數表示任務實際工時可能花費的估計時間量。 依預設，Workfront會平均分配每個任務期間內每天的計畫時數總計。 計畫時數的每日金額會變成任務的每日分配。 如果將任務指派給多個資源，預設會為每個資源分配相等的每日時數。

使用工作負載平衡器，您可以修改指派給任務之使用者的每日配置。 當任務期間型別為簡單時，這也可以更新任務的計畫時數。 如需詳細資訊，請參閱文章中的「管理使用者指派時更新任務規劃時數」一節 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).
...
您無法編輯專案上的計畫時數數量。 專案上的計畫時數是專案上所有任務的所有計畫時數的計算總和。

「計畫時數」的計算中是否包含問題，取決於您在專案中檢視計畫時數的位置。 您可以在專案的下列位置中檢視專案計畫時數：

* **專案詳細資訊區段與編輯專案方塊**：僅考慮專案上任務的計畫時數。 在專案詳細資訊區段或編輯專案方塊中檢視專案的計畫時數總數時，未考慮專案上問題的計畫時數。

* **工作負載平衡器**：只有與工作負載平衡器中可見的任務相關的計畫時數會顯示在專案的工作負載平衡器中。 使用者每日分配可以在工作負載平衡器中變更專案每日計畫時數。
* **使用率區段**：在使用情況區段中檢視專案的計畫時數總數時，會考慮與指派給任務的使用者關聯的計畫時數以及專案上的問題。
* **角色配置面板** 在任務清單中：此區域顯示指派給工作角色或與工作角色相關聯之使用者的任務和專案問題的計畫時數。 與未指派或指派給團隊的任務和問題相關的計畫時數未顯示在此區域中。 如需詳細資訊，請參閱 [在角色配置面板中檢視專案計畫時數](../../../manage-work/projects/planning-a-project/view-planed-hours-in-role-allocation-panel.md).

## 整個任務持續期間的計畫時數分佈

依預設，Workfront會平均分配計畫時數至任務期間，並根據專案排程的可用性，為任務的每天分配相等數量的計畫時數。

例如，如果任務設定在下午4點開始，而排程在任務的第一天還剩一小時，Workfront會在任務期間的第一天放入一個計畫小時，然後將計畫小時的剩餘時間平均分配到任務期間的其餘天中。

>[!NOTE]
>
>每日計畫時數或每日配置是在任務期間每天的計畫時數配置。 如果任務有一個指派，則此數字代表每個指派的每日計畫時數。 如果任務有多個指派，則每個指派的每日計畫時數與任務的每日計畫時數不同。 對於具有多個指派的任務，Workfront中沒有每個指派的每日計畫時數視覺化表示法。

## 找到並瞭解計畫時數值

您可以在Workfront的各個區域找到計畫時數值。 

顯示的計畫時數源自於專案上的工作專案，或根據您在哪個區域及哪個物件中檢視計畫時數而計算出來的不同。

您可以在Workfront的下列區域中找到計畫時數：

* [專案、任務或問題的詳細資訊區段](#the-details-section-of-a-project-task-or-issue)
* [「編輯任務」或「編輯問題」方塊](#the-edit-task-or-edit-issue-box)
* [報告](#reports)
* [工作負載平衡器](#the-workload-balancer)
* [資源規劃工具](#the-resource-planner)
* [使用率報表](#the-utilization-report)
* [角色配置面板](#The%C2%A0Role)

### 專案、任務或問題的詳細資訊區段 {#the-details-section-of-a-project-task-or-issue}

![](assets/planned-hours-on-details-for-project-nwe-350x138.png)

任務、問題或專案之詳細資訊區段中的計畫時數是與個別專案相關聯的總計畫時數。

如需有關專案計畫時數的詳細資訊，請參閱 [任務上的計畫時數與專案上的計畫時數](#planned-hours-on-tasks-vs-planned-hours-on-projects) 章節。
...
對於任務，您只能編輯特定期間型別的計畫時數。 如需詳細資訊，請參閱 [根據期間型別更新任務計畫時數](#update-task-planned-hours-based-on-duration-type) 章節。

您可以在「工作總攬」區域中檢視指派給任務或問題的每個使用者或職務角色的計畫時數個別配置。
...
任務、問題或專案報告中的計畫時數是個別專案在詳細資訊區段或編輯專案方塊中顯示的總計畫時數。

如需建立報表的相關資訊，請參閱 [建立自訂報表](../../../reports-and-dashboards/reports/creating-and-managing-reports/create-custom-report.md).

>[!NOTE]
>
>如果您建立專案（財務資料）報告並按日期將其分組，則計畫時數可能會根據專案上任務的時間表顯示部分專案計畫時數。 依預設，Workfront會平均分配任務期間每天的計畫時數。 特定時間範圍的計畫時數符合Workfront在專案（財務資料）報表中為該時間範圍設定的相等分佈。
...
* 對於任務和問題，會顯示與其關聯的計畫時數。
* 對於專案，畫面上顯示的任務和問題的總計畫時數。

  >[!TIP]
  >
  >工作負載平衡器沒有顯示專案的所有計畫時數在專案詳細資訊區域。

您可以在工作負載平衡器中檢視指派給任務或問題的每個使用者的每日規劃時數分配。

計畫時數的每日小時量代表下列其中一項： 

* Workfront在任務、問題或專案持續期間的每天均分預設金額
* 資源管理員管理的調整後每日配置。

  如需有關在工作負載平衡器中調整每日分配的資訊，請參閱 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).
...
您可以在「資源規劃工具」的「計畫」欄中，檢視與工作專案相關聯之使用者與職務角色的每週計畫時數配置。

>[!TIP]
>
>工作負載平衡器中的每日配置調整會影響資源規劃工具中任務和問題的每週配置。

每個物件的計畫時數會依您套用至「資源規劃工具」的檢視而有所不同。 如需詳細資訊，請參閱 [資源規劃工具的「專案」與「角色」檢視中的時數、約當全職人數及成本資訊概要](../../../resource-mgmt/resource-planning/overview-of-planner-hour-fte-cost-information-in-role-project-views.md).

任務和問題的每週計畫時數表示下列其中一項：

* Workfront平均分配任務或問題期間每天的預設每週金額
* 調整後的每週配置，由工作負載平衡器中的資源管理員管理。

  如需有關在工作負載平衡器調整每日分配的資訊，請參閱 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).

專案、使用者和角色的每週金額會受與其相關之任務和問題的每週計畫時數金額影響。

### 使用率報表 {#the-utilization-report}

專案計畫時數是指與每個任務和問題的工作分派相關聯的時數。

>[!IMPORTANT]
>
>請注意，「使用率」報表中的「計畫時數」與指派相關聯，而不是與任務和問題本身相關聯。 使用率報告中的計畫時數並不總是與專案任務和問題的計畫時數相符。 但是，計畫時數不符合與任務和問題的指派相關聯的時數。

您可以在「使用率」報表中檢視下列型別的計畫時數：

* 專案上所有工作分派在包含專案之整個生命週期的總計畫時數
* 所有指派的總計畫時數僅針對指定的日期範圍（您可以指定個別周或月）。

  當使用工作負載平衡器調整使用者每日時數分配時，如果在使用率報告中選取的日期僅包含任務或問題期間的一部分，則特定日期範圍的規劃時數可能會受到影響。 如需有關調整使用者每日配置的資訊，請參閱 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).

如需詳細資訊，請參閱 [檢視資源使用率資訊](../../../resource-mgmt/resource-utilization/view-utilization-information.md).

### 角色配置面板

「角色配置」面板中的規劃時數代表在專案總期間內，與指派給專案上任務或問題的每個工作角色相關聯的規劃時數。 數字元合資源規劃工具中的角色「計畫時數」。

>[!TIP]
>
請注意，與使用者相關聯的計畫時數未顯示在角色配置面板中。

如需詳細資訊，請參閱 [在工作負載平衡器中顯示專案和方案的角色分配](../../../scenario-planner/show-role-allocation-workload-balancer.md).

## 根據期間型別更新任務計畫時數 {#update-task-planned-hours-based-on-duration-type}

只有在任務具有特定期間型別時，您才可以在編輯任務時更新任務的總計畫時數。

存在下列情況：

* 您只能在編輯任務時使用計算的指派或簡單期間型別來修改任務的計畫時數。

  如需有關計算指派持續時間型別的詳細資訊，請參閱 [期間型別概觀：計算的工作分派](../../../manage-work/tasks/taskdurtn/calculated-assignment.md).

  如需有關簡單期間型別的詳細資訊，請參閱 [期間型別概觀：簡單](../../../manage-work/tasks/taskdurtn/simple-duration-type.md).

* 當您管理使用者對任務的分配時，只能為簡單期間型別任務更新工作負載平衡器中的任務計畫時數。 如需在工作負載平衡器中管理使用者配置的相關資訊，請參閱 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).
* 您無法修改工期型別為投入比導向或已計算工時的任務的計畫時數。 在這些情況下，Workfront會根據任務的期間來決定計畫時數；但在這種情況下，計畫時數一律等於期間（以時數為單位），不受指派資源的百分比配置影響。

  如需投入比導向期間型別的詳細資訊，請參閱 [期間型別概觀：投入比導向](../../../manage-work/tasks/taskdurtn/effort-driven.md).

  如需有關計算的工作期間型別的詳細資訊，請參閱 [期間型別概觀：計算的工作](../../../manage-work/tasks/taskdurtn/calculated-work.md).

## 管理使用者分派時更新任務計畫時數

當您手動更新任務的使用者或職務角色配置時，可以更新任務的計畫時數。 僅當任務的期間型別為簡單時才能執行此操作。

如需詳細資訊，請參閱 [期間型別概觀：簡單](../../../manage-work/tasks/taskdurtn/simple-duration-type.md).

您可以更新指派給任務的使用者和角色的整體配置，或在使用工作負載平衡器時更新使用者每日配置。

如需有關管理工作的整體使用者與工作角色配置的資訊，請參閱 [管理任務的使用者和角色配置時數](../../../manage-work/tasks/assign-tasks/manage-allocation-hours-on-tasks.md).

如需有關管理每日任務分配的資訊，請參閱 [在工作負載平衡器中管理使用者配置](../../../resource-mgmt/workload-balancer/manage-user-allocations-workload-balancer.md).

手動更新任務的使用者或工作角色配置時，會出現下列情況：

* 如果您未手動更新個別使用者或角色指派以觸發對任務計畫時數的變更，則當您在任務上新增、移除或取代指派時，計畫時數保持不變。 將新指派新增至任務時，個別指派會重新分配至所有受指派人。
* 當您手動更新分配以觸發任務的變更計畫時數時，當您從任務移除指派時計畫時數會減少。 當您取代指定任務時，它們保持不變。
* 當您手動更新指派以觸發對任務計畫時數的變更並將指派新增到任務時，新指派預設會指派0小時。 您必須手動更新其分配，以處理可能影響計畫時數的任務。
* 如果您未手動更新指派以觸發對任務計畫時數的變更，且您移除任務的所有指派，則計畫時數保持不變。
* 當您手動更新指派以觸發任務的變更計畫時數，並移除任務的所有指派時，計畫時數也會被移除，且任務的計畫時數會變成0。

>[!NOTE]
>
例如，如果一項任務有10個計畫時數，而您有2個受指派人，則預設會各分配5個小時。
>
* 如果您沒有使用工作負載平衡器更新個別使用者配置或每日配置，並且您從任務中移除任何或所有受指派人，任務計畫時數仍為10小時。
* 如果您分別手動將工作分派的指派變更為4小時和6小時，並且您移除指派給6小時的使用者及其工作角色，則任務「計畫小時」將更新為4小時。 如果您也移除分配到4小時的使用者，但保留與已移除使用者相關聯的工作角色，則任務的規劃小時數將保留4小時。 如果您移除最後一個分配至4小時的使用者及其工作角色，並且任務仍然未指派，則任務的計畫時數會變成0。
...
如需使用工作量來預估任務工作量的相關資訊，請參閱 [工作投入概觀](../../../manage-work/tasks/task-information/work-effort.md).
