...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>編輯問題、任務和專案的存取權</p> <p>編輯財務資料的存取權，以更新專案從問題轉換的財務資訊</p> <p>注意：如果您還是沒有存取權，請詢問您的Workfront管理員，他們是否在您的存取層級中設定其他限制。 如需有關Workfront管理員如何變更您的存取層級的資訊，請參閱 <a href="../../../administration-and-setup/add-users/configure-and-grant-access/create-modify-access-levels.md" class="MCXref xref">建立或修改自訂存取層級</a>.</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">物件許可權</td> 
   <td> <p>檢視問題的許可權</p> <p>您可在問題轉換後取得專案的管理許可權</p> <p>如需請求其他存取許可權的詳細資訊，請參閱 <a href="../../../workfront-basics/grant-and-request-access-to-objects/request-access.md" class="MCXref xref">要求物件的存取權 </a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
1. 在顯示的問題清單中，執行下列任一項作業：
   ...
   * 若要使用範本將問題轉換為專案，請執行下列任一項作業：

      * 按一下問題的名稱，然後按一下 [!UICONTROL **更多**] 功能表 ![](assets/more-icon.png) 問題名稱右側

        ![](assets/issue-more-menu-expanded-with-convert-to-project-options-nwe-350x213.png)

        或

      * 在問題清單、報告或控制面板中選取問題，按一下 **更多** 功能表 ![](assets/more-icon.png) 在清單頂端，然後按一下 **從範本轉換為專案** 並開始在 **搜尋範本** 方塊，然後按一下範本在清單中顯示時的名稱。 繼續進行步驟3。
...
1. （視條件而定）如果您選擇使用範本將問題轉換為專案，請繼續下列步驟：
   ...
   1. （可選）將滑鼠移至範本名稱上，然後按一下「我的最愛」圖示 ![](assets/favorites-icon-small.png) 以標示為未來使用的最愛。
   ...
   1. 按一下 [!UICONTROL **自訂Forms**] 並執行下列任一項作業：
      ...
      * 如有必要，請將問題中的自訂表單資訊傳輸至專案。
        ...
        >* 如果您使用範本進行轉換，並且附加到範本的自訂表單包含附加到問題的自訂表單中也找到的自訂欄位，則問題的欄位值將用於新專案。 但是，如果問題的自訂欄位為空白，則會使用範本的值。
...
1. 按一下 [!UICONTROL **前往專案**] 內部 [!UICONTROL 成功] 通知的頁面。 如此將可開啟專案頁面。
