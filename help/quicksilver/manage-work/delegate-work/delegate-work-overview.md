...
有關委派專案、任務、問題和檔案核准的資訊，請參閱 [委派核准請求](../../review-and-approve-work/manage-approvals/delegate-approval-requests.md).
...
* 您的 [!DNL Workfront] 或群組管理員必須在以下位置啟用委派偏好設定： [!UICONTROL 設定] 將工作委派給他人之前的區域。
...
* 您只能從「 」委派任務和問題。 [!UICONTROL 首頁] 區域。
* 下列授權型別存在例外：

   * 您可以將工作委派給檢閱者或請求者，但 [!DNL Workfront] 不建議使用。
   * 檢閱者可以將工作委派給其他人。 他們無法檢視中的工作專案 [!UICONTROL 首頁] 區域。 他們只能檢視核准。
   * 請求者無法將工作委派給其他人。 他們無法檢視中的工作專案 [!UICONTROL 首頁] 區域
* 您只能委派指派給您的任務和問題。 您無法委派指派給其他使用者、團隊或工作角色的任務和問題。
* 您只能委派在委派開始日期之前未完成的任務和問題。
* 如果工作專案在委派時間範圍內完成，則該專案會保留在委派和被指派人之「首頁」區域中達2週前 [!DNL Workfront] 會自動將其移除。
...
* 多位使用者可選取同一位使用者作為委派。
* 委派的任務和問題未顯示在資源管理工具中，例如 [!UICONTROL 工作負載平衡器] 或 [!UICONTROL 資源規劃工具] 適用於委派的使用者。
* 您可以在下列幾個區域中檢視委派的工作和委派名稱 [!DNL Workfront]. 如需詳細資訊，請參閱文章中的「找出委派的工作並委派資訊」一節 [管理任務和問題](../delegate-work/how-to-delegate-work.md).
...
* 如果系統或停用 [!UICONTROL 允許使用者委派其任務和問題] 在中設定 [!UICONTROL 設定] 區域，目前委派的使用者會從先前已委派給的任務和問題中移除。 不會移除其對任務或問題的許可權。
