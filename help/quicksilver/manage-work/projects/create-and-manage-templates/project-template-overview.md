...
* 決定您要在未來的專案中使用的檔案儲存空間。 有些組織可以存取下列檔案儲存型別：

   * 舊版Workfront儲存空間
   * Adobe 雲端儲存空間

  新增檔案會依您選擇的儲存型別而有所不同。 您為範本選擇的儲存型別將影響未來專案將繼承的儲存型別。

  將範本新增至現有專案不會影響專案的儲存型別。

  如需詳細資訊，另請參閱：

   * [建立專案範本](/help/quicksilver/manage-work/projects/create-and-manage-templates/create-template.md)
