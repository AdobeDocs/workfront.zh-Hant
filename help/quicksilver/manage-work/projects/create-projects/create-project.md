...
* 身為Workfront管理員或群組管理員，您可以在為Workfront執行個體或群組設定專案偏好設定時，為新專案設定預設設定。
* Workfront會先套用群組的設定（如果有的話），再套用Workfront管理員設定的設定。
* 新項目的預設狀態與主「項目首選項」區域中由您的Workfront管理員或組管理員(或Workfront管理員)在組的「項目首選項」區域中定義的狀態相對應。
...
* Workfront如何定義新專案的群組和狀態，有下列案例：

   * 如果您從頭建立項目，項目組即為您的首頁組。

      「項目狀態」是「主組」的「項目首選項」（如果有）或「Workfront」實例的預設狀態。 建立專案時，您可以將預設狀態變更為專案群組可用的任何狀態。

   * 如果您使用範本建立專案，範本的設定優先於Workfront或群組管理員所建立的設定。

      新項目組是模板組。 如果模板未與組關聯，則項目組是建立項目的用戶的首頁組。

      從模板建立的新項目的預設狀態與主「項目首選項」區域中的Workfront管理員或組的「項目首選項」區域中的組管理員(或Workfront管理員)所定義的狀態相對應。 從模板建立項目時，您可以將預設狀態更改為項目組的任何狀態，該項目組是模板組或建立項目的用戶的首頁組。

   * 如果您透過轉換問題來建立專案，新專案的群組即為問題現有專案的群組。 如果轉換問題的使用者無權存取問題的專案，或問題的專案沒有群組，則新專案的群組是轉換問題的使用者的首頁群組。

      新項目的狀態與與項目關聯的組的組狀態匹配，該組是原始項目的組或轉換問題的用戶的主組。

      如果您在轉換問題以建立專案時使用範本，請參閱上述第二個案例，了解新專案適用的群組以及Workfront的狀態。
