...
1. （選擇性）在&#x200B;**自訂Forms**&#x200B;下，選取要新增至付費記錄的付費記錄自訂表單。
   ...
   您（或其他可存取自訂表單的使用者）必須先建立自訂表單的計費記錄，您才能在這裡選取它。 清單中只會顯示作用中的自訂表單。 如需建立自訂表單的相關資訊，請參閱[建立或編輯自訂表單](../../../administration-and-setup/customize-workfront/create-manage-custom-forms/create-or-edit-a-custom-form.md)。
