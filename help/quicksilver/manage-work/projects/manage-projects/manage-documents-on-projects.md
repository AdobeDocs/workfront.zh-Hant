...
>[!IMPORTANT]
>
>並非所有客戶都能同時存取Workfront和Adobe儲存空間。 有些客戶只能存取Workfront，有些客戶預設僅能存取Adobe企業儲存空間。 無法存取Workfront儲存空間的客戶無需任何設定。
...
儲存在Workfront儲存空間中物件上的檔案，其管理方式與Adobe企業儲存空間中的檔案不同。
...
* 您可以將Adobe儲存體範本附加至Workfront儲存體專案，但這不會變更專案的儲存位置。
