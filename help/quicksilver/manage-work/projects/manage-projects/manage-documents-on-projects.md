...
>[!IMPORTANT]
>
>您的Workfront執行個體可能無法同時存取Workfront和Adobe儲存空間。 有些Workfront執行個體僅能存取Workfront，其他則預設僅能存取Adobe企業儲存空間。 僅能存取一種儲存型別的客戶，不需要任何設定。
...
以下小節記錄當Workfront和Workfront企業儲存選項存在於相同環境中時，檔案儲存如何用於Adobe物件。
...
* 當您建立或將Adobe企業儲存專案移動到舊版Workfront儲存專案組合或方案時，如果組合或方案在新增專案之前沒有附加檔案，則專案組合或方案會自動轉換為Adobe企業儲存物件。
* 您無法為Workfront企業儲存產品組合或方案建立Adobe儲存專案。
* 當您從MS Project匯入專案時，Workfront會建立Workfront儲存專案，即使Workfront管理員將Adobe企業儲存設定為您的系統的預設值。
...
* 當您將Adobe儲存專案新增至Workfront儲存專案組合，且該組合附加有檔案時，該組合的檔案儲存空間仍會保留在Workfront儲存空間中。 不過，產品組合![舊版產品組合儲存圖示](assets/legacy-storage-project-icon.png)的舊版Workfront儲存圖示已從產品組合中移除。
...
* 當您建立Adobe企業儲存方案時，Workfront會在方案的檔案區段中建立一個資料夾，用來儲存檔案。 資料夾名稱與方案的名稱相同。 您無法刪除或手動重新命名資料夾。 如果您變更方案名稱，資料夾會重新命名，以符合方案的新名稱。

* 當您將Adobe儲存體專案新增至Workfront儲存體方案，而方案沒有附加任何檔案時，方案會轉換為Adobe儲存體方案。
* 當您將Adobe儲存體專案新增至Workfront儲存體方案，且方案具有附加的檔案時，方案檔案儲存體仍會保留在Workfront儲存體中。 不過，程式![舊版產品組合儲存圖示](assets/legacy-storage-project-icon.png)的舊版Workfront儲存圖示已從程式中移除。
* 您無法將Workfront儲存體專案新增到Adobe儲存體計畫。
...
* 將任務轉換為專案時有下列情況： <!--this info also duplicated in Convert tasks to projects-->
   * Workfront儲存任務會建立Workfront儲存專案。
   * Adobe企業儲存空間任務會建立Adobe儲存空間專案。
   * 使用Workfront儲存範本轉換Adobe儲存任務時，會建立Adobe儲存專案。
   * 使用Adobe儲存範本轉換Workfront儲存任務時，會建立Workfront儲存專案。
* 您無法在摘要面板中新增檔案至Adobe儲存空間任務。
...
* 當您提交具有附加至Workfront儲存專案之檔案的請求時，請求的「檔案」區域會使用專案的儲存型別來顯示檔案，即使系統儲存預設偏好設定為Adobe Enterprise亦然。
