---
product-area: projects
navigation-topic: approvals
title: 提醒核准者已提交的核准
description: 您可以提醒核准者下列物件的擱置核准 — 編輯我。
author: Courtney
feature: Work Management, Digital Content and Documents
exl-id: eece6fc8-14e8-4751-8662-080ecb4371e3
source-git-commit: 95679dd71ef7e4991853e63573a387f26321159d
workflow-type: tm+mt
source-wordcount: '453'
ht-degree: 3%

---

# 提醒核准者已提交的核准

您可以提醒核准者下列物件的擱置核准： 

* 存取要求
* 文件

## 存取需求

您必須具有下列存取權才能執行本文中的步驟：

<table style="table-layout:auto"> 
 <col> 
 <col> 
 <tbody> 
  <tr> 
   <td role="rowheader">Adobe Workfront計畫*</td> 
   <td> <p>任何</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">Adobe Workfront授權*</td> 
   <td> <p>要求或更高版本</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">存取層級設定*</td> 
   <td> <p>檢視專案、任務、問題、範本、Portfolio、方案、報告、儀表板和行事曆、檔案的或更高存取權</p> <p>注意：如果您還是沒有存取權，請詢問您的Workfront管理員，他們是否在您的存取層級中設定其他限制。 如需有關Workfront管理員如何修改您的存取層級的資訊，請參閱 <a href="../../administration-and-setup/add-users/configure-and-grant-access/create-modify-access-levels.md" class="MCXref xref">建立或修改自訂存取層級</a>.</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">物件許可權</td> 
   <td> <p>檢視或更高的存取許可權以存取與要求存取或核准相關聯的物件 </p> <p>如需請求其他存取許可權的詳細資訊，請參閱 <a href="../../workfront-basics/grant-and-request-access-to-objects/request-access.md" class="MCXref xref">要求物件的存取權 </a>.</p> </td> 
  </tr> 
 </tbody> 
</table>

&#42;若要瞭解您擁有的計畫、授權型別或存取權，請聯絡您的Workfront管理員。

## 存取要求

您可以請求下列物件的其他存取權：

* 專案
* 任務
* 問題
* 專案組合
* 計劃
* 範本
* 報告
* 儀表板
* 行事曆
* 文件

若要提醒其他使用者核准您的物件存取要求，請執行下列動作：

1. 按一下 **首頁** 圖示 ![](assets/home-icon-30x29.png) Adobe Workfront左上角。

   >[!NOTE]
   >
   >您的Workfront管理員可能會對您環境中的「首頁」圖示進行下列變更：
   >
   >* 以自訂的影像取代，以說明您的組織。 在此情況下，圖示看起來將會與本文中顯示的有所不同。
   >* 使用其他頁面取代連結至該頁面的頁面。 在此情況下，請按一下 **主要功能表** ![](assets/main-menu-icon.png) 然後按一下「 」 **首頁**.

1. 在 **工作清單** 區域，移至 **我已提交的核准** 分組。

1. 選取 **存取要求** 在工作清單中核准。

   這會開啟工作清單右側的存取要求。

   ![](assets/access-request-pending-approval-nwe-350x104.png)

1. 按一下 **提醒** 傳送通知給核准者。

## 文件

1. 按一下 **首頁** 圖示 ![](assets/home-icon-30x29.png) Adobe Workfront左上角。

   >[!NOTE]
   >
   >您的Workfront管理員可能會對您環境中的「首頁」圖示進行下列變更：
   >
   >* 以自訂的影像取代，以說明您的組織。 在此情況下，圖示看起來將會與本文中顯示的有所不同。
   >* 使用其他頁面取代連結至該頁面的頁面。 在此情況下，請按一下 **主要功能表** ![](assets/main-menu-icon.png) 然後按一下「 」 **首頁**.

1. 在 **工作清單** 區域，移至 **我已提交的核准** 分組。

1. 按一下 **檔案** 在工作清單中核准。

   這會開啟工作清單右側的檔案核准。\
   ![](assets/document-350x232.png)

1. 按一下 **管理核准** 在右面板的右上角。
1. 按一下 **提醒** 圖示以傳送通知給核准者。\
   ![提醒.png](assets/remind-350x41.png)
