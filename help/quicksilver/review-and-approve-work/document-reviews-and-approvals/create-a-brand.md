...
1. 在建立品牌之前，您必須先在Admin Console和Workfront存取層級中授與品牌許可權的存取權。 如需指示，請參閱[授予品牌許可權的存取權](/help/quicksilver/administration-and-setup/add-users/configure-and-grant-access/grant-access-brands.md)。
...
* 如果您無法移除主觀，請縮小其範圍。 以特定限製取代廣泛的形容詞。 例如，「直接和行動導向」會變成「直接和行動導向；省略填充字詞和對衝語言」。