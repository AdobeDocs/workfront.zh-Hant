...
title: Adobe雲端儲存模式的物件許可權和存取層級總覽
description: Adobe雲端儲存空間許可權和存取許可權概覽
author: Courtney
...
Adobe雲端儲存是以雲端為主的儲存解決方案，可作為Adobe企業產品中資產的中央儲存庫。 使用Adobe雲端儲存空間的Workfront環境與使用舊版Workfront檔案儲存空間的環境相比，物件許可權和存取層級行為稍有不同。

## 存取層級

Workfront存取層級僅適用於Workfront。 Workfront中的專案和檔案限制並不一定適用於其他Adobe應用程式。

### 同時使用Adobe雲端儲存空間與舊版Workfront儲存空間的環境

視專案位於Adobe雲端儲存空間或舊版Workfront儲存空間而定，檔案存取的行為會有所不同：
...
您無法修改使用Adobe雲端儲存空間之專案、程式和產品組合的存取層級檔案許可權。
...
>[!IMPORTANT]
>
>在Adobe雲端儲存空間中，檔案許可權的運作方式與舊版Workfront儲存空間不同。 檔案會繼承其連結的專案、任務或問題的許可權。
