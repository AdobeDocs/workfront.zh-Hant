---
content-type: reference
product-area: documents
navigation-topic: manage-proof-deadlines
title: 新增、編輯或刪除階段的截止日期
description: 您可以調整建立或擁有的校訂截止日期。
author: Courtney
feature: Digital Content and Documents
exl-id: 23010732-dab7-461d-876a-56f9928a2a23
source-git-commit: 49950895440fec8cebdf12ec81191c6e890383cf
workflow-type: tm+mt
source-wordcount: '288'
ht-degree: 2%

---

# 新增、編輯或刪除階段的截止日期

您可以調整建立或擁有的校訂截止日期。

## 存取需求

您必須具有下列存取權才能執行本文中的步驟：

<table style="table-layout:auto"> 
 <col> 
 <col> 
 <tbody> 
  <tr> 
   <td role="rowheader">Adobe Workfront計畫*</td> 
   <td> <p>目前計畫：Pro或更高版本</p> <p>或</p> <p>舊版計畫：選擇或Premium</p> <p>如需有關不同方案的校訂存取許可權的詳細資訊，請參閱<a href="/help/quicksilver/administration-and-setup/manage-workfront/configure-proofing/access-to-proofing-functionality.md" class="MCXref xref">存取Workfront中的校訂功能</a>。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">Adobe Workfront授權*</td> 
   <td> <p>目前計畫：工作或計畫</p> <p>舊版計畫：任何（您必須為使用者啟用校訂）</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">校樣權限設定檔 </td> 
   <td>經理或以上</td> 
  </tr> 
  <tr> 
   <td role="rowheader">校樣角色</td> 
   <td>您必須是校訂擁有者或建立者</td> 
  </tr> 
  <tr> 
   <td role="rowheader">存取層級設定*</td> 
   <td> <p>編輯檔案的存取權</p> <p>如需請求其他存取權的資訊，請參閱<a href="../../../../workfront-basics/grant-and-request-access-to-objects/request-access.md" class="MCXref xref">請求物件</a>的存取權。</p> </td> 
  </tr> 
 </tbody> 
</table>

&#42;若要瞭解您擁有的計畫、角色或校訂許可權設定檔，請連絡您的Workfront或Workfront Proof管理員。

## 新增、編輯或刪除階段的截止日期

1. 將滑鼠停留在校訂上，然後按一下&#x200B;**校訂工作流程**&#x200B;以開啟「工作流程」頁面。
1. 找到您要管理的階段，請執行下列任一項作業：

   <table>
      <tbody>
      <tr>
      <td>新增截止日期至階段</td>
      <td>連按兩下<strong>無截止日期</strong>，然後指定截止日期的日期和時間。</td>
      </tr>
      <tr>
      <td>編輯現有期限</td>
      <td>按兩下舞台右上角的日期和時間，然後指定截止日期的日期和時間。</td>
      </tr>
      <tr>
      <td>刪除現有期限</td>
      <td>按兩下舞台右上角的日期和時間，然後按一下出現的[行事曆]方塊底部的[清除]。<strong></strong></td>
      </tr>
      </tbody>
      </table>


1. 若要儲存變更，請在截止日期方塊外按一下。
1. 在出現的提示中，指定是否要傳送電子郵件警示給稽核者。 
