...
>[!TIP]
>
>建議對需要授權和密碼保護頁面的內部頁面使用互動式校樣，而非靜態校樣。 如需詳細資訊，請參閱 [互動式內容校訂概觀](../../../review-and-approve-work/proofing/proofing-overview/interactive-content-proofs.md).

1. 前往您要建立新網站校訂或現有網站校訂新版本的專案、任務或問題。
1. 按一下 **檔案** 在左側面板中。
1. （視條件而定）如果您要建立新校樣，請按一下 **新增**，然後按一下 **證明** 在出現的選單中。
...
1. 輸入您要在中校訂的網站URL **新增檔案** 區域，然後按 **輸入**.
...
1. （選用）如果要將網站URL的校訂名稱變更為其他名稱，請輸入 **校訂名稱。**
1. 確定 **擷取熒幕擷圖** ，並使用下列任一選項：
...
1. 按一下 **完成**.
   ...
   如果您在步驟8中選取了多個熒幕擷圖解析度，清單會包含每個解析度的一組熒幕擷圖。 您可以產生這些熒幕擷取畫面做為個別校訂，或將其合併為單一校訂（請參閱中的）。 建議您將兩者合併，尤其是當您要建立靜態網站校訂時。
