...
| [檢閱和核准檢視者](#review-and-approval-viewer) | Frame.io檢視器會取代Workfront校訂檢視器。 | 包含所有Workfront使用者，包括指派給稽核或核准的外部使用者。 支援標示、時間戳記註解、版本記錄、行動裝置、40多種格式、最大500 GB的檔案。 |
| [物件命名規則](#object-naming-rules) | 套用嚴格的命名規則：產品組合或專案中的唯一名稱、無特殊字元、無結尾句點或空格、255字元限制。 | 發生衝突時，Workfront會自動重新命名物件。 產生新專案名稱和結構的稽核範本。 |
| [物件可攜性](#object-portability) | 在大多數情況下，您只能在類似儲存模型之間移動、複製和轉換物件。 | 您可以在三種特定情況下，將舊版物件轉換為Adobe雲端儲存空間。 在轉換期間，檔案和檔案資料夾不會從舊版儲存空間移動。 |
...
在Adobe雲端儲存物件上，Frame.io檢視器是稽核和核准介面，取代Workfront Proof。 所有Workfront使用者皆可免費使用Frame.io檢視器，包括指派給檢閱或核准的外部使用者。
...
在大多數案例中，您可以在類似儲存模式之間移動、複製和轉換Workfront物件。 例如，您可以將任務從一個Adobe雲端儲存空間專案移動至另一個Adobe雲端儲存空間專案。 在三種特定情況下，您可以將舊版Workfront儲存體物件轉換為Adobe雲端儲存體：
...
>[!NOTE]
>
>在這三種轉換情況下，檔案和檔案資料夾不會從舊版Workfront儲存空間移至Adobe雲端儲存空間。 轉換前存在於舊版物件上的檔案仍會保留在舊版儲存體上。

#### 將舊版任務轉換為Adobe雲端儲存空間專案

若要將舊版Workfront儲存體任務轉換為Adobe雲端儲存體專案，請在該任務上使用現有的轉換至專案流程。 轉換期間：

* 子任務和問題移至新專案。
* 附加到任務的檔案及其核准工作流程會保留在原始專案上。
* 已移除工作核准和解析物件連結。
* 已刪除原始任務。
...
#### 將舊版產品組合轉換為Adobe雲端儲存產品組合

Workfront管理員可從「設定」區域將舊版Workfront儲存產品組合轉換為Adobe雲端儲存產品組合。 轉換後：
...
如需詳細資訊，請參閱[將舊版產品組合轉換為Adobe雲端儲存空間](/help/quicksilver/administration-and-setup/set-up-workfront/configure-system-defaults/convert-portfolios-to-acs.md)。

#### 從舊版範本建立Adobe雲端儲存空間專案

當您從舊版Workfront儲存範本建立專案時，專案建立對話方塊中的&#x200B;**在Adobe雲端儲存空間上建立此專案**&#x200B;核取方塊會決定新專案的儲存型別。 核取方塊行為取決於您建立專案的位置：

* **在投資組合外**：預設會提供並清除核取方塊。 選取它以在Adobe雲端儲存空間上建立新專案。
* **在Adobe雲端儲存產品組合內**：核取方塊已選取並鎖定。 新專案必須符合產品組合的儲存型別。
* **在舊版Workfront儲存產品組合中**：無法使用核取方塊。 新專案使用舊版Workfront儲存空間。

如需詳細資訊，請參閱[建立專案](/help/quicksilver/manage-work/projects/create-projects/create-project.md)。
