...
現在，項目的利用率報告將考慮計畫小時數是否已在任務期間重新分配。 修改小時的用戶分配時(如 [在「計畫」區域中管理用戶分配](../../../../resource-mgmt/resource-scheduling/manage-allocations-scheduling-areas.md) in [在「計畫」區域中管理用戶分配](../../../../resource-mgmt/resource-scheduling/manage-allocations-scheduling-areas.md))，則如果在「利用率報告」中選擇的日期僅包含任務的一部分，則「利用率報告」中的資料可能會受到影響。
