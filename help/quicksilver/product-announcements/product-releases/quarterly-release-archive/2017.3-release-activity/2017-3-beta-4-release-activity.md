...
如需有關複製工作的詳細資訊，請參閱[複製和複製工作](../../../../manage-work/tasks/manage-tasks/copy-and-duplicate-tasks.md)。
...
現在，執行「使用率」報告時，除了先前可用的「任務」、「問題」和「角色」欄位外，建立篩選器時，可使用「投資組合」、「計畫」和「專案」的新篩選欄位。
...
## 依計畫或Portfolio檢視使用率報表 {#view-the-utilization-report-by-program-or-portfolio}
