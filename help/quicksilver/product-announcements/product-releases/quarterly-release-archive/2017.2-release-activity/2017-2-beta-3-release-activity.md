...
## 匯出至PDF時包含甘特圖圖例 {#include-the-gantt-chart-legend-when-exporting-to-pdf}

現在將甘特圖匯出至PDF時，您可以選取是否也要連同圖表本身一併匯出圖表的圖例。 圖例中包含的專案只是您已啟用在UI的甘特圖中顯示之選項。 如果這些選項存在於專案上的任務中，則會包含在圖例中。 例如，如果您啟用在甘特圖中顯示里程碑，圖例也會顯示里程碑，但前提是至少有一個工作與里程碑關聯。
