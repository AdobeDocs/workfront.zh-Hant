---
content-type: release-notes
keywords: 備註，每季，更新，發行
navigation-topic: 2021-2-release-activity
title: 21.2專案增強功能
description: 本頁說明21.2版對預覽環境所做的所有專案增強功能。 這些增強功能將在2021年5月10日當週的生產環境中提供。 如需21.2版所有可用變更的清單，請參閱21.2版總覽。
author: Luke
feature: Product Announcements
recommendations: noDisplay, noCatalog
exl-id: 4e954d79-da5a-42f5-b43f-b1fdd17316d2
source-git-commit: b18a7835c6de131c125b77c6688057638c62fa4a
workflow-type: tm+mt
source-wordcount: '1653'
ht-degree: 1%

---

# 21.2專案增強功能

本頁說明21.2版對預覽環境所做的所有專案增強功能。 這些增強功能將在2021年5月10日當週的生產環境中提供。 如需21.2版本可用的所有變更清單，請參閱[21.2版本概觀](../../../product-announcements/product-releases/21.2-release-activity/21-2-release-overview.md)。

## 用於建立和編輯週期性任務的新介面

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

在重新設計「新增」和「編輯任務」方塊時，我們新增了使新任務重複執行以及編輯重複執行任務之父系的功能。 此功能未包含在先前發行的「新增及編輯任務」方塊重新設計中。

如需詳細資訊，請參閱[建立週期性工作](../../../manage-work/tasks/create-tasks/create-recurring-tasks.md)。

此功能現在包含在新Workfront體驗的[規劃基礎中，第2部分：在Workfront One上規劃專案](https://experienceleague.adobe.com/zh-hant/docs/workfront-learn/tutorials-workfront/home)學習路徑。

## 專案標題中可用的新按鈕

>[!NOTE]
>
>預覽版尚未提供此功能。 它將於2021年4月29日發佈到預覽環境。

為了讓您快速篩選「專案」頁面上的專案清單，標題中有兩個新按鈕，可讓您快速套用我所在的專案或我擁有的專案篩選。

此功能現在包含在新Workfront體驗的[規劃師基礎課程第1部分：在Workfront One上建立專案](https://experienceleague.adobe.com/zh-hant/docs/workfront-learn/tutorials-workfront/home)學習路徑。

## 從Scrum展示板移動或刪除劇本或問題

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

您現在可以按一下內文或問題卡上的「更多」圖示，並選取「移至」，將內文或問題移出Scrum面板。 如果定義了未來的版序，則可將專案移動到另一個版序。 您也可以將專案移至團隊的待辦專案。

您也可以按一下卡片上的「更多」圖示並選取「刪除」，選擇直接從Scrum面板刪除內文或問題。 當您刪除內文或問題時，內文或問題會移至資源回收筒並保留30天，且只有系統管理員才能復原。

如需詳細資訊，請參閱[管理Scrum展示板上的劇本和問題](../../../agile/use-scrum-in-an-agile-team/scrum-board/manage-scrum-board.md)。

## 新增編輯問題方塊

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

為了更新新Workfront體驗的外觀與風格，我們重新設計了「編輯問題」方塊。 您可以從個別問題存取新的「編輯問題」方塊，或在從清單編輯單一問題時進行存取。

除了更新的外觀和感覺外，「問題任務」方塊中還提供以下變更：

* 您可以自訂版面範本一次，並在「詳細資訊」頁面和「編輯問題」方塊中反映這些自訂。
* 現在，個別自訂表單名稱在編輯問題方塊的左側面板上可用，您可以從那裡快速存取每個表單。
* 我們已將計畫時數欄位新增到指派區段，以便更輕鬆地更新完成問題可能需要資源的時間。
* 我們已從編輯問題畫面中移除評論功能，以消除更新區段的備援。

## 重新定位「編輯」和「新增物件」方塊的「儲存」和「取消」按鈕

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

根據使用者的意見反應，我們已將「儲存」和「取消」按鈕移動到專案和任務的「編輯物件」和「新物件」方塊的左下角。

在此增強功能之前，這些按鈕位於這些方塊的右上角。

如需有關編輯專案或任務的資訊，請參閱下列文章：

* [編輯專案](../../../manage-work/projects/manage-projects/edit-projects.md)
* [編輯任務](../../../manage-work/tasks/manage-tasks/edit-tasks.md)

![編輯專案範例](assets/pasted-image-4-5-2021-0-350x278.png)

## 自訂表單增強功能

>[!NOTE]
>
>此功能僅在新的Adobe Workfront體驗中可用。

為了在您填寫自訂表單時獲得更好的體驗，我們改進了長自訂欄位標籤的顯示方式。 當有足夠的水平空間來完整顯示它們時，這些標籤將不再被截斷。

如需填寫自訂表單的相關資訊，請參閱[編輯自訂表單欄位中的資訊](../../../workfront-basics/work-with-custom-forms/edit-custom-forms.md)。

## 問題的主要連絡人現在可以從專案詳細資訊區域看到

>[!NOTE]
>
>此功能於2020年3月10日發佈到生產環境。

將問題轉換為專案時，問題的請求者會轉移至專案，並顯示在「轉換的問題建立者」欄位中。 您可以在專案詳細資訊區域，以及專案報告和清單中檢視此欄位。 只有報告和清單中的工作才能看到這項資訊。

您必須在版面配置範本中啟用此欄位，才能在專案的新Adobe Workfront體驗中檢視它。

如需轉換問題的相關資訊，請參閱[在Adobe Workfront中轉換問題的概述](../../../manage-work/issues/convert-issues/convert-issues.md)。

## 檢視您要指派給投資組合、方案或團隊之群組的相關資訊

>[!NOTE]
>
>此功能僅在新的Adobe Workfront體驗中可用。

當您指派群組給投資組合、方案或團隊時，我們讓您更容易確定您擁有正確的群組 — 您可以按一下群組名稱旁的資訊圖示，以顯示工具提示，其中包含群組上方的階層以及群組的管理員。 根據為群組設定的詳細資訊，您也可能看到群組的業務主管和說明。

這是最近發行的相同功能，可協助您識別需要指派給專案的群組，如[編輯專案](../../../manage-work/projects/manage-projects/edit-projects.md)中所述。

如需詳細資訊，請參閱下列文章：

* [建立投資組合](../../../manage-work/portfolios/create-and-manage-portfolios/create-portfolios.md)
* [建立程式](../../../manage-work/portfolios/create-and-manage-programs/create-program.md)
* [建立團隊](../../../people-teams-and-groups/create-and-manage-teams/create-a-team.md)

## 自訂欄位中的文字格式現在會顯示在所有Workfront Classic清單和報表中

>[!NOTE]
>
>此功能僅適用於Adobe Workfront Classic。

使用者現在可以在檢視清單和報表時，在自訂欄位中檢視文字格式，例如粗體、專案符號清單和超連結。

先前，在Workfront Classic中，文字格式只顯示在專案、任務和小時清單中。

如需有關在自訂欄位中格式化文字的資訊，請參閱[建立自訂表單](https://experienceleague.adobe.com/zh-hant/docs/workfront/using/home)。

如需有關在清單中顯示資訊的資訊，請參閱[開始使用清單](https://experienceleague.adobe.com/zh-hant/docs/workfront/using/home)。

## 新增編輯任務方塊

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

為了更新新Workfront體驗的外觀與風格，我們重新設計了「編輯任務」方塊。 您可以從個別任務存取新的「編輯任務」方塊，或在從清單編輯單一任務時存取該方塊。

>[!NOTE]
>
>在此更新中，「資源範圍」欄位將從新的Adobe Workfront體驗中移除。

除了已更新的外觀和感覺外，「編輯任務」方塊中還提供下列變更：

* 您可以自訂「版面配置範本」一次，並在「詳細資訊」頁面和「編輯工作」方塊中反映這些自訂。
* 現在，各個自訂表單名稱在編輯任務方塊的左側面板上可用，您可以從那裡快速存取每個表單。
* 我們已將期間、期間型別和計畫時數欄位新增到「工作總攬」區段，以在更新各種期間型別中的指派以及他們花費在任務上的時間或期間時提供一致性。
* 我們已從編輯工作畫面中移除評論功能，以消除「更新」區段的備援。

如需有關新編輯工作方塊的資訊，請參閱[編輯工作](../../../manage-work/tasks/manage-tasks/edit-tasks.md)。

## 新任務方塊

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

為了更新新Workfront體驗的外觀與風格，我們重新設計了「新任務」方塊。 按一下工作清單頂端的「新增工作」，即可存取「新增工作」方塊，將工作新增至專案。

>[!NOTE]
>
>在此更新中，「資源範圍」欄位將從新的Adobe Workfront體驗中移除。

除了已更新的外觀和感覺外，「新增任務」工作流程中還提供下列變更：

* 當您在專案清單中首次按一下「新增任務」時，可以使用較小的「新增任務」畫面，其中要更新的欄位數量有限，這樣當您想要快速將任務新增到專案時，就可以節省時間。 您可以存取完整的新任務表單，其中包含我們已重新命名為「建立任務」的所有工作列位。
* 您可以自訂「版面配置範本」一次，並在「詳細資訊」頁面和「新增任務」方塊中反映這些自訂。
* 我們已將期間、期間型別和計畫時數欄位新增到指派區段，以便在更新指派時以及在任務上所花費的時間或期間保持一致。

如需有關新任務方塊的資訊，請參閱[在專案中建立任務](../../../manage-work/tasks/create-tasks/create-tasks-in-project.md)。

## 停用不再使用的團隊

>[!NOTE]
>
>此功能將在2021年4月15日（21.2版本之前）發行到生產環境。

您現在可以停用不再使用的團隊。 當您停用團隊時，它不再出現在Workfront的大部分預先輸入、搜尋欄位或常見選單中。 但是，會保留與團隊相關的歷史資料。

如需詳細資訊，請參閱[停用團隊](../../../people-teams-and-groups/create-and-manage-teams/deactivate-a-team.md)。

## 更新為允許使用者刪除具有記錄時數的任務和問題的設定

>[!NOTE]
>
>僅於新的Adobe Workfront體驗中提供。

為了讓您在所有物件上獲得一致的體驗，我們變更了Workfront中「允許使用者刪除具有記錄時數的任務和問題」設定的運作方式。 現在，當您的Workfront或群組管理員停用任務和問題偏好設定時，您無法再刪除包含具有記錄時數的任務或問題的專案。

在此變更之前，停用設定會防止使用者僅刪除任務或問題，而不會刪除專案。

如需詳細資訊，請參閱[設定全系統的任務和問題偏好設定](../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-task-issue-preferences.md)。

