---
title: 取代Adobe Workfront中的Flash工具
description: 取代Adobe Workfront中的Flash工具
author: Luke
draft: Probably
...
Workfront現在提供根據目前標準的取代工具。 這些變更與Adobe宣佈終止支援Flash產品一致。
...
* **2020年11月19日**：已從所有Workfront產品中移除所有Flash工具。
...
如需有關取代工具的資訊，請參閱本文中的[舊版Flash工具及其取代](#legacy-flash-based-tools-and-their-replacements)。
...
* [在網頁校訂檢視器中檢閱校訂](../../../workfront-proof/wp-work-proofsfiles/review-proofs-wpv/review-proofs-in-wpv.md)
 ...
 <tbody> 
  <tr> 
   <td> <p><strong>舊版資源集區</strong> </p> <p>舊版資源集區是完成專案時同時需要的職務角色群組或集合。 舊版資源集區有許多缺點：</p> 
    <ul> 
     <li> 您可以將使用者與一個舊版資源集區建立關聯，但該集區僅用於報表用途。 由於「舊版資源集區」是以抽象職務角色實體來運作，因此未考慮任何使用者的排程例外與休假，導致有關資源可用性的資料不準確。 </li> 
    </ul> 
    <ul> 
     <li>您只能為每個專案指定一個舊版資源集區，這會導致多個舊版資源集區僅支援獨立工作的不同群組流程，且永遠不會共用資源。 在所有其他情況下，建議保留一個包含系統中所有資源的舊版資源集區，這會導致大量專案和資料的效能問題。</li> 
    </ul> </td> 
   <td> <p><strong>資源集區</strong> </p> <p>新資源集區是同時需要用來完成專案的使用者集合。 Workfront也瞭解專業使用者有時需要個別將工作分配給自己。 因此，您現在可以預算使用者，而非職位角色。 </p> 資源集區較舊式資源集區的優點包括： 
    <ul>
     <li>由於資源集區是以使用者為基礎，因此計算使用者和角色可用性時，已考慮其休假和排程例外。 這樣可產生精確且最新的資料，以便做出正確的預算決定，並儘可能減少專案執行期間發生變更的可能性。</li>
     <li>由於現在對資源的可用性和資料的準確性有更多的控制權，Workfront可讓您將多個資源集區與專案建立關聯。 一位使用者也可以屬於多個資源集區，如果他們擁有可同時用於多個專案的多種技能。 </li>
    </ul><p>如此控制資料後，就不再需要有一個資源集區，其中包含分配可用預算的所有資源。 事實上，我們不建議這麼做。 反之，我們建議將您的資源集區多元化，並僅將相關的資源集區與專案建立關聯。</p><p> 如需資源集區的詳細資訊，請參閱<a href="../../../resource-mgmt/resource-planning/resource-pools/work-with-resource-pools.md" class="MCXref xref">資源集區概觀</a></p></td> 
  </tr> 
  <tr> 
   <td> <p><strong>資源預算管理員</strong> </p> <p>您可以使用資源預算管理員來指定多個資源集區間工作角色資源的可用性。 然而，由於舊版資源集區的缺點，此功能很少使用。 使用時，它會強制使用者手動輸入職位角色的可用性，以使預算更準確。 未考慮排程例外狀況和使用者的休假。</p> </td> 
   <td> <p>根據資源集區中的使用者自動計算可用性時，不再需要資源預算管理員。 此工具已淘汰，所有計算可用性的手動工作都已淘汰。</p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>資源預計</strong> </p> <p>每個舊版資源集區底下的「資源預計」標籤與「資源預算管理員」用途相同，但僅限於一個舊版資源集區的內容。 此工具與Resource Budget Manager和舊版資源集區有相同的限制：不正確的資料和手動輸入可用性。 </p> </td> 
   <td> <p>隨著使用者可用性的自動計算，「資源估計」已過時，並已移除。</p> <p>在專案的業務案例中，該工具在舊版資源集區和舊版資源估計上被清除。
   <!--
      <MadCap:conditionalText data-mc-conditions="QuicksilverOrClassic.Draft mode">
  ...
  <tr> 
   <td> <p><strong>能力規劃工具</strong> </p> <p>Capacity Planner是一種Workfront工具，用於根據資源的可用性，為舊版資源集區內的資源編列預算並排定專案的優先順序。 由於資源估計和資源預算管理員提供能力規劃工具資訊的資料不完整，因此必須針對使用者的可用性來重複檢查專案優先順序。</p> <p>最常見的案例是使用包含系統中所有職務角色的單一舊版資源集區，這會導致能力規劃工具嘗試載入大量專案時出現效能問題。</p> </td> 
   <td> <p><strong>資源規劃工具的專案檢視</strong> </p> <p>在「資源規劃工具」的「專案型」檢視中，您可以像在「舊版產能規劃工具」中一樣編列資源預算並排定專案的優先順序。 與舊版工具不同，現在支援更多資料，考慮到使用者的休假和排程例外情況，可用資訊會更精確。</p> <p>可用的、計畫的及預算的資訊一目瞭然，這樣「資源管理員」就可以檢視是否有足夠人員執行工作，以及專案計畫是否超過初始預算估計。</p> <p> 如需有關在資源規劃工具中使用專案檢視的資訊，請參閱<a href="../../../resource-mgmt/resource-planning/get-started-resource-planner.md" class="MCXref xref">資源規劃工具概觀</a></p> <p><strong>情境規劃工具</strong> </p> <p>對於長期容量規劃、假設案例建模和優先順序，我們也推出了Workfront案例規劃工具。 </p> <p>Scenario Planner僅在新的Adobe Workfront體驗中可用，並且需要額外的授權。 如需Workfront Scenario Planner的相關資訊，請參閱<a href="../../../scenario-planner/scenario-planner-overview.md" class="MCXref xref">Scenario Planner概觀</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>舊版資源估算（業務案例）</strong> </p> <p>您可以使用業務案例的「舊版資源估算」區域，將特定的人工時數與成本預算為專案計畫與資源請求的一部分。 此檢視未提供資源可用性的任何可見度，這會導致資源請求的大致性，並增加拒絕專案工作的機會。</p> </td> 
   <td> <p><strong>資源預算（商業案例）</strong> </p> <p>「業務案例」下的「資源預算」區段將「資源規劃工具」功能帶入業務案例，提供使用者和角色可用性的可見度，以及在使用者層級預算的能力。 </p> <p> 如需業務案例的資源預算區域相關資訊，請參閱<a href="../../../manage-work/projects/define-a-business-case/areas-of-business-case.md" class="MCXref xref">業務案例區域概觀</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>資源估計報告</strong> </p> <p>使用舊版資源管理工具時，您可以從業務案例報告預算和計畫時數。 這可讓您建立矩陣報告，顯示特定時間範圍內每個工作角色的預算與計畫工作總計。 此報表無法編輯，且您無法根據報表發現專案對資源預算進行任何變更。 </p> </td> 
   <td> <p><strong>使用報告</strong> </p> <p>內建使用率報告會並排顯示計畫、預算和實際時數、成本和收入。 </p> <p>如需有關使用率報告的資訊，請參閱<a href="../../../resource-mgmt/resource-utilization/view-utilization-information.md" class="MCXref xref">檢視資源使用率資訊</a>。 </p> 
    <div> 
     <p><strong>可報告預算時數</strong> </p> 
     <p>建立預算時數的報告以檢閱報告表單中資源規劃工具中的預算時數。 </p> 
     <p>如需有關預算時數的資訊，請參閱<a href="../../../workfront-basics/navigate-workfront/workfront-navigation/workfront-terminology-glossary.md" class="MCXref xref">Adobe Workfront術語辭彙表</a>。</p> 
     <p>如需建立報告的詳細資訊，請參閱<a href="../../../reports-and-dashboards/reports/creating-and-managing-reports/create-custom-report.md" class="MCXref xref">建立自訂報告</a>。</p> 
    </div> <p><strong>資源規劃工具角色檢視</strong> </p> <p>舊版資源管理工具中業務案例的預算與計畫時數，現在可以在新的原生檢視 — 資源規劃工具的角色型檢視中使用。 此檢視表提供可用時數、計畫時數和預算時數等一目瞭然的資訊，可讓您在同一位置控制和變更預算。 這確保在高層級工作角色規劃期間做出更好的決策。 </p> <p> 如需有關資源規劃工具角色檢視中預算資源的資訊，請參閱<a href="../../../resource-mgmt/resource-planning/get-started-resource-planner.md#using2" class="MCXref xref">資源規劃工具概觀</a>中的<a href="../../../resource-mgmt/resource-planning/get-started-resource-planner.md" class="MCXref xref">使用專案和角色檢視來預算資源</a>區段</p> </td> 
  </tr> 
  <tr> 
    ...
    <ul> 
     <li> <p>分析階段的<strong>：</strong> </p> 
      <ul> 
       ...
       <li> <p><strong>資源規劃工具的使用者檢視</strong><strong>：</strong>嘗試在更高的層級瞭解您的使用者要分配哪些專案時，請使用資源規劃工具的使用者檢視。 這可讓您檢視使用者正在處理哪些工作，以及他們在特定時間範圍內的過度與不足配置。 資源規劃工具也會提供使用者整體配置的視覺效果，以及實際記錄時數的視覺效果，有助於分析完成工作的進度。 </p> <p>如需有關在資源規劃工具中使用使用者檢視的資訊，請參閱<a href="../../../resource-mgmt/resource-planning/get-started-resource-planner.md#using" class="MCXref xref">資源規劃工具概觀</a>中的<a href="../../../resource-mgmt/resource-planning/get-started-resource-planner.md" class="MCXref xref">使用使用者檢視來檢視可用、計畫和實際時數，或FTE </a>區段</p> </li> 
      </ul> </li> 
     <li>戰術階段的<strong>：</strong> 
      <ul> 
       <li><strong>工作負載平衡器</strong>您可以使用工作負載平衡器執行下列動作： 
        <ul>
         <li>指派工作給使用者。</li>
         <li>管理使用者對工作專案的配置。 </li>
  ...
  <tr> 
   <td> <p><strong>舊版甘特圖，工作清單</strong> </p> <p> 工作清單上的「舊版甘特圖」讓使用者能夠檢視專案的時間表，並執行假設案例計畫，而不需將變更提交至資料庫。 舊版甘特圖是以Flash技術為基礎，這帶來了安全性風險。 </p> </td> 
   <td> <p><strong>甘特圖，</strong> <strong>工作清單</strong></p> <p> 新的HTML式甘特圖與舊版甘特圖的用途相同。 使用者可以透過變更工作清單工具列中的「手動儲存」選項，將專案的時間表視覺化，並執行假設案例計畫，而不將變更提交至資料庫。 </p> <p>使用「自動儲存」選項時，新的甘特圖是互動式的，當您想要在變更發生時自動儲存變更時，可以使用此選項。 </p> <p>新的工作清單甘特圖採用最新技術，非常可靠。 這個新的甘特圖直接位於工作清單上，當在工作清單上工作時，無需切換標籤或變更檢視，即可輕鬆存取。 </p> <p>雖然新的甘特圖提供與舊版圖表相同的功能，但與舊版甘特圖在功能上仍有一些差異。 </p> <p> 範本上範本任務清單中的「舊版甘特圖」子標籤、「範本任務」之「子任務」標籤中的「舊版甘特圖」檢視以及「範本任務」報告中的「舊版甘特圖」也已被以HTML為基礎的甘特圖取代。 </p> <p>如果您主要使用「舊版甘特圖」進行簡易檢視與快速編輯，且不使用實際圖表，則新的「時程表計畫」選項可讓您快速變更主要計畫欄位。 您可以從工作清單工具列選取「時程表計畫」而非「自動儲存」。</p> <p>如需使用「時程表計畫」選項儲存工作清單的詳細資訊，請參閱<a href="../../../manage-work/tasks/manage-tasks/edit-tasks-in-a-list.md" class="MCXref xref">編輯清單中的工作</a>一文中的「當您選取「時程表計畫」選項時，手動儲存工作清單中的變更」一節。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>專案清單的舊版甘特圖</strong> </p> <p>專案清單上的舊版甘特圖讓使用者能夠在一個檢視中檢視專案及其任務。 使用者無需離開專案清單的內容，即可檢視專案中任務的詳細資訊以及專案之間的相依性。 專案清單上的舊版甘特圖是以Flash技術為基礎的，這帶來了安全性風險。 </p> </td> 
   <td> <p><strong>甘特圖，專案清單</strong> </p> <p>HTML式甘特圖與舊版甘特圖的用途相同。 使用者可以在一個檢視中檢視專案及其任務，以視覺化方式識別專案與任務之間的相依性。 專案清單甘特圖直接位於專案清單上。 新的甘特圖具有現代化的介面，並且建構在最新的技術上。</p> <p>如需有關專案清單甘特圖的資訊，請參閱<a href="../../../manage-work/gantt-chart/use-the-gantt-chart/view-info-in-gantt.md" class="MCXref xref">在甘特圖</a>中檢視資訊。</p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>共用報告、行事曆和檔案的對話方塊</strong> </p> <p>共用報表、行事曆和檔案時，使用的對話方塊是以Flash技術為基礎。</p> </td> 
   <td> <p>在Workfront中共用報告、行事曆和檔案的體驗未變更。 不過，體驗不再依賴Flash。</p> <p>如需共用這些專案的詳細資訊，請參閱<a href="../../../workfront-basics/grant-and-request-access-to-objects/sharing-permissions-on-objects-overview.md" class="MCXref xref">物件共用許可權概述</a>。</p> </td> 
  </tr> 
  <tr> 
   <td> <p><strong>舊版校訂檢視器</strong> </p> <p>舊版校訂檢視器是網頁型校訂檢視器，提供靜態、視訊和互動式校訂的校訂功能。</p> </td> 
   <td> <p><strong>網頁校訂檢視器和案頭校訂檢視器</strong> </p> <p>網路校訂檢視器提供靜態和視訊校訂的校訂功能。</p> <p>案頭校訂檢視器除了提供靜態和視訊校訂的完整支援外，還針對互動校訂提供校訂功能。</p> <p>SWF檔案格式已不受任何主要提供者支援，並已由HTML5校訂橫幅取代。 </p> <p>如需有關可用校訂檢視器之間差異的詳細資訊，請參閱<a href="../../../review-and-approve-work/proofing/proofing-overview/understand-differences-between-web-viewer.md" class="MCXref xref">網頁校訂檢視器與案頭校訂檢視器之間的差異</a>。</p> </td> 
  </tr> 
 </tbody> 
</table>
