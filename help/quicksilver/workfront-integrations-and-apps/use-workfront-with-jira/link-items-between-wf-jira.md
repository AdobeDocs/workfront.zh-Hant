...
* 安裝[!DNL Jira]的[!DNL Workfront]
  ...
  如需有關安裝Jira適用的Workfront的說明，請參閱[安裝Jira適用的Adobe Workfront](../../workfront-integrations-and-apps/use-workfront-with-jira/install-workfront-for-jira.md)。

* 為Jira設定[!DNL Workfront]

  如需為Jira設定Workfront的說明，請參閱[為Jira設定Adobe Workfront](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md)。

## 自動將[!DNL Workfront]個專案連結至[!DNL Jira]個問題

作為[!DNL Workfront]管理員，您可以定義觸發器，以便在每次任務或[!DNL Workfront]中的問題符合特定條件時，在[!DNL Jira]中自動建立問題。 Workfront和[!DNL Jira]專案會變成連結。
...
如需有關定義自動建立Jira問題的觸發器的詳細資訊，請參閱[為Jira](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md)設定 [!DNL Adobe Workfront] 。
...
1. 按一下「**儲存變更**」。
   ...
   在新任務的&#x200B;**[!UICONTROL 更新]**&#x200B;區域中，有註解表示已在[!DNL Jira]中建立新問題。

1. （選用）按一下Jira問題的連結以在Jira中開啟。

   或

   按一下&#x200B;**[!UICONTROL 詳細資料]**&#x200B;區段的&#x200B;**[!UICONTROL 整合]**&#x200B;區域中的&#x200B;**[!UICONTROL 移至Jira]**&#x200B;連結，或按一下任務或問題標題，以開啟[!DNL Jira]問題。
   ...
   如需有關在Workfront和Jira之間同步欄位的詳細資訊，請參閱[為Jira設定Workfront](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md)中的[為Jira設定Adobe Workfront](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md#configuring-the-add-on-for-jira)區段。

   >[!NOTE]
   >
   >從Workfront自動建立[!DNL Jira]問題時，[!DNL Jira]中的任何人未獲指派。

## 手動將[!DNL Jira]個問題連結至[!DNL Workfront]個專案

在[!DNL Jira]和[!DNL Workfront]中建立彼此獨立的專案後，您可以手動將[!DNL Jira]問題連結至現有的[!DNL Workfront]任務或問題。\
...
1. 按一下&#x200B;**[!UICONTROL 連結]**。
   ...
   這兩個專案會連結，[!DNL Workfront]右側面板會填入[!DNL Workfront]專案的資訊。

   下列[!DNL Workfront]欄位預設顯示在[!DNL Workfront]右側面板的[!DNL Jira]中：

   * 專案的&#x200B;**[!UICONTROL Name]**：您可以按一下面板中的名稱來存取[!DNL Workfront]專案。
   * **[!UICONTROL 專案名稱]**
   * 專案的&#x200B;**[!UICONTROL 狀態]**
   * 專案的&#x200B;**[!UICONTROL 優先順序]**
   * 建立日期為[!DNL Workfront]
   * 專案的&#x200B;**[!UICONTROL 計畫時數]**
   * **[!UICONTROL 參考號碼]**：您可以按一下面板中的[!UICONTROL 參考號碼]來存取[!DNL Workfront]專案。

如需有關啟用其他欄位顯示在右側面板的詳細資訊，請參閱[設定 [!DNL Adobe Workfront for Jira]](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md)中的[設定 [!DNL Jira] 與 [!DNL Workfront] 專案](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md#setting-up-field-synchronization)節之間的欄位同步。 來自與整合相關聯之[!DNL Workfront]管理員的註解已發佈在[!DNL Jira]問題的&#x200B;**[!DNL Workfront]**&#x200B;標籤中，以確認已建立新的[!DNL Jira]專案。 註解包含[!DNL Jira]問題的連結。

## 取消連結[!DNL Jira]和[!DNL Workfront]之間的專案

從[!DNL Jira]可以手動取消連結介於[!DNL Jira]和[!DNL Workfront]之間的連結專案。\
您無法從[!DNL Workfront]中相對應的[!DNL Jira]專案取消連結[!DNL Workfront]專案。
...
1. 在[!DNL Jira]中，導覽至連結至[!DNL Workfront]任務或問題的問題。
1. 移至[!DNL Workfront]右側面板，按一下&#x200B;**[!UICONTROL 取消連結]**&#x200B;圖示，然後按一下&#x200B;**[!UICONTROL 取消連結]**。

   先前連結的[!DNL Jira]和[!DNL Workfront]專案現在已取消連結。 任何未來可能會個別更新的欄位、註解或檔案，都不會在其他應用程式中更新其先前的對應專案。
