...
當您為[!DNL Jira]設定[!DNL Workfront]時，身為[!DNL Jira]系統管理員，您可以設定一個應用程式中的特定欄位，以便與另一個應用程式中連結專案的欄位同步。
...
如果您主要在[!DNL Workfront]中工作，則可以更新[!DNL Workfront]中的工作專案，而[!DNL Jira]中的對應專案也會更新。 此更新是透過[!DNL Jira]的[!DNL Workfront]整合而進行，不需要您擁有[!DNL Jira]授權。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>[!UICONTROL 問題名稱]</p> <p>有關名稱變更的註解已新增至[!DNL Jira]問題的<strong>[!DNL Workfront]</strong>索引標籤。 </p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 問題或任務說明]</td> 
   <td> <p> [!UICONTROL 問題說明]</p> <p>有關已更新說明的註解已新增至[!DNL Jira]問題的<strong>[!DNL Workfront]</strong>索引標籤。<br></p> </td> 
  </tr> 
  <tr> 
   ...
   <td> <p>[!UICONTROL 附件]</p> <p>已上傳附件的相關註解已新增至[!DNL Jira]問題的<strong>[!DNL Workfront]</strong>索引標籤。<br></p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 計畫完成日期]</td> 
   <td> <p>[!UICONTROL 到期日期]</p> <p>有關[!UICONTROL 到期日]已變更的註解已新增至[!DNL Jira]問題的[!DNL Workfront]索引標籤。 </p> <p>注意：您必須針對您的[!DNL Jira]問題啟用<strong>[!UICONTROL 到期日]</strong>，才能在[!UICONTROL Jira]中看到此欄位已更新。 </p> </td> 
  </tr> 
  <tr> 
   <td>自訂Forms和自訂欄位</td> 
   <td> <p> 顯示在[!DNL Jira]問題的[!DNL Workfront]右側面板中。 <br>面板中只會顯示具有實際值的自訂欄位。<br></p> <p>注意：自訂表單區段會以[!DNL Workfront]管理員的存取層級顯示。 </p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 問題或任務優先順序]</td> 
   <td>顯示在[!DNL Jira]問題的[!DNL Workfront]右側面板中。 <br>它不會更新[!DNL Jira]中的問題<strong>[!UICONTROL 優先順序]</strong>欄位。 </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 記錄時間] </td> 
   <td> <p>有關記錄時間的註解已新增到[!DNL Jira]問題的<strong>[!DNL Workfront]</strong>索引標籤中。 這包括記錄時間的使用者名稱，以及記錄時間的使用者名稱（如果不同）。 [!DNL Jira]中的<strong>[!UICONTROL 工作記錄檔]</strong>索引標籤未記錄任何時間。<br></p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 註解]</td> 
   <td> <p>註解已新增至[!DNL Jira]問題的<strong>[!DNL Workfront]</strong>索引標籤。 未新增至[!DNL Jira]問題的<strong>[!UICONTROL 註解]</strong>索引標籤</p> <p>注意：當您手動連結兩個現有專案時，在連結至[!DNL Jira]之前新增至[!DNL Workfront]專案的評論不會同步至[!DNL Jira]問題。 </p> <p>Jira註解會同步至Workfront。</td> 
  </tr> 
 </tbody> 
</table>
...
如果您主要在[!DNL Jira]中工作，則可以更新[!DNL Jira]中的工作專案，而[!DNL Workfront]中的對應專案也會更新。 連結至您[!DNL Jira]問題的[!DNL Workfront]專案不一定要有[!DNL Workfront]授權才能接收您在[!DNL Jira]中進行的更新。

在您的[!DNL Workfront]管理員已為[!DNL Jira]設定[!DNL Workfront]同步處理連結專案之間的欄位的情況下，您在[!DNL Jira]中更新的某些欄位也會更新連結的[!DNL Workfront]專案。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>時數<br>除了將登入Jira的時數新增至連結的[!DNL Workfront]專案外，有關登入時間的註解也會新增至[!DNL Workfront]專案的[!UICONTROL 更新]索引標籤。</p> <p>如需有關連結[!DNL Jira]問題的記錄時間的詳細資訊，包括更新在[!DNL Workfront]中記錄時間的[!DNL Jira]使用者，請參閱連結[!DNL Jira]和[!DNL Workfront]專案的<a href="#log-time-for-linked-jira-and-workfront-items" class="MCXref xref">記錄時間</a>。</p> </td> 
  </tr> 
  <tr> 
   <td> 註解 <br><br></td> 
   <td> <p>如果在[!UICONTROL Setup]索引標籤的[!UICONTROL SYNCHRONIZE FROM JIRA TO WORKFRONT]區段中的<strong>[!UICONTROL Comments]</strong>設定為<strong>[!UICONTROL Always]</strong>，則會將評論新增至[!DNL Workfront]問題或任務的[!UICONTROL Updates]索引標籤。</p> <p>如需有關在[!DNL Jira]中設定Workfront設定的資訊，請參閱<a href="../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md">設定[!DNL Workfront for Jira]</a>。</p> <p>如需有關對連結[!DNL Jira]問題的專案發表意見的資訊，請參閱連結[!DNL Jira]問題</a>的<a href="#comment-from-a-linked-jira-issue" class="MCXref xref">發表意見。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]
>
>如果將時間新增至連結至[!DNL Workfront]任務的[!DNL Jira]專案，則[!DNL Workfront]中時間的[!UICONTROL 小時型別]為[!UICONTROL 任務時間]。 如果將時間新增至連結至[!DNL Workfront]問題的[!DNL Jira]專案，則[!DNL Workfront]中該時間的[!UICONTROL 小時型別]為[!UICONTROL 問題時間]。

註解已新增至[!DNL Jira]中的&#x200B;**[!DNL Workfront]**&#x200B;標籤以及[!DNL Workfront]中專案的&#x200B;**[!UICONTROL 更新]**&#x200B;標籤，以記錄記錄記錄時間。\
時間也會顯示在[!DNL Workfront]專案的&#x200B;**[!UICONTROL 小時]**&#x200B;索引標籤中。

* 已連結 [!DNL Jira] 和 [!DNL Workfront] 專案的[記錄時間](#log-time-for-linked-jira-and-workfront-items)
* [將時間從 [!DNL Jira] 記錄到 [!DNL Workfront] 專案](#log-time-from-jira-to-a-workfront-item)

### 連結的[!DNL Jira]和[!DNL Workfront]專案的記錄時間

您可以從連結至[!DNL Workfront]專案的[!DNL Jira]問題記錄時間，該時間會記錄在[!DNL Jira]問題及[!DNL Workfront]專案上。

>[!IMPORTANT]
>
>如果在[!DNL Jira]中記錄時間的使用者不存在於[!DNL Workfront]中，則整合會在Workfront中建立新的作用中使用者(如果&#x200B;**[!UICONTROL 在[!DNL Workfront]中自動建立使用者(&#x200B;如果[!DNL Jira]使用者沒有&#x200B;*[!DNL Workfront]帳&#x200B;戶]**)，設定為&#x200B;**[!UICONTROL &#x200B;一律&#x200B;]**。 此使用者未佔用[!DNL Workfront]授權。 您可以將作用中的使用者指派給[!DNL Workfront]中的工作專案，但您不能將其包含在更新中。 如需有關從[!DNL Jira]設定[!DNL Workfront]使用者自動建立的資訊，請參閱[設定 [!DNL Workfront for Jira]](../../workfront-integrations-and-apps/use-workfront-with-jira/configure-workfront-for-jira.md)。

若要為[!DNL Jira]中的專案記錄時間，並將它同時記錄在[!DNL Jira]和[!DNL Workfront]中：

1. 登入[!DNL Jira]。
1. 移至連結至[!DNL Workfront]專案的[!DNL Jira]問題。
...
1. 繼續新增資訊至您的時間專案，包括&#x200B;**[!UICONTROL 工作描述]**，然後按一下&#x200B;**[!UICONTROL 記錄]**。\
   時間已新增至[!DNL Jira]專案的&#x200B;**[!UICONTROL 工作記錄]**&#x200B;索引標籤，以及連結至它的[!DNL Workfront]專案。\
   時間專案的工作說明會在[!DNL Workfront]中記錄為時數專案的備註。

### 將時間從[!DNL Jira]記錄到[!DNL Workfront]專案

您可以僅將時間記錄到從[!DNL Jira]問題連結的[!DNL Workfront]專案，而無需將時間記錄到[!DNL Jira]問題。

1. 登入[!DNL Jira]。
1. 導覽至連結至[!DNL Workfront]專案的[!DNL Jira]問題。
...
1. 按一下&#x200B;**[!UICONTROL 記錄時間]**。
   ...
   此時間未新增至[!DNL Jira]問題的[!UICONTROL 工作記錄]索引標籤。
...
1. 按一下「[!DNL Workfront]」面板或「**[!UICONTROL 註解]**」標籤中的「**[!UICONTROL 註解]**」圖示。

1. 開始輸入註解，然後按一下&#x200B;**[!UICONTROL 傳送]**。

   註解會新增至下列專案：

   * [!DNL Jira]問題的&#x200B;**[!DNL Workfront]**&#x200B;標籤。
   * [!DNL Jira]問題的&#x200B;**[!UICONTROL 註解]**&#x200B;標籤。
