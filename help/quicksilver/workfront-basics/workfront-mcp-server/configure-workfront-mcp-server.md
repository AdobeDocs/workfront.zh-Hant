...
1. 按一下&#x200B;**連線**。
1. 出現提示時，請登入您的Workfront執行個體。 您可能需要選取設定檔和Workfront執行個體。 您選取的設定檔會決定要連線的工作區。

   驗證完成後，就會連線Workfront MCP伺服器。
...
+++ 展開以檢視將Workfront連線至ChatGPT Desktop或ChatGPT Codex的逐步指示。
...
+++ 展開以檢視將Workfront連線至網頁上ChatGPT的逐步指示。
...
1. 按一下「**建立**」。
1. 建立應用程式後，Workfront登入視窗會隨即顯示。 使用您的Adobe ID憑證進行驗證。 您可能需要選取設定檔和Workfront執行個體。 您選取的設定檔會決定要連線的工作區。
...
## 將Workfront連線至Copilot

+++ 展開以檢視將Workfront連線至Copilot的逐步指示。
...
1. 在上方導覽列中，按一下&#x200B;**工具**，然後按一下&#x200B;**新增工具**。
1. 選取&#x200B;**模型內容通訊協定**&#x200B;拼貼。
1. 在出現的面板中，輸入此連線的名稱和說明。
1. 在伺服器URL欄位中，輸入URL：
...
1. 針對驗證，請選取&#x200B;**OAuth 2.0**，然後選取&#x200B;**動態探索**。
1. 按一下面板底部的「**建立**」。
