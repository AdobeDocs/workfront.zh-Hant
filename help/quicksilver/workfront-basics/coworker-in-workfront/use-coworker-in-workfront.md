...
description: 瞭解如何在Workfront中使用CX Co-worker。
author: Becky
...
您可以在Workfront中存取CX Co-worker。

在Workfront中使用同事時，它可以處理符合以下條件的資訊和物件：

* 位於Workfront或Workfront Planning。
* 您擁有下列專案的許可權。
...
* 您必須先擁有Workfront物件的現有存取權和許可權，才能在CX Co-worker中與其互動。 例如，您至少必須擁有專案的檢視存取權，才能在同事中檢視專案的相關資訊。
* 您的Workfront管理員必須先在您組織的系統偏好設定中啟用「寫入MCP工具」選項，您才能透過CX Co-worker在Workfront中進行任何變更。 CX Co-worker預設為唯讀功能。

  如需資訊與指示，請參閱[設定系統偏好設定](/help/quicksilver/administration-and-setup/manage-workfront/security/configure-security-preferences.md)。


## 在Workfront中使用CX Co-worker

1. 在任何Workfront頁面頂端，按一下CX Co-worker圖示![AI圖示](assets/ai-icon.png)。
1. 在熒幕右側的面板中輸入您的問題或提示。

1. 如果Co-worker沒有提供您需要的答案，請調整您的提示，然後再試一次。
...
1. 使用聊天輸入方塊中的任何動作：
   ...
   * **計畫模式**：要求同事聊天室提出逐步計畫，並在它生效之前暫停您的核准。 將其關閉，讓「同事聊天」直接執行。
   * **成績單檢視**：控制顯示多少同事聊天室內部活動：正常、焦點或詳細資訊。
