...
<p>[!UICONTROL 實際成本]計算已考慮[!UICONTROL 舊版實際時數]。 如需詳細資訊，請參閱本表中的「[!UICONTROL 實際時數]」或「[!UICONTROL 舊版實際時數]」。 </p>   </td> 
  </tr> 
  <tr> 
...
<tr> 
   <td>[!UICONTROL 成本績效指數(CPI)]</td> 
   <td> <p>[!UICONTROL 成本績效指數(CPI)]說明專案或任務層級的計畫成本與實際成本之間的關係。 專案經理會檢閱此量度，以識別目前於指定時間點追蹤成本過低或過高的任務或專案。 視您的[!UICONTROL 績效指數方法(PIM)]而定，成本可用小時或貨幣計量。</p> 
...
<tr> 
   <td>[!UICONTROL 成本排程績效指數(CSI)]</td> 
   <td> <p>[!UICONTROL 成本排程績效指數(CSI)]是一種自動計算，它將[!UICONTROL 成本績效指數(CPI)]和[!UICONTROL 排程績效指數(SPI)]結合為一個一般度量，以平衡成本與排程。 將這些值相乘，單一量度就能以較低預算處理較長的排程，反之亦然。 當犧牲成本以推動專案中間的排程時，專案經理可以使用此項來判斷一般專案或任務狀況。</p> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <p>將此欄位新增到[!UICONTROL 任務]報告時，任務子物件上剩餘的任何更新（如問題、子任務或檔案）都不會顯示在此欄中。</p> 
   <p><b>附註</p>
...
<tr> 
   <td>[!UICONTROL 績效指數方法(PIM)]</td> 
   <td> <p>專案的[!UICONTROL 績效指數方法(PIM)]控制Adobe Workfront用於計算專案績效指標的方法，例如成本績效指數(CPI)、成本排程績效指數(CSI)、排程績效指數(SPI)和完工估算(EAC)。</p> 
  ...
  <tr> 
   ...
   <td> <p>在專案、任務和目標報告中，此欄位會顯示專案、任務或策略目標的進度狀態。 如需詳細資訊，請參閱下列文章：</p> 
    <ul> 
     ...
     <p>只有貴組織已購買[!DNL goals]，才會顯示[!DNL Workfront Goals]的[!UICONTROL 目標]報表和[!UICONTROL 進度狀態]。 如需[!DNL Workfront Goals]中策略目標的相關資訊，請參閱<a href="../../../workfront-goals/goal-management/wf-goals-overview.md">[!DNL Workfront Goals]總覽</a>。 </p> </li>
    </ul> </td> 
  </tr> 
  <td>[!UICONTROL 專案]</td> 
...
<tr> 
   <td>[!UICONTROL 排程績效指數(SPI)]</td> 
   <td><p>[!UICONTROL 排程績效指數(SPI)]說明計畫排程與實際排程之間的關係。 Adobe Workfront會計算專案與任務層級的SPI。 專案經理會檢閱此量度，以識別任務或專案目前追蹤的時間是否比排程提前或落後。</p>
