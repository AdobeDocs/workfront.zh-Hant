...
動作列會顯示您可以對清單專案執行的動作。 有些動作可能是該清單專屬的動作，不會出現在任何其他清單中。
...
1. 選取增強型清單中專案旁的核取方塊。
   ...
   >[!NOTE]
   >
   >不同的增強型清單可能允許選取單一專案、大量編輯（選取多個專案）或無專案選取。

1. 按一下動作列上的按鈕即可執行該動作，例如檢視清單專案、刪除專案或編輯專案。

   如果選取的專案沒有可用的動作，動作列會顯示「沒有可用的動作」。

   ![動作列範例](assets/glist-action-bar-statuses.png)

1. 暫留在清單專案上的主要欄位上以檢視&#x200B;**更多**&#x200B;功能表。 （主要欄位是表格最左側的欄。）

1. 按一下功能表以檢視專案的其他動作。 有些動作可能是該清單專屬的動作，不會出現在任何其他清單中。
...
某些增強型清單提供&#x200B;**資料行管理員**，可讓您輕鬆新增資料行至清單及移除資料行。 原生和自訂欄位都可選取為欄。 自訂欄位必須先存在於系統中，您才能將其新增為清單欄。
