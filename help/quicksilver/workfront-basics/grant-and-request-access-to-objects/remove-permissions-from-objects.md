...
1. （視條件而定）針對計畫、投資組合和檔案，執行下列動作：
   ...
   1. 按一下物件名稱旁的&#x200B;**更多**&#x200B;圖示![](assets/more-icon.png)，然後按一下&#x200B;**共用**&#x200B;或&#x200B;**共用**。
   ...
   1. 在&#x200B;**&lt;使用者名稱>的Workfront存取權將從此**&#x200B;下拉式選單中移除，選擇您是要從您選取的物件中移除其存取權，還是從與其關聯的所有子物件中移除其存取權。
...
1. 選取清單中的數個物件，然後按一下清單頂端的&#x200B;**共用**&#x200B;圖示![](assets/share-icon.png)。
1. 在&#x200B;**編輯`<Object Name>`對**&#x200B;欄位的存取權中，輸入您要移除其存取權的使用者、角色、團隊、群組或公司的名稱。
1. 從存取下拉式功能表，選取&#x200B;**無存取權**。

   ![](assets/no-access-option-removing-permissions-bulk-tasks-nwe-350x166.png)

1. 在`<User Name>`的Workfront存取權將從此下拉式功能表中移除，選擇您是否要僅從您選取的物件移除其存取權，還是從與其關聯的所有其他子物件移除其存取權。\
   存在下列情況：

   * 如果您只從物件中移除實體，則該實體會失去其對物件的許可權，以及其對子物件的繼承許可權。 如果先前已個別授予子專案的許可權，當您選取此選項時，它們會保留與其相關聯之所有子物件的相同許可權。 
   * 如果您將實體從物件及所有子物件中移除，該實體會失去其對該物件及所有子物件的許可權，即使先前已授予他們個別對各個子物件的許可權。

   **範例：**&#x200B;選取是否僅移除您在清單中選取之任務的許可權，或移除附加至任務的問題和檔案的許可權。

   ![](assets/remove-permissions-bulk-drop-down-for-attached-objects-nwe-350x96.png)

1. （可選）若要大量變更數個物件的許可權，請為選取的實體選取另一個共用層級。

   例如，如果他們擁有管理許可權，請改為選取Contribute或檢視。
...
1. 選取共用方塊上&#x200B;**繼承的許可權**&#x200B;旁的&#x200B;**x**，以移除此處列出的任何使用者。

   ![](assets/remove-inherited-permissions-on-project-nwe-350x475.png)

   這可確保授予父系物件（例如專案）許可權的任何人都不會預設擁有此任務的許可權。 您必須列出  任務共用清單中的個別實體，以授予任務的許可權。
...
1. 按一下&#x200B;**移除公開存取權**&#x200B;以移除外部使用者檢視報告的存取權。
1. 按一下&#x200B;**移除系統範圍存取權**&#x200B;以停止與所有Workfront使用者共用。 
1. 按一下「**儲存**」。
