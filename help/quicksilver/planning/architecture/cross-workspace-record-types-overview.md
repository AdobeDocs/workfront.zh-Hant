...
* 任何使用者都無法在次要工作區中對全域記錄型別執行下列動作：
  ...
  * 建立及管理申請表單
    ...
    <span class="preview">您可以使用次要記錄型別頁面中的要求表單，或透過在填寫針對主要全域記錄型別建立的要求表單並更新Workspace欄位時，明確指定要將記錄儲存到哪個工作區，來建立次要記錄型別的記錄。
    如需詳細資訊，請參閱[提交Adobe Workfront Planning要求以建立記錄](/help/quicksilver/planning/requests/submit-requests.md)。</span>
...
1. 將記錄型別設定為在特定工作區中可連線。
   ...
   >[!TIP]
   >
   >只有系統管理員才能指定記錄型別，使其可從系統中的所有工作區連線。 所有其他工作區管理員可以指定特定的工作區，以從中連線到記錄型別。
   >
   >如需詳細資訊，請參閱[設定記錄型別的跨工作區功能](/help/quicksilver/planning/architecture/configure-record-type-cross-workspace-capabilities.md)。
