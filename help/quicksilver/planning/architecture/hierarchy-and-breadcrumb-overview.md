...
* 您只能在工作區階層中連線下列物件型別：
   ...
   * Workfront專案。
* 您無法在階層中新增下列物件型別：
   * 來自其他工作區的記錄型別，即使這些記錄型別設為可連線或全域記錄型別亦然。
   * 所有其他Workfront物件。
   * AEM Assets
* 階層可同時包含Planning記錄型別和Workfront物件型別。

      例如，您可以將具有規劃策略和Workfront專案的行銷活動記錄型別當做相同工作區階層中的子項。
  * 如果選取的記錄型別之間已經存在連線，系統會重複使用現有的連線。
* 如果沒有任何連線，Workfront會在階層設定中建立一個連線。
* 必須為連線的欄位開啟&#x200B;**在連結的記錄型別**&#x200B;上建立對應欄位設定。

  有連線的記錄型別若未在其連結的記錄型別上建立對應欄位，這些記錄型別也可以是階層的一部分，但當您在階層設定期間建立新連線時，將一律必須在連結的記錄型別上建立對應欄位。
* 以下是階層設定的規則：
...
例如，如果您建立階層並連結「行銷活動」與「戰術」、「方案」，然後連結「專案」，當您導覽至階層中連線之任何型別的記錄時，您可以檢視記錄在該階層中的放置位置。
