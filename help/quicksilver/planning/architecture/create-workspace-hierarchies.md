...
在工作區中連線記錄型別後，您可以建立組織這些連線的階層。

階層將記錄和物件型別組織成父子關係，每個最多可包含4個物件型別。 您最多可以在一個工作區中建立5個階層。
...
1. （選擇性）按一下階層中最後一個記錄型別的名稱，然後按一下該記錄型別檢視中的記錄名稱。 這會開啟記錄的詳細資訊頁面。 在記錄頁面頂端的記錄階層連結中，尋找您建立的階層。
