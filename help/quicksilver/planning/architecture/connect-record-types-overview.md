...
* 在「一對一」或「一對多」連線型別中複製具有連線記錄的記錄時，會出現以下情況：

  * 在生產環境中，連線的記錄會移至複製的記錄，或者您必須先從原始記錄中手動移除該記錄，然後再複製記錄，然後將其重新新增。
  * <span class="preview">在[預覽]環境中，您可以選取連線的記錄是保留在原始記錄上，還是移至重複的記錄。</span>
