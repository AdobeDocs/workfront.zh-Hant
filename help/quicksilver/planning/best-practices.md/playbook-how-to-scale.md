---
title: 將您的首次成功轉化為永續的動力：管理式擴展的行動手冊
...
但這也是最脆弱的採用階段：過快地轉向僵化的治理，可能會讓團隊回到試算表上；過鬆地移動，會重新製造出您想要解決的無序蔓延。

本教戰手冊概述平衡的方法，說明如何使用Workfront計畫作為調解引擎，將能見度放在首位，促進本地創新，並讓企業標準從真正的營運成功中誕生，而不是過早強加。

以下是從我們的經驗中學習，並提供了一些關於觀看內容的提示。

## 1.成功陷阱

有時候，在第一次成功的使用案例或概念證明後，就覺得採用Workfront Planning最危險的階段了。 熱情很高，但對技術性債務和管理過度擴張的恐懼可能導致兩種同樣有害的反應：

* **過度控管**：將系統鎖定得太緊，以致新團隊會回覆成試算表。

* **零控管**：讓每個團隊建立自己的欄位和記錄型別，重新建立舊版環境中找到的分散式中繼資料無序蔓延。

## 2.核心理念：Workfront計畫作為調節引擎

我們將Workfront Planning定位為顯示差異的位置，而非嘗試阻止團隊有所差異，這樣就能調解差異。

若要支援此工作，請考量下列事項：

* **管理採用速度**：在最佳化現有環境之前，很自然要小心地擴展至新的工具。 雖然選擇先簡化提供了高度控管的基礎，但對於準備結盟的團隊來說，這可能會延遲實現價值的時間。 我們相信，領導這項變更的最有效方法是認識到可見度是步驟1；走向共用、企業就緒型工具（遠離簡報與試算表的大量棧疊）的動力是最終消除長期目標障礙的因素。

  我們建議您不要先強制清理，而是將一小部分資源用於持續性維護，而大幅增加部分用於解決緊迫的業務需求。

  例如，將一年的時間完全花在「清理」分類上，所產生的增量價值微乎其微。 但是，提供跨團隊可見度可提供利害關係人所需的轉型價值，同時提供您隨時間控管環境所需的統一資料結構。

  您可以透過統一的企業行事曆或整合的上市藍圖，實現跨團隊的可見度。

* **瞭解真實情況**：獨立團隊自然會開發自己的程式，這些程式通常隱藏在孤立的電子表格中。 透過讓這些程式顯示在共用環境中，您可以將它們放在最終可解決和改進的地方。

  >[!IMPORTANT]
  >
  >轉換至Workfront Planning不會造成混亂；而是照亮已經存在的專案。


* **認可進度符號**：當團隊要求他們自己的欄位時，它不是「蔓延」。 將其視為採用率的標誌。 這表示團隊將Workfront規劃視為其工作區。

* **管理債務，不要隱藏債務**：很自然要關注日後清理不同分類所需的工作。 然而，替代方案 — 過早強制實施嚴格的標準 — 通常迫使團隊回到其流程（和債務）仍然隱藏的試算表上。 透過允許團隊以他們目前的分類開始在Workfront Planning中，您正在將該債務移至可視、受管理的環境中。 這使得最終調解成為反複的工作，而不是單一且壓倒性的移轉專案。

## 3.引導式自主治理模式

您可以定義道路上的車道和當地遊樂場（或控管的預設值和範本），同時讓團隊可以靈活地選擇自己的路徑。

請考量下列引導式自主治理模式的元件：

* [全域通道](#the-global-lanes)
* [當地遊樂場](#the-local-playgrounds-or-the-spokes)
...
* **實驗物件**：專屬於團隊戰術需求的欄位或記錄型別（例如，社交團隊的`Influencer Handle`或網路團隊的`URL Slug`）。

* **管理者**：團隊負責人（提供簡易指引）。

* **規則**：團隊可以在這裡創新。 如果三個以上的團隊採用本機欄位，則可將其升級為全域車道。

## 4.考慮治理悖論：團隊優先，標準將遵循

擴展規模Workfront Planning的常見挑戰是決定哪一項優先：企業治理或團隊的營運調整。

我們相信，最有效的方法就是要認識到，企業價值是建立在雙向基礎上的：

* **團隊需要關聯性**：企業僅在團隊主動執行時實現值。 因此，治理必須提供結構來支援團隊已知的運作需求，從而為團隊服務。

* **企業需要可見度**：領導階層只能在資料足夠乾淨，能夠彙總時做出明智的決策。 因此，團隊必須提供投資組合可見度所需的最低可行中繼資料，從而為企業提供服務。

Managed Scaling的目標是透過標準化程度足以提供可見度，但不會造成團隊執行延遲，來找出這兩種需求的交集。

在建立治理模型時，請考量下列階段：

* [對齊優先順序：資料與視覺效果](#align-priorities-data-vs-visualization)
* [服務導向的觀察階段](#the-service-led-observation-phase)
* [追溯協調](#retroactive-harmonization)

### 對齊優先順序：資料與視覺效果

縮放時，請注意值定義在不同角色間所代表的差異：

* **管理員或產品擁有者**&#x200B;會設定統一分類與分類的值。 他們的目標是建立支援長期擴充能力的乾淨資料架構。

* **利害關係人或領導者**&#x200B;會重視視覺化效果和insight，這些可擷取在全球行事曆或投資組合時間表。 他們的目標是找出讓資料可操作的閃電時刻。

>[!NOTE]
>
>策略是使用利害關係人的視覺化需求作為團隊遵循管理員資料標準需求的獎勵。 透過提供領導力所需的行事曆，您可獲得統一的分類法。


### 服務導向的觀察階段

在早期擴充中，管理員的角色是促進團隊與企業之間的這種交換。

作為管理員，您可以執行下列動作來促進此交換：

* **將作業優先順序置於標準化之上**：讓團隊在孤立的工作區中主動規劃，總比讓團隊因缺乏全域定義而停滯不前要好。 此活動是建立健康且真實世界的標準所需的原始資料。

* **識別可見度最小值**：與領導合作以識別企業報表中必須清除的3-5個欄位（例如，`Strategic Alignment`、`Start Date`、`Budget`）。 僅將您的執行精力集中在可見度最低欄位上。

### 治理即服務

一旦跨團隊出現已知需求的模式，企業就可以將這些模式整合為全域服務。 這是治理即服務。

執行以下動作來達成服務或控管：

* **觀察成功的模式**：識別團隊已建置並採納的成功分類。

* **達成合作交握**：將團隊冠軍聚集在一起，將他們的本地成功提升為共用的企業標準。

* **部署為服務**：推出新的全域通道不是作為限制，而是為了簡化工作人員的報告和跨團隊協調。

>[!IMPORTANT]
>
>請記住，治理應該是應對運作成功的回應，而不是其先決條件。

## 5.縮放管理欄位的機制

### 圖樣式欄位成長模型

套用此理念需要對資料結構採取深思熟慮的方法。 為避免治理蔓延，請抵製為每個個別請求建立全球欄位的衝動。

請改用以下欄位成熟度路徑，讓實際使用為您的企業標準提供指引：

* **層級1：本機實驗**：團隊A在其工作區中建立自訂欄位。
...
當欄位不再相關時，您就會看到您必須將其淘汰。

由於Workfront Planning目前沒有欄位的原生封存功能，因此若要淘汰本機欄位，必須經過審慎的軟淘汰程式，才能保留歷史資料，而不會影響UI。

若要淘汰欄位，請執行下列動作：

1. 進行資料移轉。 使用表格檢視（或Fusion）將本機陰影欄位的值大量複製到新的全域車道欄位。 請確定資料在此移動期間已驗證並清除。

2. 重新命名e欄位以供取代：重新命名首碼為`[DEPRECATED]`或`z_`的本機欄位（例如`z_Language (Old)`）。 這會將欄位推到欄位選擇器的底部，並向使用者表示它不應該再是真實來源。

3. 從所有記錄表單中移除已棄用的欄位。 這是最關鍵的步驟。 這可防止輸入新資料，同時視需要在現有表格檢視或報告中顯示舊資料。

4. 將已廢止的欄位（加上前置詞並從表單中移除）保留30至60天，以啟動日落時段，確保移轉期間不會遺漏任何資料。 在此期間之後，如果在「全域通道」中完全調解資料，則可以從工作區中刪除本機欄位。

## 6.避免Workfront漂移

若要避免Planning變得雜亂，請執行下列動作：

* **瞭解抽象層**： Planning中的每個欄位都應該回答策略性問題。 如果欄位僅用於戰術追蹤（例如，如果欄位回答諸如「此證明是否核准？」的問題），請將其保留在Workfront中。

* **先完成合併**：如果團隊想要新的中繼資料欄位，請他們先檢查全域分類法。 這需要授與團隊潛在客戶對全域分類工作區的唯讀存取權（請參閱第7節）。 透過將其戰術需求對應到現有的策略欄位，您可以防止不必要的重複並維護報告的完整性。

## 7.唯讀存取可見度模型

您可以讓團隊以唯讀方式存取全域分類工作區，檢視哪些概念可能套用至他們自己的工作流程，藉此解決定址感覺，而不會產生定址工作的雜訊。

考慮以下事項：

* **問題**：團隊（在輪輻中）感到孤立，因為他們只能看到自己的記錄。

* **修正**：授與團隊對這些共用記錄型別指定為主要的工作區的唯讀存取權。

  如需詳細資訊，請參閱[共用工作區](/help/quicksilver/planning/access/share-workspaces.md)。

* **結果**：團隊可以檢視更廣闊的企業背景來尋找靈感與一致性，但他們的當地工作區保持乾淨，並專注於他們的特定工作。

## 8.透過研討會管理成長

擴展Workfront Planning既是技術上的挑戰，也是文化上的挑戰。 使用目標研討會來縮短治理差距。

以下是您可以參加的研討會構想：

* [「必要的雜亂」探索研討會](#the-necessary-mess-discovery-workshop)
* [「策略性可見度」校準會議](#the-strategic-visibility-alignment-session)
* [「與全球對話」展示](#the-spoke-to-global-showcase)
* [「持續支援」營業時間](#the-ongoing-support-office-hours)

### 「必要的雜亂」探索研討會

* **對象**：地區行銷銷售線索與營運達人。

* **目標**：記錄目前定址的實際情況，或實際分散的作業資料。
...
* **對象**：高層級行銷關係人（例如擔任領導角色的人員）。

* **目標**：重新設定簡化優先焦慮。

* **訊息**：「我們不需要完美的分類法才能開始。 我們正使用Workfront Planning作為環境，以建置完美的分類法。」

* **結果**：核准在Workfront保持目前狀態時，繼續以Workfront計畫作為調解引擎。

### 「與全球對話」展示

* **對象**：探索Workfront Planning的新團隊。

* **目標**：減少孤立的感覺。

* **訊息**： 「瞭解小組A的本機工作如何自動饋送指定的主要Workspace？ 您也能擁有同樣的工作可見度。」

* **結果**：從新部門選擇加入，因為新部門看到連線帶來的好處，而不失去其本機獨立性。

### 「持續支援」營業時間

* **對象**：所有Workfront Planning使用者（目前和未來）。
...
在Managed縮放模型中成功並不只是需要工具設定。 它需要在全域和分支團隊之間明確分配角色。

在以下子區段中，您可以找到管理縮放模型的主要玩家想法。

### 企業架構師（卓越中心或行銷營運中心）

* **焦點**：企業完整性、系統效能和統一的資料分類法。
...
* **焦點**：策略對應、OKR可見度和投資組合視覺化（例如全域行事曆）。
...
從成功實作中學到的所有知識應該都會產生檢查清單，供您進一步實作。

以下為檢查清單應包含的範例：

* [ ] **識別衛士**：誰是這個新團隊的處理序擁有者或衛士？

* [ ] **定義本機差異**：此團隊目前不需要全域標準提供的哪些2-3欄位？

* [ ] **對應至全域通道**：哪些現有的全域欄位可以滿足80%的未來團隊需求？
