...
本指南適用於正在實施Workfront Planning的Workfront管理員。

## 如何快速啟動並擴充智慧型應用程式概述

成功推出的最常見障礙並非技術。 這是「技術責任」。 當團隊遇到挑戰，需要同時與新人員、新流程和新工具合作時，該工具就會成為挫折的目標。

30天啟動平台採用「團隊優先」的採用模式。 我們專注於分散式、團隊主導的方式，讓特定職能單位立即實現ROI，而不是嘗試通常需要數月協調才能在企業範圍內推出。

## 使用引導式自主性來實現分階段轉換

若要成功推出產品，使用引導式自主是明智的做法。 這表示作為管理員，您可以定義專案團隊在控管預設值和範本的結構化系統中應如何運作。

您可定義道路上的路線（或管轄的預設值和範本），同時允許專案團隊靈活地選擇其內部的工作路徑。

最重要的是，引導式自主應該是進化式的。 您不必在第一天強制實施完整的企業分類法，而是可以根據第一個實驗團隊的需求，以最低限度的全域定義開始。

隨著更多團隊上線，您可以觀察共同模式並重複使用全球標準。

身為管理員，您不會放棄控制權，而是會根據實際運作的情況聰明地調整控制權的比例。

若要啟動此進化轉換，請執行下列動作：

1. 選擇準備且願意創新的試驗團隊。
2. 使用您啟動的前15天，透過兩個高動量路徑之一來證明模型：

   * **平行路徑（或雙追蹤方法）：**&#x200B;團隊維持目前的工作流程，同時在Workfront Planning中建立下一個規劃週期的模型。

     這可以建立肌肉記憶力和自信，不會發生單點故障。

   * **乾淨的插播：**&#x200B;使用範例資料進行初始設計練習以驗證模型後，團隊會將其即時起始點直接移入Workfront Planning進行下一個週期。

   在這兩個路徑中，目標是儘快超越理論並進入實際應用程式，確保該工具用於實際策略工作，而不是僅僅停留在沙箱練習中。
...
雖然現在不需要複雜的全企業分類法，但您應立即建立兩個環境。 例如，當您採用軸輻式架構方法時，您可以建立下列專案：

* **全域分類工作區** （中樞）：這應該是您系統的一般結構和命名法。
* 前&#x200B;**個團隊工作區** （分支）：這應該符合個別團隊的工作方式。
...
* **連線策略以運作：**&#x200B;啟用規劃記錄與Workfront物件之間的連結。

  如需詳細資訊，請參閱[連線記錄型別概觀](/help/quicksilver/planning/architecture/connect-record-types-overview.md)。

* **映象目前的資料：**&#x200B;使用團隊的現有工具（例如Excel或其他核心應用程式）來填入Workfront Planning，確保資料看起來熟悉且相關。

* **專為彈性打造：**&#x200B;請記住，Workfront Planning是專為改進而設計。 您可以從團隊層級的流程開始，並在業務準備就緒時輕鬆提升或遷移至企業層級的控管。

### 第15天：第一個突破時刻

我們建議您在15日之前將目標定在第一次重大勝利。 這應該是自動建立專案。

若要完成此操作，您必須實作下列專案：

1. 設定可識別「準備執行」狀態的原生自動化。

2. 在Workfront中觸發連結專案的建立。

如需詳細資訊，請參閱[設定Adobe Workfront規劃自動化](/help/quicksilver/planning/records/configure-automations-to-create-records.md)。

這可確保您的團隊看到其策略意圖，並自動觸發執行工作。 這個突破時刻證明了策略與工作之間的橋樑價值，並建立起認同感。

### 16-30天：逐步過渡到企業規模

完成閃電時刻後，您就可以將團隊的起點移至Workfront Planning中。

隨著此團隊優先模型成功運作，您會透過識別有效的模式來開始企業擴展之路。

若要從團隊優先邁向企業規模，請考量下列事項：

* **引入引入表單：**&#x200B;使用請求表單作為策略請求的新閘道，以確保資料完整性。

  如需詳細資訊，請參閱[在Adobe Workfront Planning中建立和管理要求表單](/help/quicksilver/planning/requests/create-request-form.md)。

* **觀察並標準化：**&#x200B;使用全域分類工作區來觀察實驗團隊如何使用工具。 這些真實世界的範例將塑造您最終的企業設計。

* **淘汰舊版試算表：**&#x200B;隨著程式成熟完全移入Workfront Planning。

* **Shift超級使用者：**&#x200B;使用記錄表格檢視進行直接、高速的計畫管理。
...
* **採用：**&#x200B;前5至10筆策略記錄已成功連結至執行專案，而且建立記錄及自動建立連結專案的程式是順暢且可輕鬆重複的。
...
實作成功與否，取決於您使用的方法，以及您從一開始就設定的期望。

以下是如何領導成功實作的一些建議。

### 待辦事項：

* **從興奮的試驗小組開始：**&#x200B;確認全球推出是一項顯著的提升。 透過團隊專注於概念證明，團隊渴望創新並能為其他人提供藍圖。

* **定義通道，而非牆壁：**&#x200B;使用引導式自主功能提供結構，而不會造成探索失敗。

* **排定心理安全的優先順序：**&#x200B;讓團隊知道舊版工具會保留在原處，直到他們熟悉新的工作流程為止。

* **慶祝快速成功：**&#x200B;將第一個自動化專案的建立工作向整個團隊展示。

* **建立冠軍：**&#x200B;識別每個團隊中的超級使用者以領導轉換。

### 不要：

* **不強制進行大規模變更：**&#x200B;強制所有人在第一天進行切換會導致技術上的過失。

* **不要忽略原因：**&#x200B;請確定每個團隊成員都瞭解策略執行緒如何讓他們的工作更顯眼。

* **不要等待完美：**&#x200B;您的分類將會演化。 從最佳猜測開始，並視需要調整。
