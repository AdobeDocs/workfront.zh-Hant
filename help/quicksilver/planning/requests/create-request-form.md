...
1. 按一下任何欄位，然後使用表單右側面板中的控制項來定義其大小或下列任何資訊：
   ...
   * **新增邏輯**：定義必須符合哪些條件才能顯示或隱藏欄位。
   ...
   在表單上選取欄位後，每個欄位的欄位型別都會顯示在右側面板的頂端。 請求表單上欄位型別的名稱與它們在表格檢視中的名稱不同。 下表說明表格檢視中的欄位型別名稱，與記錄型別請求表單中相同欄位名稱之間的差異：

   | Workfront Planning欄位型別 | 請求表單欄位型別 |
   |-------------------------------|-------------------------|
   | 單行文字 | 單行文字 |
   | 段落 | 含格式的文字 |
   | 貨幣、數字、百分比 | 單行文字 |
   ...
   >[!NOTE]
   >
   >   欄位格式會保留。 例如，雖然「貨幣」和「百分比」會顯示為「單行」文字欄位，但欄位中的資訊會顯示為「貨幣」和「百分比」值。
