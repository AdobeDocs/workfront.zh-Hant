...
1. （選擇性和條件式） <span class="preview">在記錄頁面標頭的階層連結中，按一下記錄型別的名稱，以存取相同階層中的其他記錄型別。 階層必須存在於您所連線的記錄型別中，才能在階層連結中檢視這些記錄。 如需詳細資訊，請參閱[建立工作區階層](/help/quicksilver/planning/architecture/create-workspace-hierarchies.md)。</span>
