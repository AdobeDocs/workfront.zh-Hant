...
<span class="preview">根據您從哪個環境存取資料表檢視，管理該資料表檢視會有所不同。</span>
...
1. 建立資料表檢視，如文章[管理記錄檢視](/help/quicksilver/planning/views/manage-record-views.md)中所述。
   ...
   ![資料表檢視範例](assets/table-view-example-g-list.png)
...
1. 執行下列任一項作業，重新排序表格中的欄：
   ...
   * 按一下表格工具列中的&#x200B;**欄位**，然後依所需順序拖放欄位，再按一下&#x200B;**欄位可見度和順序**&#x200B;方塊外部以關閉它。
     ...
     >* 依預設，「名稱」欄位永遠是表格檢視中的第一個欄位。 系統會將此視為主要欄位。
     >
     >* 除非您指定其他欄位作為主要欄位，否則您無法將「名稱」欄位移至其他位置。 如需詳細資訊，請繼續進行步驟5。<!--**********************accurate?**************-->
     >
     >

   * 變更主要欄位，以其他欄位取代第一欄中的欄位。 如需詳細資訊，請繼續步驟5。<!--**********************accurate?**************-->
...
1. 按一下&#x200B;**設定欄位**&#x200B;以進行確認。
   ...
   >[!TIP]
   >
   >按一下資料行中欄位名稱右側的&#x200B;**資訊**&#x200B;圖示![資訊圖示](assets/info-icon.png)以檢視其&#x200B;**描述**。

1. 按一下並拖曳欄分隔線，並將它們拖曳到所需的位置以增加欄寬。

   >[!TIP]
   >
   >您對欄寬和順序所做的變更是永久性的，並且所有存取相同檢視的使用者都能看到。

1. 暫留在欄標題上，然後按一下向下箭頭，然後按一下&#x200B;**隱藏欄位**

   或

   按一下表格工具列中的&#x200B;**欄位**，然後取消選取與您要隱藏的欄位（或欄）相關聯的切換。 顯示&#x200B;**欄位可見性和順序**&#x200B;方塊。
...
1. 按一下&#x200B;**欄位**&#x200B;圖示，然後選取與您要顯示在表格欄位中的欄位相關聯的切換。 預設會顯示所有欄位。

1. 執行下列動作以快速尋找符合關鍵字的記錄：

   1. 在&#x200B;**搜尋**&#x200B;方塊![搜尋圖示](assets/search-icon.png)中，開始輸入與熒幕上顯示之記錄的任何欄位相關聯的關鍵字。 正確相符的數目會顯示在搜尋專案旁邊，而具有正確相符專案的欄位會反白顯示。

      ![搜尋方塊在資料表檢視中具有結果藍色外框](assets/search-box-with-results-blue-outline-g-table.png)
...
在預覽環境的表格檢視中新增篩選器，等同於在生產環境中新增篩選器。

如需詳細資訊，請參閱本文中的[新增篩選器](#add-filters)一節。


<!--***************** at production, paste here the information from the Production section above *******************-->

### 新增排序 {#sort-information-1}
...
1. 建立資料表檢視，如文章[管理記錄檢視](/help/quicksilver/planning/views/manage-record-views.md)中所述。
1. 按一下表格頂端的&#x200B;**排序**&#x200B;圖示![排序圖示](assets/sort-icon.png)

   或

   暫留在表格檢視中的欄名稱上，按一下欄標題名稱右側的向下箭頭，然後按一下&#x200B;**排序**。

1. （視條件而定）如果按一下表格頂端的&#x200B;**排序**，會開啟&#x200B;**依**&#x200B;排序記錄方塊。 按一下其中一個建議的欄位，或按一下&#x200B;**選擇其他欄位**&#x200B;並搜尋其他欄位，然後在欄位顯示在清單中時按一下它。
...
1. （選擇性）按一下&#x200B;**新增條件**，然後重複上述步驟以依其他欄位排序。
   ...
   排序依據的欄位數會顯示在工具列右上角的「排序」圖示左側。 您只能選擇顯示在表格檢視欄中的欄位。
1. （視條件而定）如果您按一下表格頂端的&#x200B;**排序**&#x200B;圖示，請在&#x200B;**依**&#x200B;排序記錄方塊中選取欄位排序的順序。 排序順序的選項取決於您排序的欄位型別。
...
1. 按一下&#x200B;**依**&#x200B;排序記錄方塊外部以將其關閉。
   ...
   ![在資料表檢視中排序](assets/sorting-in-table-view-g-list.png)

   表格中顯示的資訊會根據您選取的條件排序。

   為排序選取的欄位會顯示排序圖示，後面跟著數字，表示套用排序的順序。

### 新增群組 {#add-groupings-1}
...
1. 建立記錄型別的資料表檢視，如文章[管理記錄檢視](/help/quicksilver/planning/views/manage-record-views.md)中所述。
1. 按一下表格檢視右上角的&#x200B;**群組**。
...
1. 若要展開或收合群組，請按一下&#x200B;**群組**&#x200B;圖示，然後&#x200B;**全部展開**，或&#x200B;**全部收合**。 這會展開表格檢視中的所有群組和子群組。
...
在「預覽」環境中的表格檢視中啟用即時存在指示器，與在「生產」環境中啟用它相同。

如需詳細資訊，請參閱本文中的[啟用即時狀態指示器](#enable-the-real-time-presence-indicator)區段。
