...
依預設，Planning API請求會從清單開頭傳回500個結果。 若要覆寫預設的結果數量限制，您可以使用 `limit` 引數，並將其設定為不同的數字，最多可達2000個結果。

建議您針對大型資料集使用分頁回應，方法是新增 `offset` 引數的對應章節。 分頁回應可讓您指定應傳回的第一個結果的位置。
...
如需排序的詳細資訊，請參閱 [排序API中的查詢結果](#sorting-query-results-in-the-api) 本文章內容。
