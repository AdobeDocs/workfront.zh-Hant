---
title: 適用於Workfront Planning的Workfront Data Connect
description: Workfront Data Connect可讓您使用Snowflake技術存取Workfront資料，包括表單Adobe Workfront Planning，進而回答業務關鍵問題並評估程式的價值。
author: Courtney, Alina
feature: Workfront Planning
role: User, Admin
recommendations: noDisplay, noCatalog
source-git-commit: 871bb92456f95b7f70072a072971b5114ad02890
workflow-type: tm+mt
source-wordcount: '202'
ht-degree: 0%

---


# 適用於Workfront Planning的Workfront Data Connect

Workfront Data Connect可讓您使用Snowflake技術存取Workfront資料，包括來自Adobe Workfront Planning的資料，以回答業務關鍵問題並評估流程的價值。 企業客戶可使用資料視覺化工具，直接從Workfront資料湖分析其資料，或將Workfront資料匯出至客戶擁有的資料倉儲。

訂閱者可自行選擇連結商務智慧工具（Tableau、Power BI、Domo、Looker等） 使用Data Connect建立具有以下資料型別的資料視覺效果：

* 低延遲，目前狀態資料
* 變更時間點比較的事件記錄
* 趨勢分析的每日歷史快照

如需使用Snowflake建立讀者帳戶及建立Workfront Data Connect連線的詳細資訊，請參閱下列文章：

* [建立Snowflake的讀取器帳戶或連線](/help/quicksilver/reports-and-dashboards/data-lake/create-a-reader-account.md)
* [建立與Workfront資料連線的連線](/help/quicksilver/reports-and-dashboards/data-lake/share-data-externally.md)


如需Workfront Data Connect資料字典中Planning資訊的相關資訊，請參閱[Workfront Data Connect資料字典](/help/quicksilver/reports-and-dashboards/data-lake/data-dictionary.md)。

