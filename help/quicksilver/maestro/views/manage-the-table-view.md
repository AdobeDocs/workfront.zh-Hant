...
1. 若要增加欄的寬度，請按一下並拖曳欄分隔線，然後將它們拖放到所需位置。

   >[!TIP]
   >
   >您對欄寬和順序所做的變更是永久性的，並且所有存取該記錄型別的使用者都可以看到。

1. 若要隱藏欄，請將游標停留在欄標題上，然後按一下向下箭頭，然後按一下 **隱藏欄位**

   或

   按一下 **欄位** ，並停用與您要隱藏的欄位關聯的切換。
...
1. 從 **欄位** 設定，啟用與您要顯示在表格欄位中的欄位相關聯的切換。 預設會顯示所有欄位。
