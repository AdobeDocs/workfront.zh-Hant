...
* 永久篩選器，可讓您在多個清單和報告上儲存和使用許多時間。 本文會說明如何建立永久篩選器，或編輯清單或報表中的現有篩選器。
...
1. 按一下 **新增篩選器** 以選取其他欄位，並將新篩選條件新增至篩選陳述式。
...
1. （選用）按一下 **新增篩選器群組** 以新增另一組篩選條件。 集合之間的預設運運算元為AND。 按一下運運算元以將其變更為OR。
...
1. （選用）按一下 **文字模式** 以使用文字模式繼續建立篩選器。
   ...
   >[!TIP]
   >
   >建議您使用標準產生器介面，並在您必須修改僅於文字模式支援的篩選器時，才使用文字模式，以儘可能多地建立篩選器。
...
1. 選取 **未命名的篩選器** 並輸入新篩選器的名稱。
   ...
   >[!TIP]
   >
   >請務必為篩選器命名，以便您稍後可以尋找。 如果您不為篩選器命名，則會在系統中將其稱為「未命名的篩選器」。
...
1. （可選）若要將篩選器移至「我的最愛」清單，請將滑鼠游標移至篩選器抽屜中的任何篩選器上，然後按一下「我的最愛」圖示 ![我的最愛圖示](assets/favorites-icon-small.png).
...
1. 按一下 **儲存篩選器** 建立新的篩選器，或將選取的篩選器取代為您所做的變更。
