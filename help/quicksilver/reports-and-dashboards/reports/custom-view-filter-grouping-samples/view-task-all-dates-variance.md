...
description: 此任務檢視類似於Adobe Workfront帳戶隨附的「所有日期」檢視。 此特定視圖包括「差異」列，這些列計算以下日期（「編輯我」）之間的天數差。
...
此任務檢視類似於Adobe Workfront帳戶隨附的「所有日期」檢視。 此特定檢視包含 *變異數* 欄，以計算下列日期之間的天數差異：
...
此檢視代表計算欄的範例，您可從兩個不同的欄取用值，並透過計算將其連結，以取得第三個值。 在此案例中，您會從日期2中減去日期1。

有關在視圖中使用計算的詳細資訊，請參閱 [文字模式常見用途概觀](../../../reports-and-dashboards/reports/text-mode/understand-common-uses-text-mode.md) 區段  [文字模式常見用途概觀](../../../reports-and-dashboards/reports/text-mode/understand-common-uses-text-mode.md).
