...
例如，當您需要將某個應用程式或Web服務的日期格式變更為相同案例中連線應用程式或Web服務的日期格式時，這會很有用。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL格式] </td> 
   <td>文字 </td> 
   <td> <p>可讓您使用日期/時間格式代號來指定格式。 如需詳細資訊，請參閱 <a href="../../workfront-fusion/functions/tokens-for-date-and-time-formatting.md" class="MCXref xref">日期和時間格式的Token [!DNL Adobe Workfront Fusion]</a>.</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例: </b></span></span><code>DD.MM.YYYY HH:mm</code> </p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL時區] </td> 
   <td>文字 </td> 
   <td> <p>（選用）可讓您指定用於轉換的時區。 </p> <p>如需可辨識時區的清單，請參閱維基百科中的「TZ資料庫名稱」欄 <a href="https://en.wikipedia.org/wiki/List_of_tz_database_time_zones">tz資料庫時區清單</a>. 函式只會將此欄中列出的值識別為有效時區。 系統會忽略任何其他值，而改用您的設定檔中指定的案例時區。 如需詳細資訊，請參閱文章中的 <a href="../../workfront-fusion/workfront-fusion-basics/change-profile-settings.md" class="MCXref xref">變更中的設定檔設定 [!DNL Adobe Workfront Fusion]</a>.</p> <p>如果您省略此引數，則會套用設定檔設定中指定的情境時區。 </p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例: </b></span></span><code>Europe/Prague</code>, <code>UTC</code></p> </td> 
  </tr> 
 </tbody> 
</table>

如果提供了不同的型別，則會套用型別強制轉換。 如需詳細資訊，請參閱 [輸入強制 [!DNL Adobe Workfront Fusion]](../../workfront-fusion/mapping/type-coercion.md).
...
當您有代表日期的文字值時(例如 `12-10-2019 20:30` 或 `Aug 18, 2019 10:00 AM`)，並想要將其轉換（剖析）為日期值（二進位電腦可讀表示法）。 如需詳細資訊，請參閱 [日期](../../workfront-fusion/mapping/item-data-types.md#date) 和 [文字](../../workfront-fusion/mapping/item-data-types.md#text) 在文章中 [中的專案資料型別 [!UICONTROL Adobe Workfront Fusion]](../../workfront-fusion/mapping/item-data-types.md).

### 參數

第二欄指出預期的型別。 如果提供了不同的型別，則會套用型別強制轉換。 如需詳細資訊，請參閱 [輸入強制 [!DNL Adobe Workfront Fusion]](../../workfront-fusion/mapping/type-coercion.md).
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL格式] </td> 
   <td>文字 </td> 
   <td> <p>可讓您使用日期/時間格式代號來指定格式。 如需詳細資訊，請參閱 <a href="../../workfront-fusion/functions/tokens-for-date-and-time-formatting.md" class="MCXref xref">Adobe Workfront Fusion中日期和時間格式的Token</a>.</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例: </b></span></span><code>DD.MM.YYYY HH:mm</code> </p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL時區] </td> 
   <td>文字 </td> 
   <td> <p>（選用）可讓您指定用於轉換的時區。 </p> <p>如需可辨識時區的清單，請參閱維基百科中的「TZ資料庫名稱」欄 <a href="https://en.wikipedia.org/wiki/List_of_tz_database_time_zones">tz資料庫時區清單</a>. 函式只會將此欄中列出的值識別為有效時區。 系統會忽略任何其他值，而改用您的設定檔中指定的案例時區。 如需詳細資訊，請參閱文章中的 <a href="../../workfront-fusion/workfront-fusion-basics/change-profile-settings.md" class="MCXref xref">變更Adobe Workfront Fusion中的設定檔設定</a>.</p> <p>如果您省略此引數，則會套用設定檔設定中指定的情境時區。</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例: </b></span></span><code>Europe/Prague</code>, <code>UTC</code></p> </td> 
  </tr> 
 </tbody> 
</table>
...
此函式根據您指定的格式和時區，將文字字串轉換為日期。 值的資料型別為「日期」。

>[!INFO]
>
>**範例：** 在以下範例中，傳回的Date值是根據ISO 8601來表示，但結果的資料型別為Date。
>
...
將指定時數新增至日期後，傳回新日期。 若要減去時數，請輸入負數。
...
將指定年數新增至日期後，傳回新日期。 若要減去年數，請輸入負數。
...
指定從0到59的數字。 如果數字超出該範圍，此函式會傳回前一分鐘（代表負數）或後續分鐘（代表正數）的秒數。
...
如果您需要指定範圍以外的數字，建議您使用addMinutes，如上方所述 [addMinutes (date； number)](#addminutes-date-number).
...
指定從0到23的數字。 如果數字超出此範圍，此函式會傳回前一天的一小時（若為負數）或後續的一天（若為正數）。

如果您需要指定範圍外的數字，我們建議您使用addHours，如上方所述 [addHours (date； number)](#addhours-date-number).
...
## [!UICONTROL setDay （日期；英文的日期數字/名稱）]

此函式使用引數中指定的日期傳回新日期。

您可以使用此函式將星期日設為1，將星期六設為7。 如果您指定從1到7的數字，則產生的日期在目前（星期日至星期六）的一週內。 如果數字超出該範圍，此函式會傳回前一週（負數）或後續周（正數）的某天。

如果您需要指定範圍以外的數字，建議您使用addDays，如上方所述 [addDays (date； number)](#adddays-date-number).
...
指定從1到31的數字。 如果數字超出此範圍，此函式會傳回前一個月中的一天（若為負數）或後續的月份（若為正數）。
...
本節適用於 [!DNL Workfront Fusion] 從 [!DNL Exceljet] 說明如何取得一個月中每週第n天的網頁。

如果您需要計算與當月某周n日相對應的日期（例如，第一個星期二、第三個星期五等），您可以使用下列公式：
...
如果您只想計算一個特定案例（例如，每逢星期三），則可以取代專案 `1.n` 和 `2.dow` 在公式中具有對應編號。 對於當月的第二個星期三，您會使用下列值：
...
* `setDate(now;1)` 傳回目前月份的第一個
* `formatDate(....;E)` 傳回星期幾(1、2、... 6)
...
>[!NOTE]
>
>* 值 `D1`和 `D2` 具有日期型別值。 如果是字串型別值（例如20.10.2018），請使用 `parseDate()` 函式以將其轉換為日期型別值。
>
>* 此 `round()` 函式適用於其中一個日期在日光節約時間期間內，而另一個日期不在日光節約時間期間的情況。 在這些情況下，時數的差異會少一小時或更多。 您可以將其除以24來得出非整數的結果。 您會失去一小時的日光節約時間。 將平面化倒圓角，這樣就不會有百分比

### 如何計算月份的最後一天/毫秒

例如，當您指定搜尋模組中的日期範圍時，如果範圍涵蓋整個前一個月為關閉間隔（包含其兩個限制點的間隔），則需要計算該月的最後一天。
...
在某些情況下，您不僅需要計算月份的最後一天，還需要實際計算其最後一毫秒：
