...
title: 在中建立簡單案例 [!DNL Adobe Workfront Fusion]
...
1. 在 [!UICONTROL 記錄型別] 方塊，選取 **[!UICONTROL 問題]**. 這會將模組設定為僅搜尋問題。

   您可以找到 **[!UICONTROL 問題]** 在清單中(如果您開始輸入&quot;[!UICONTROL 問題].」

1. 在 **[!UICONTROL 結果集]** 方塊，選取 **[!UICONTROL 第一個比對記錄]**.

   這會將模組設定為只傳回它認為符合條件的第一個記錄。
1. 在 **[!UICONTROL 搜尋條件]** 區域，設定條件以傳回特定問題。
   ...
   1. 在底下的最後一個欄位中 [!UICONTROL 搜尋條件]，輸入您知道位於搜尋問題名稱中的字詞或片語。
...
1. 以滑鼠右鍵按一下模組，然後按一下 **[!UICONTROL 重新命名]**，然後輸入名稱並描述您希望模組執行的動作（例如「搜尋問題」），然後按一下 **[!UICONTROL 確定]**.
...
1. 選取 [!DNL Adobe Workfront] 從應用程式清單中，然後選擇模組 **[!UICONTROL 其他動作]**.

   其他動作模組可讓您在Workfront中執行沒有專用模組的動作。 在此範例中，此模組用於將問題轉換為專案。
1. 在 [!UICONTROL 連線] 欄位中，選取您在上一個模組中使用的相同Workfront連線。
1. 在**中[!UICONTROL 記錄型別]**欄位，選擇 **[!UICONTROL 問題]**，因為要執行的動作與問題有關。
1. 在 **[!UICONTROL 動作]** 欄位，選取 **convertToProject**. 此動作會將所選問題轉換為專案。
