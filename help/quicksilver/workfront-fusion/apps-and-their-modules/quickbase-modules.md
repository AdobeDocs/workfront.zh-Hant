...
title: Quickbase模組
description: 在 [!DNL Adobe Workfront Fusion] 案例中，您可以自動執行使用Quickbase的工作流程，並將其連線到多個協力廠商應用程式和服務。
author: Becky
...
在 [!DNL Adobe Workfront Fusion] 情境下，您可以自動化使用下列專案的工作流程： [!DNL Quickbase]，以及將其連線到多個協力廠商應用程式和服務。
...
當您設定 [!DNL Quickbase] 模組， [!DNL Workfront Fusion] 顯示下列欄位。 除了這些以外， [!DNL Quickbase] 視您在應用程式或服務中的存取層級等因素而定，可能會顯示欄位。 模組中的粗體標題表示必填欄位。
...
此模組會呼叫任何可用的 [!DNL Quickbase] RESTful API端點紀錄在 [developer.quickbase.com](http://developer.quickbase.com/).
