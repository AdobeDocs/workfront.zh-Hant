...
在 [!DNL Adobe Workfront Fusion] 情境下，您可以自動化使用下列專案的工作流程： [!DNL Microsoft Dynamics 365]，以及將其連線到多個協力廠商應用程式和服務。
...
您可以建立與的連線 [!DNL Microsoft Dynamics 365] 直接從「 」內部 [!DNL Microsoft Dynamics 365] 模組。
...
當您設定 [!DNL Microsoft Dynamics 365] 模組， [!DNL Workfront Fusion] 顯示下列欄位。 除了這些以外， [!DNL Microsoft Dynamics 365] 視您在應用程式或服務中的存取層級等因素而定，可能會顯示欄位。 模組中的粗體標題表示必填欄位。
...
此排定的觸發模組會在您指定的物件中的記錄於中上次排定的執行之後建立或更新時執行情境 [!DNL Dynamics 365].

模組輸出會指出找到的記錄是新的還是更新的（如果在這段時間新增並更新了，則會標示為新記錄）。 您可以將此資訊對應到情境中的後續模組中。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <td> <p>如需有關連線您的電腦的指示， [!DNL Microsoft Dynamics 365] 帳戶至 [!DNL Workfront Fusion]，請參閱 <a href="#connect-microsoft-dynamics-365-to-workfront-fusion" class="MCXref xref">Connect [!DNL Microsoft Dynamics 365] 至 [!DNL Workfront Fusion]</a> 本文章內容。 </p> </td> 
  </tr> 
  <tr> 
...
當您指定的記錄（物件）在中建立或更新時，此即時觸發模組會執行案例 [!DNL Dynamics 365].
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>選取您要用於此模組的webhook。 </p> <p>若要新增新的webhook：</p> 
    <ol> 
     <li value="1"> <p>按一下 <strong>[！UICONTROL新增]</strong> Webhook欄位右側</p> </li> 
     <li value="2"> <p>在 <strong>[！UICONTROL Webhook]</strong> 名稱欄位，輸入webhook的描述性名稱。</p> </li> 
     <li value="3"> <p>在 <strong>[！UICONTROL連線]</strong> 欄位中，選取您要使用的連線已選取</p> <p>如需有關連線您的電腦的指示， [!DNL Microsoft Dynamics 365] 帳戶至 [!DNL Workfront Fusion]，請參閱 <a href="#connect-microsoft-dynamics-365-to-workfront-fusion" class="MCXref xref">Connect [!DNL Microsoft Dynamics 365] 至 [!DNL Workfront Fusion]</a> 本文章內容。 </p> </li> 
...
模組會傳回新實體的ID和任何關聯欄位，以及連線存取的任何自訂欄位和值。 您可以將此資訊對應到情境中的後續模組中。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr data-mc-conditions=""> 
   ...
   <td>在這些欄位中，輸入您想要工作專案對於指定屬性的值。 可用欄位取決於實體型別。</td> 
  </tr> 
 </tbody> 
</table>
...
模組會傳回狀態代碼、標題和本文的相關資訊。 您可以將此資訊對應到情境中的後續模組中。

若要深入瞭解，請參閱 [!DNL Microsoft] 關於使用的檔案 [!DNL Dynamics 365 Customer Engagement Web API].
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>選取設定API呼叫所需的HTTP要求方法。 如需詳細資訊，請參閱 <a href="../../workfront-fusion/modules/http-request-methods.md" class="MCXref xref" data-mc-variable-override="">中的HTTP要求方法 [!DNL Adobe Workfront Fusion]</a>.</p> <p>如需詳細資訊，請參閱</p> </td> 
  </tr> 
  <tr> 
...
模組會傳回實體的ID和任何關聯欄位，以及連線存取的任何自訂欄位和值。 您可以將此資訊對應到情境中的後續模組中。
...
模組會傳回更新記錄的ID和任何關聯欄位，以及連線存取的任何自訂欄位和值。 您可以將此資訊對應到情境中的後續模組中。
...
此搜尋模組會在中尋找物件中的記錄 [!DNL Microsoft Dynamics 365] 符合您指定的搜尋查詢。 您可以將此資訊對應到情境中的後續模組中。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>選取您要用於此搜尋的篩選器。</p> 
    <ul> 
     <li> <p><strong>[！UICONTROL標準篩選器]</strong> </p> <p>選取欄位和運運算元，然後輸入或對應您要搜尋的值，以設定篩選。 您可以使用AND或OR規則來篩選篩選器。</p> </li> 
     <li> <p><strong>[！UICONTROL查詢函式]</strong> </p> <p>輸入 [!DNL Dynamics 365] 您要用來搜尋的網頁API查詢函式。 </p> <p>如需查詢函式的詳細資訊，請參閱 <a href="https://docs.microsoft.com/en-us/dynamics365/customer-engagement/web-api/queryfunctions?view=dynamics-ce-odata-9">Web API查詢函式參考</a> 在 [!DNL Microsoft] 說明檔案。</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL排序]</td> 
   <td> <p>指定傳回專案的順序。 您可以新增多個排序。</p> 
    <ul> 
     <li> <p><strong>[！UICONTROL欄位]</strong> </p> <p>指定您要排序結果的欄位。</p> </li> 
     <li> <p><strong>[！UICONTROL方向]</strong> </p> <p>指定排序的方向（升序或降序）。</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL最大記錄數]</td> 
