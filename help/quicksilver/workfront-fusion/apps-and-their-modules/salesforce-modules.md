...
>* 有關從傳回的特定錯誤的資訊 [!DNL Salesforce] API，請參閱 [!DNL Salesforce] API檔案。 您也可以檢查 [!DNL Salesforce] API用於任何可能的服務中斷。
>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL型別] </td> 
   <td> <p>選擇型別 [!DNL Salesforce] 錄製您想要模組觀看的內容。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL記錄欄位]</td> 
...
1. 前往 [!DNL Salesforce] 設定頁面。
   ...
   若要存取設定頁面，請尋找並按一下標示為「 」的按鈕[!UICONTROL 設定]&quot;，位於的右上角 [!DNL Salesforce] 帳戶。 從 [!DNL Salesforce] 設定頁面，找出&quot;[!UICONTROL 快速尋找/搜尋]「左側的列。 搜尋&quot;[!UICONTROL 工作流程規則].」

1. 按一下 **[!UICONTROL 工作流程規則]**.
1. 在 [!UICONTROL 工作流程規則] 在出現的頁面上，按一下 **[!UICONTROL 新規則]** 並選取規則將套用的物件型別(例如&quot;[!UICONTROL 商機]&quot;如果您在監視商機記錄的更新)。
1. 按一下 **[!UICONTROL 下一個]**.
...
1. 指定名稱、說明、端點URL，以及要納入新傳出訊息的欄位，然後按一下 **[!UICONTROL 儲存]**.
   ...
   此 **[!UICONTROL 端點Url]** 欄位包含在上提供的URL [!DNL Salesforce] [!UICONTROL 傳出訊息] 在 [!DNL Workfront Fusion].

1. 設定以開頭的情境 [!UICONTROL 傳出訊息] 事件。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL記錄型別] </td> 
   <td> <p>選擇型別 [!DNL Salesforce] 錄製您希望模組監視傳出訊息的內容。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL欄位]</td> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL記錄型別] </td> 
   <td> <p>選取記錄型別，其中包含您要模組觀看的欄位。 您必須選擇已開啟[！UICONTROL欄位歷程記錄]的記錄型別 [!DNL Salesforce] 設定。 如需詳細資訊，請參閱 <a href="https://help.salesforce.com/articleView?id=tracking_field_history.htm&amp;type=5">欄位歷史記錄追蹤</a> 在 [!DNL Salesforce] 檔案。 </p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL欄位]</td> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>選擇型別 [!DNL Salesforce] 您要模組建立的記錄。 根據[！UICONTROL記錄型別]欄位中所選的記錄型別，欄位將變為可用。 這些欄位是根據 [!DNL Salesforce] API。</p> </td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr>
    <td>[！UICONTROL記錄型別]</td>
    <td>選擇型別 [!DNL Salesforce] 記錄您希望模組[action].read執行的動作。</td>
  </tr> 
  <tr>
    <td>[！UICONTROL記錄欄位]</td>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[！UICONTROL記錄型別] </td> 
   <td> <p>選擇型別 [!DNL Salesforce] 您要模組刪除的記錄。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL ID]</td> 
...
* **[!UICONTROL 內文]** （物件）：根據您在設定模組時選取的HTTP要求，您可能會收到傳回的部分資料。 該資料，例如來自 [!UICONTROL GET] 請求，包含在此物件中。
...
>[!INFO]
>
>**範例：** 以下API呼叫會傳回您網站中 [!DNL Salesforce] 帳戶：
>
>* **URL**： `query`
>
>* **方法**： [!UICONTROL GET]
  ...
  <tr>
    ...
    <td> <p>指定您要從Salesforce下載的檔案型別。</p> 
     <ul> 
      <li>[！UICONTROL附件]</li> 
      <li>[！UICONTROL檔案]</li> 
      <li>[！UICONTROL ContentDocument] （這是已上傳至中的程式庫的檔案） [!DNL Saleforce CRM Content] 或 [!DNL Salesforce Files].)</li> 
     </ul> </td>
  </tr> 
  <tr>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> <p>[！UICONTROL記錄型別] </p> </td> 
   <td> <p>選擇型別 [!DNL Salesforce] 記錄您希望模組更新的內容。 根據在「記錄型別」欄位中選取的記錄型別，欄位將變為可用。 這些欄位是根據 [!DNL Salesforce] API。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL選擇要對應的欄位]</td> 
   <td> <p>選取建立新記錄時您希望模組設定的欄位。 必填欄位位於清單頂端。 </p> <p>您選取的欄位會在此欄位下方開啟。 您現在可以在這些欄位中輸入值。</p> </td> 
...
此搜尋模組會在中尋找物件中的記錄 [!DNL Salesforce] 符合您指定的搜尋查詢。 您可以在情境中的後續模組中對應此資訊。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>如果您選取了「簡單」搜尋型別，請選擇型別 [!DNL Salesforce] 您要模組搜尋的記錄。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL Query] / [！UICONTROL SOSL Query] / [！UICONTROL SOQL Query]</td> 
   <td> <p>輸入搜尋依據的查詢。</p> <p>如需有關SOSL的詳細資訊，請參閱 <a href="https://developer.salesforce.com/docs/atlas.en-us.soql_sosl.meta/soql_sosl/sforce_api_calls_sosl.htm">Salesforce物件搜尋語言(SOSL)</a> 在 [!DNL Salesforce] 檔案。</p> <p>如需有關SOQL的詳細資訊，請參閱 <a href="https://developer.salesforce.com/docs/atlas.en-us.soql_sosl.meta/soql_sosl/sforce_api_calls_soql.htm">Salesforce物件查詢語言(SOQL)</a> 在 [!DNL Salesforce] 檔案。</p> <p>注意：請注意，引數的值 <code>RETURNING </code>會影響模組的輸出。 如果您使用 <code>LIMIT</code>， [!DNL Fusion] 將忽略[！UICONTROL最大記錄計數]欄位中的設定。 如果您未設定任何限制，Fusion將會插入值[！UICONTROL LIMIT =最大記錄計數]。</p> </td> 
  </tr> 
  <tr> 
   <td>[！UICONTROL最大記錄數]</td> 
   <td> <p>輸入或對應您希望模組在每個案例執行週期中傳回的最大記錄數。</p> </td> 
