...
title: Split.io模組
description: 在 [!DNL Adobe Workfront Fusion] 情境下，您可以自動化使用下列專案的工作流程： [!DNL Split.io]，以及將其連線到多個協力廠商應用程式和服務。
author: Becky
...
在 [!DNL Adobe Workfront Fusion] 情境下，您可以自動化使用下列專案的工作流程： [!DNL Split.io]，以及將其連線到多個協力廠商應用程式和服務。
...
使用 [!DNL Split.io] 模組，您必須擁有 [!DNL Split.io] 帳戶。

## Connect [!DNL Split.io] 至 [!DNL Workfront Fusion] {#connect-split-io-to-workfront-fusion}

您可以建立與的連線 [!DNL Split.io] 直接從a內的帳戶 [!DNL Split.io] 模組。

1. 在任何 [!DNL Split.io] 模組，按一下 **[!UICONTROL 新增]** 旁邊 [!UICONTROL 連線] 欄位。
1. 輸入連線的名稱。
1. 輸入您的 [!DNL Split.io] api金鑰。

   如需詳細資訊，請參閱 [!DNL Split.io] API金鑰，請參閱 [API金鑰](https://help.split.io/hc/en-us/articles/360019916211-API-keys) 在 [!DNL Split.io] 說明檔案。

1. 按一下 **[!UICONTROL 繼續]** 以建立連線並返回模組。

## [!DNL Split.io] 模組及其欄位

當您設定 [!DNL split.io] 模組， [!DNL Workfront Fusion] 顯示下列欄位。 除了這些以外， [!DNL split.io] 視您在應用程式或服務中的存取層級等因素而定，可能會顯示欄位。 模組中的粗體標題表示必填欄位。
...
* [[!UICONTROL 環境中的部分更新分割定義]](#partial-update-split-definition-in-environment)
* [[!UICONTROL 關聯標籤]](#associate-tags)

#### [!UICONTROL 自訂API呼叫]

此動作模組可讓您對 [!DNL split.io] API。 如此一來，您就可以建立其他人無法完成的資料流程自動化 [!DNL split.io] 模組。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>如需有關連線您的電腦的指示， [!DNL Split.io] 帳戶至 [!DNL Workfront Fusion]，請參閱 <a href="#connect-split-io-to-workfront-fusion" class="MCXref xref">Connect [!DNL Split.io] 至[！UICONTROL Workfront Fusion] </a> 本文章內容。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL URL]</td> 
   <td>輸入相對於 <code>https://api.split.io/internal/api/v2/</code>.</td> 
  </tr> 
  <tr> 
...
在指定流量型別的情況下，此動作模組會在您的組織中建立新的分割。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td role="rowheader">[！UICONTROL分割名稱]</td> 
   <td> <p>輸入或對映您要建立的分割名稱。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL分割說明]</td> 
   <td>輸入或對應您要建立的分割的[！UICONTROL split]說明。</td> 
  </tr> 
 </tbody> 
</table>
...
此動作模組會從您的組織刪除分割。 這會自動從所有環境中取消設定分割定義。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>針對您想要新增至定義的每個鎖定目標規則，按一下 <b>[！UICONTROL新增專案]</b>，然後輸入或對應規則。</p> <p>如需鎖定目標規則的詳細資訊，請參閱 <a href="https://docs.split.io/reference#create-split-definition-in-environment">在環境中建立分割定義</a> 在 [!DNL Split.io] 說明檔案。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL預設規則]</td> 
   <td> <p>輸入或對映您想要分割用於不符合其他規則規格的流量的規則。</p> <p>如需鎖定目標規則的詳細資訊，請參閱 <a href="https://docs.split.io/reference#create-split-definition-in-environment">在環境中建立分割定義</a> 在 [!DNL Split.io] 說明檔案。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL預設處理]</td> 
   <td> <p>輸入或對應您要分割使用的處理方式（如果分割被終止或客戶未包含在流量分配中）。</p> <p>如需處理的詳細資訊，請參閱 <a href="https://docs.split.io/reference#create-split-definition-in-environment">在環境中建立分割定義</a> 在 [!DNL Split.io] 說明檔案。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL處理]</td> 
   <td> <p>針對您想要新增至定義的每個處理，按一下 <b>[！UICONTROL新增專案]</b>，然後輸入或對映處理方式。</p> <p>如需處理的詳細資訊，請參閱 <a href="https://docs.split.io/reference#create-split-definition-in-environment">在環境中建立分割定義</a> 在 [!DNL Split.io] 說明檔案。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>針對您要更新的分割的每個屬性，按一下 <b>[！UICONTROL新增專案]</b> 並輸入或對應所需的變更。</p> <p>如需詳細資訊，請參閱 <a href="https://docs.split.io/reference#partial-update-split-definition-in-environment">環境中的部分更新分割定義</a> 在 [!DNL Split.io] 說明檔案。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[！UICONTROL註解]</td> 
   <td>輸入或對應您要新增至分割定義的任何註解。</td> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>針對您要新增的每個標籤，按一下 <b>[！UICONTROL新增專案]</b> 並輸入或對應標籤。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td role="rowheader">[！UICONTROL限制]</td> 
   <td>輸入或對應每個案例執行週期中您希望模組擷取的最大工作區數量。</td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td role="rowheader">[！UICONTROL限制]</td> 
   <td>輸入或對應您希望模組在每個案例執行週期中擷取的分割數上限。</td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td role="rowheader">[！UICONTROL限制]</td> 
   <td>輸入或對應每個案例執行週期中您希望模組擷取的最大分割定義數目。</td> 
  </tr> 
 </tbody> 
</table>
