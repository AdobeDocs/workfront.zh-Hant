...
title: Adobe大師模組
description: 使用 [!DNL Adobe Maestro] 模組，您可以啟動 [!DNL Adobe Workfront Fusion] 根據您設定之事件的案例 [!DNL Adobe] Maestro帳戶、建立、讀取或更新協定與其他記錄、使用您設定的條件搜尋記錄，以及上傳檔案。
author: Becky
...
# [!DNL Adobe Maestro] 模組

使用 [!DNL Adobe Maestro] 模組，則您可以在Maestro中發生事件時觸發情境。 您也可以建立、讀取、更新和刪除記錄，或執行自訂API呼叫至 [!DNL Adobe Maestro] 帳戶。
...
## 建立與的連線 [!DNL Adobe Maestro]

您可以建立與您的電腦的連線， [!DNL Maestro] 直接從a內的帳戶 [!DNL Workfront Fusion] 模組。

1. 在任何 [!DNL Maestro] 應用程式模組，按一下 **[!UICONTROL 新增]** 在 [!UICONTROL 連線] 方塊。
1. 輸入此連線的名稱。
1. 選取您要連線至生產環境或非生產環境。
1. 選取您要連線到服務帳戶還是個人帳戶。
1. 按一下 **[!UICONTROL SAML登入]** 以建立連線並返回模組。

## [!DNL Adobe Maestro] 模組及其欄位

### 觀看活動

在Maestro中建立、更新或刪除記錄、記錄型別或工作區時，此觸發模組就會啟動案例。
...
<table style="table-layout:auto"> 
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>有關建立與的連線的指示 [!DNL Adobe Maestro]，請參閱 <a href="#create-a-connection-to-adobe-maestro" class="MCXref xref" >建立與的連線 [!DNL Adobe Maestro]</a> 本文章內容。</td>
    </tr>
    <tr>
...
此動作模組會依據其ID刪除Maestro中的單一記錄型別。

>[!WARNING]
>
>刪除Maestro中的記錄型別也會刪除記錄型別表格中的所有記錄。
  ...
  <tbody>
    <tr>
     ...
     <tr>
      ...
      <td>輸入或對應您要建立的記錄型別。 可用的記錄型別取決於您的Maestro帳戶。</td> 
      </tr>
     <tr>
      <td role="rowheader">
        <p>其他欄位</p>
...
此動作模組會從擷取單一記錄 [!DNL Adobe Maestro]，以其ID指定。
  ...
  <tbody>
    <tr>
     ...
     <tr>
      ...
      <td>輸入或對應您要更新的記錄型別。 可用的記錄型別取決於您的Maestro帳戶。</td> 
      </tr>
     <tr>
      <td role="rowheader">
        <p>其他欄位</p>
