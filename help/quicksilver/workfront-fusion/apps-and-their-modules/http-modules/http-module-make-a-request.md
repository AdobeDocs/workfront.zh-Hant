...
title: HTTP &gt;提出要求模組
description: Adobe Workfront Fusion HTTP &gt;Make要求模組是通用模組，可讓您設定HTTP要求並將其提交至伺服器。 接著，接收的HTTP回應便包含在輸出套件中。
author: Becky
...
# [!UICONTROL HTTP] >[!UICONTROL 提出請求] 模組

>[!NOTE]
>
>Adobe Workfront Fusion需要 [!DNL Adobe Workfront Fusion] 除 [!DNL Adobe Workfront] 授權。

此 [!DNL Adobe Workfront Fusion] [!UICONTROL HTTP] > [!UICONTROL 提出要求模組] 是通用模組，可讓您設定HTTP要求並提交至伺服器。 接著，接收的HTTP回應便包含在輸出套件中。
...
如需 [!DNL Adobe Workfront Fusion] 許可證，請參閱 [Adobe Workfront Fusion授權](../../../workfront-fusion/get-started/license-automation-vs-integration.md)

## [!UICONTROL HTTP] >[!UICONTROL 提出請求] 模組配置

當您設定 [!UICONTROL HTTP] >[!UICONTROL 提出請求] 模組， [!DNL Adobe Workfront Fusion] 顯示下列欄位。 模組中的粗體標題表示必填欄位。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>HTTP Body是在HTTP交易訊息中傳送的資料位元組，若有需要使用，則緊接在標題後。</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>此內文類型是用於[!UICONTROLPOST]資料，使用 <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>針對 <code>application/x-www-form-urlencoded</code>，則傳送至伺服器的HTTP訊息內文基本上是一個查詢字串。 索引鍵和值會以由 <code>&amp;</code> 和 <code>=</code> 在索引鍵和值之間。 </p> <p>對於二進位資料，請使用 <code>[!UICONTROL multipart/form-data]</code> 。</p> 
  ...
  <tr> 
   ...
   <td> <p> 如果要使用基本授權傳送請求，請輸入用戶名。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL密碼] </td> 
   <td> <p>如果要使用基本授權發送請求，請輸入密碼。</p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>確保 [!UICONTROL JSON] 有效，則您可以使用其中一個可用的線上服務，例如 [https://jsonlint.com/](https://jsonlint.com/). 您也可以使用 [!UICONTROL JSON] >[!UICONTROL 建立JSON模組] 以動態建立JSON並處理所有必要的逸出。
>
>將JSON片段與運算式和項目直接混合在 [!UICONTROL 要求內容] 不建議使用欄位，因為欄位可能導致JSON無效。
