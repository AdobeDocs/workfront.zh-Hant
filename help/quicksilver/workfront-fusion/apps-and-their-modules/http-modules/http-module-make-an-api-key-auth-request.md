...
title: HTTP &gt;提出API金鑰授權要求
description: 此 [!DNL Adobe Workfront Fusion] 動作模組會傳送HTTPS要求至需要API金鑰驗證授權的指定URL，並處理回應。
author: Becky
...
# HTTP >[!UICONTROL 提出API金鑰授權要求]

>[!NOTE]
>
>[!DNL Adobe Workfront Fusion] 要求 [!DNL Adobe Workfront Fusion] 授權，以及Adobe Workfront授權。

此 [!DNL Adobe Workfront Fusion] 動作模組會傳送HTTPS要求至需要API金鑰驗證授權的指定URL，並處理回應。
...
如需 [!UICONTROL Adobe Workfront融合] 許可證，請參閱 [Adobe Workfront Fusion授權](../../../workfront-fusion/get-started/license-automation-vs-integration.md)

## [!UICONTROL HTTP] >[!UICONTROL 提出API金鑰授權要求] 模組配置

當您設定 [!UICONTROL HTTP] >[!UICONTROL 提出API金鑰授權要求] 模組， [!DNL Adobe Workfront Fusion] 顯示下列欄位。 模組中的粗體標題表示必填欄位。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>選取包含您API金鑰驗證憑證的金鑰。 若要新增金鑰，請按一下 <strong>[!UICONTROL添加]</strong> 和設定下列資訊：</p> 
    <ul> 
     <li> <p><strong>[!UICONTROL密鑰名]</strong></p> <p>輸入此API憑證集的名稱。</p> </li> 
     <li> <p><strong>[!UICONTROL密鑰]</strong> </p> <p>輸入API金鑰。</p> </li> 
     <li> <p><strong>[!UICONTROL API密鑰位置]</strong> </p> <p>選取要將API金鑰放在標題或API呼叫的查詢中。</p> </li> 
     <li> <p><strong>[!UICONTROL API密鑰參數名稱]</strong> </p> <p>輸入API呼叫用來識別API金鑰的名稱，例如"apiKey"或"X-API-Key"。 您可以在模組所連接的Web服務文檔中找到此資訊。</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL將所有狀態評估為錯誤（2xx和3xx除外）] </td> 
   <td> <p>使用此選項可設定錯誤處理。</p> <p>如需詳細資訊，請參閱 <a href="../../../workfront-fusion/errors/error-handling.md" class="MCXref xref">Adobe Workfront Fusion中的錯誤處理</a>.</p> </td> 
  </tr> 
  <tr> 
