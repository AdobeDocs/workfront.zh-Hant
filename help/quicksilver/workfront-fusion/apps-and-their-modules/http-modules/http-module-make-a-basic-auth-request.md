...
此 [!DNL Adobe Workfront Fusion] 模組可讓您使用HTTP基本授權來設定HTTP要求，並將其提交至伺服器。 接著，接收的HTTP回應便包含在輸出套件中。
...
## [!UICONTROL HTTP] >[!UICONTROL 提出基本授權請求] 模組配置
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>選取包含基本驗證憑證的金鑰，或按一下 <strong>[!UICONTROL添加]</strong> 將憑據添加到新密鑰中。 </p> <p>注意：您可以新增更多憑證，以便輕鬆在每個連線之間切換。</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>HTTP Body是在HTTP交易訊息中傳送的資料位元組，若有需要使用，則緊接在標題後。</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>此內文類型是用於[!UICONTROLPOST]資料，使用 <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>針對 <code>[!UICONTROL application/x-www-form-urlencoded]</code>，則傳送至伺服器的HTTP訊息內文基本上是一個查詢字串。 索引鍵和值會以由 <code>&amp;</code> 和 <code>=</code> 在索引鍵和值之間。 </p> <p>對於二進位資料，請使用 <code>multipart/form-data</code> 。</p> 
  ...
  <tr> 
   ...
   <td> <p>啟用此選項，可在HTTP要求中使用相互TLS。</p> <p>如需相互TLS的詳細資訊，請參閱 <a href="../../../workfront-fusion/apps-and-their-modules/http-modules/use-mtls-in-http-modules.md" class="MCXref xref">在的HTTP模組中使用相互TLS  </a>.</p> </td> 
  </tr> 
 </tbody> 
</table>
