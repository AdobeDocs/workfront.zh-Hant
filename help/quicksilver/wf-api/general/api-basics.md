...
<pre>GET /attask/api/v15.0/task/search？name=Planning<br>&amp;name_Mod=contains<br>&amp;OR:1:產品組合：name=FixedAssets<br>&amp;OR:1:產品組合：name_Mod=eq<br>&amp;OR:1:assignedTo：name=Steve<br>&amp;OR:1:assignedTo：name_Mod=cicontains<br>&amp;OR:2:父項：name=最終任務<br>&amp;OR:2:Name_Name mod=eq
</pre>
...
<pre>GET /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxx？method=put &amp;updates={ accessRules： [ {accessorID： 'abc123'， accessorObjCode： 'USER'， coreAction： 'VIEW'} ] }</pre>或者，若只要與新使用者共用，並保持現有許可權不變：
<pre>GET /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxx/share？method=put&amp;accessorID=abc123&amp;accessorObjCode=USER&amp;coreAction=VIEW</pre>若要擷取現有的存取規則：
...
<pre>POST /attask/api/v15.0/document？updates=&lbrace;<br>名稱： aFileName，<br>控制代碼： abc...123，（檔案上傳的控制代碼）<br> docObjCode： PROJ， （或TASK、OPTASK等）<br> objID： abc...123，<br> currentVersion：{version：v1.0，fileName：aFileName}<br></pre>
...
<pre>PUT /attask/api/v15.0/project/4c7...？updates= <br>{<br>名稱： "新專案名稱"， <br>狀態： "CUR"， <br> ... <br>}</pre>
...
下列範例示範與團隊共用專案的語法：
<pre>PUT /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxxxx/share？accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxx&amp;accessorObjCode=TEAMOB</pre>編輯物件時，您可以執行PUT並傳送類似下列範例的更新，以取代物件上的所有存取規則：
<pre>PUT /attask/api/v15.0/project/123abcxxxxxxxxxxxxxxxxxxxxxx？method=PUT&amp;updates=&lbrace;accessRules：[{accessorID：'123abcxxxxxxxxxxxxxxxxxxxxxxxxxx'，accessorObjCode：'TEAMOB'，coreAction：'VIEW'}]</pre>下列範例顯示將任務從一個專案移動到另一個專案的語法：
...
<pre>PUT /attask/api/v15.0/proj？Umethod=PUT&amp;updates=[{"ID"："123abcxxxxxxxxxxxxxxxxxxxxxxxxxx"，"name"："Test_Project_1_ Edit"}，{"ID"："123abcxxxxxxxxxxxxxxxxxxxxxxxx"，"name"："Test_Project_2_Edit"}]&amp;apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>會傳回類似下列的內容：
