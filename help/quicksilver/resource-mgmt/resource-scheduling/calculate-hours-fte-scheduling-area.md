...
title: 配置Workfront如何計算計畫區域的資源小時數和FTE可用性
description: Adobe Workfront管理員在配置資源管理首選項時確定Workfront在系統級別（考慮小時數和FTE可用性）計算資源可用性和用戶分配的方式。
author: Alina
...
# 配置Workfront如何計算計畫區域的資源小時數和FTE可用性
...
Adobe Workfront管理員在配置資源管理首選項時確定Workfront在系統級別（考慮小時數和FTE可用性）計算資源可用性和用戶分配的方式。
...
* 顯示分配指標時，如以下文章所述：
   ...
   * 該條中的「分配指標」一節  [在「計畫」區域中管理用戶分配](../../resource-mgmt/resource-scheduling/manage-allocations-scheduling-areas.md).
