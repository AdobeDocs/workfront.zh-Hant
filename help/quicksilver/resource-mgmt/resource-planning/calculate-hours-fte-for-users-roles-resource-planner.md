...
全職相當值(FTE)是時間測量值，代表使用者或工作角色在一天或一週內專用於實際工作的時數。
...
Workfront管理員在「設定」的「資源管理」區域中選取下列其中一項，以決定使用者可用時間的計算方式：
...
例如，如果使用者的「可用時數」值為40，且他們可以在75%的時間內完成一個「主要角色」，並在25%的時間內完成一個「其他角色」，則「資源規劃工具」會顯示一週「主要角色」的「可用時數」值為30小時，而「其他角色」的「可用時數」值為10小時。 在此案例中，主要角色的FTE為0.75，而其他角色的FTE為0.25。
...
下表說明如何根據系統管理員在「資源管理偏好設定」中用於FTE計算的方法，為「資源規劃工具」中的使用者計算「可用時數」和「可用FTE」。
...
>[!NOTE]
>
>計算所有值(資源規劃工具中的可用(AVL) FTE值除外)的FTE時，使用者的排程會被忽略。 計算時只考慮「預設排程」。
