...
為了將資源規劃工具中顯示的資訊量減到最少，Adobe Workfront提供了具有預先設定條件的預設篩選器。 如需有關預設篩選的資訊，請參閱區段 [資源規劃工具中的預設篩選器概要](#overview-of-the-default-filter-in-the-resource-planner) 本文章內容。

您也可以建置自訂的篩選器。 如需在資源規劃工具中自訂篩選器的資訊，請參閱區段 [建立資源規劃工具篩選器](#create-resource-planner-filters) 本文章內容。
...
* 當您為資源規劃工具選取不同的檢視時，篩選的結果不會變更。\
  如需有關變更資源規劃工具中檢視的詳細資訊，請參閱以下的「專案/角色/使用者檢視」選擇區段： [資源規劃工具導覽概觀](../../resource-mgmt/resource-planning/resource-planner-navigation.md).
...
當您第一次開啟資源規劃工具時，Workfront會套用預設篩選器。 您可以編輯「預設」篩選器，僅篩選您要顯示的專案。 如需修改篩選器的詳細資訊，請參閱區段 [在資源規劃工具中編輯篩選器](#edit-a-filter-in-the-resource-planner) 本文章內容。
...
如需有關滿足使用資源規劃工具的必要先決條件的資訊，請參閱 [資源規劃工具概觀](../../resource-mgmt/resource-planning/get-started-resource-planner.md) 文章。
...
1. 按一下 **資源**.
   ...
   根據預設，第一次存取資源規劃工具時， <strong>預設篩選器</strong> 中所有規則的URL區段。<br>如需有關預設篩選的詳細資訊，請參閱 <a href="#overview-of-the-default-filter-in-the-resource-planner" class="MCXref xref">資源規劃工具中的預設篩選器概要</a> 一節。

1. 在的左上角，按一下 **篩選** 圖示。\
   ![filter_icon.png](assets/filter-icon.png)\
   或\
   展開 **篩選** 下拉式功能表並按一下 **新增篩選器**.\
   ![](assets/rp-filter-dropdown-expanded-with-default-filter-selected-350x283.png)

1. 若要使用內建條件建立篩選器，請指定下列任一欄位：

   * **Portfolio**：開始輸入包含您要納入資源規劃工具之資訊的投資組合名稱，然後在其出現在清單中時按一下名稱。\
     重複此過程以包含來自多個產品組合的資訊。

   * **專案狀態**：展開「專案狀態」下拉式功能表，然後選取清單中可用的一個或多個專案狀態。
   * **團隊**：開始輸入與使用者相關聯的一個或多個團隊名稱，這些使用者指派給您要檢視專案中的任務。
   * **工作角色**：開始輸入與使用者相關聯的一個或多個職務角色名稱，這些使用者指派給您要檢視專案中的任務。
   * **集區**：開始輸入與專案（針對「專案檢視」）、使用者（針對「使用者檢視」），或與您要檢視的專案和使用者（針對「角色檢視」）相關聯的一或多個資源集區的名稱。
   * **群組**：開始輸入一個或多個群組的名稱，這些群組與您要檢視的使用者（在使用者檢視中）或專案（在專案和角色檢視中）相關聯。

1. 按一下 **新增篩選器規則**，然後開始輸入您要在中篩選的欄位名稱 **輸入以篩選專案** 方塊。 如果欄位可供使用，則會針對可與其關聯的每個物件填入欄位。
...
1. 當欄位出現在清單中時，按一下欄位名稱可將其新增到篩選器中。\
   有關您在清單中看到的欄位的詳細資訊，請參閱 [Adobe Workfront術語表](../../workfront-basics/navigate-workfront/workfront-navigation/workfront-terminology-glossary.md).

1. （選擇性）選取篩選的篩選和條件修飾元。 有關可用修飾元的說明，請參見 [篩選和條件修飾元](../../reports-and-dashboards/reports/reporting-elements/filter-condition-modifiers.md).

   您可以使用基於使用者或基於日期的萬用字元來篩選與登入使用者相關聯的資訊。\
   如需篩選器支援的萬用字元相關資訊，請參閱 [萬用字元篩選器變數概觀](../../reports-and-dashboards/reports/reporting-elements/understand-wildcard-filter-variables.md).

1. 按一下 **儲存**&#x200B;以儲存篩選規則。
1. （選用）按一下 **新增篩選器規則** 為另一個物件或欄位新增規則。
...
1. （視條件而定）按一下 **儲存**，請在「 」中指定篩選器的名稱 **篩選器名稱** 方塊內 **儲存篩選器** 對話方塊。 這是必填欄位。\
...
1. 前往資源規劃工具並展開 **篩選** 下拉式功能表以選取儲存的篩選器。
1. 展開 **篩選** 下拉式功能表。 找到您要重新命名的篩選器，並將滑鼠游標停留在篩選器名稱上。
1. 選取 **重新命名篩選器** 圖示加以篩選。

   ![](assets/rp-filter-options-edit-350x154.png)

1. 在中為篩選器指定新名稱 **篩選器名稱** 方塊。
1. 按一下&#x200B;**儲存**。\
   篩選器中包含的資訊相同，且名稱會更新。
...
1. 前往資源規劃工具並展開 **篩選** 左上角的下拉式功能表。
1. 選取要編輯的現有篩選器。
1. 按一下 **篩選** 圖示。\
   ![filter_icon.png](assets/filter-icon.png)

1. 新增欄位至篩選器。\
   如需建立篩選器的相關資訊，請參閱 [建立資源規劃工具篩選器](#create-resource-planner-filters).

1. 暫留在為篩選器選取的現有欄位上，然後按一下 **編輯** 圖示以選取其他欄位，或 **刪除** 圖示以刪除欄位。\
   ![RP_custom_filter_delete_and_edit_icons.png](assets/rp-custom-filter-delete-and-edit-icons-350x169.png)

1. （選用）按一下 **新增篩選器規則** 以新增欄位至篩選器。\
...
如需有關預設篩選的資訊，請參閱 [資源規劃工具中的預設篩選器概要](#overview-of-the-default-filter-in-the-resource-planner) 一節。
...
如需有關複製篩選器的資訊，請參閱 [複製篩選器](#duplicate-a-filter) 一節。
...
1. （選用）若要讓所有資源規劃工具使用者都能使用該篩選器，請按一下 **設定** 圖示，然後選取 **使其在整個系統內可見**.

   ![](assets/make-this-visible-system-wide-350x119.png)

1. 在 **將資源規劃工具篩選存取權授予：** 方塊中，開始輸入您要共用篩選器的使用者、團隊、角色、群組或公司名稱。
1. 從下列許可權層級中選取：

   * 檢視
   * 管理

     如需Workfront許可權的相關資訊，請參閱 [物件許可權共用概觀](../../workfront-basics/grant-and-request-access-to-objects/sharing-permissions-on-objects-overview.md)

1. （選用）按一下 **進階設定** s可透過選取每個層級來新增許可權，或透過取消選取每個層級來移除許可權。

   ![](assets/rp-share-filter-manage-advanced-settings-350x271.png)

1. 按一下&#x200B;**儲存**。

   篩選器會與您選取的實體共用，並出現在 **與我共用** 區域。
