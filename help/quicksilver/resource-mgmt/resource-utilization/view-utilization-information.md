  ...
  <tr> 
   ...
   <td> <p>檢視以下專案或更高存取權：</p> 
    <ul> 
     ...
     <li> <p>專案組合</p> </li> 
     <li> <p>計劃</p> </li> 
     <li> <p>包含檢視成本費率與檢視一般財務之存取權的財務資料（如果您要依成本檢視資訊）</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td>物件許可權</td> 
 ...
 <tbody> 
  <tr> 
   ...
   <td scope="col"> <p>包含的專案的總預算時數。 您可以檢視包含專案之整個生命週期的預算時數總計，或僅檢視指定日期範圍的預算時數總計（您可以指定個別周或月）。 </p> <p>預算時數是從業務案例的資源預算區域或資源規劃工具<em>.</em>中的可用資訊填入</p> <p>預算時數會出現在下列任何一列的「使用率」報表中：</p> 
    <ul> 
     <li> 預算時數是依工作角色及使用率報告中的個別使用者來摘要，如下所示： <br><strong>個別使用者： </strong>預算時數是每個使用者在使用率報告中的摘要。這些預算時數與在內含專案中指派給使用者的任務和問題相關聯。（您可以展開對應工作角色的列，以檢視具有該工作角色的使用者清單。）<br><strong>工作角色：</strong>預算時數在利用率報表中依工作角色摘要。<br>預算時數會因下列任一情況而出現在特定工作角色中：
     <ul>
...
<li><strong>工作角色</strong>：計畫時數是依工作角色在單一專案的使用率報告中摘要。<br>計畫時數會因下列任一情況而出現在特定工作角色中：  
<ul>
...
<li><strong>未分配時數</strong>：當計畫時數與任務或問題相關聯，並且沒有使用者或角色指派給任務或問題時，計畫時數會顯示在未分配時數區段的使用率報告中。僅當專案上有符合此說明的小時數以及檢視單一專案的「使用率」報告時，才會顯示此區段。<br>如需計畫時數的詳細資訊，請參閱<a href="../../manage-work/tasks/task-information/planned-hours.md">計畫時數總覽</a>。
  ...
  <tr> 
   ...
   <td> <p> 針對包含的專案，在任務、問題<span>和專案</span>上記錄的總時數。 您可以檢視所包含專案整個生命週期的實際小時總計，或僅檢視指定日期範圍內的實際小時總計（您可以指定個別周或月）。 </p> <p><strong>警告：</strong>使用率報告包含記錄到專案的時數、子系任務、問題和至少有一個指派的父系任務。 它不包括記錄到沒有指派的父系任務的小時數。 我們建議您不要將父系任務用作工作任務，並且只將子系任務指派給您的資源。 </p> <p>實際時數會顯示在任何下列資料列的使用率報表中：</p> 
    <ul> 
     <li> 實際時數是依工作角色及專案使用率報表中的個別使用者來彙總，如下所示：<br><strong>個別使用者：</strong>實際時數會顯示在記錄時數之使用者列的使用率報表中。（您可以展開對應工作角色的列，檢視該工作角色記錄時數的使用者清單。）<br><strong>工作角色：</strong>與這些角色相關聯的使用者記錄的實際時數，會摘要至對應工作角色列中的使用率報表。<br>實際時數會因下列任一情況而出現在特定工作角色中： 
      <ul> 
...
<li> <p><strong>受限使用者小時：</strong>任務會以使用者小時為單位每小時計費，但其上限金額由您指定。 Workfront會使用以下公式： </p> <p><code>具有上限計畫收入的使用者小時= [SUM（所有任務的使用者計畫時數） *使用者收費率] +任務的上限金額+ SUM（超過任務上限金額/計畫時數的金額）</code> </p> </li>
  ...
  <tr> 
   ...
   <p>與Workfront的其他區域不同，「使用率」報表會計算「實際收入」，方法是將「固定收入」平均除以任務上記錄的時數。 </p> <p>以下是一些案例： </p> </p>
   <ul> 
     <li> <p><strong>角色小時加固定：</strong>為特定角色設定的收費率，乘以該角色使用者針對工作記錄的小時數。 此外，在任務上指定的固定金額會新增至角色比率。 </p> <p>Workfront會使用以下公式：</p> <p><code>Role Hourly Plus Fixed Actual Revenue = [SUM(Actual Hours from role on all tasks) * Role Billing Rate] + SUM(Cap or Fixed Amount of the task / Actual Hours of the task)</code> </p> </li>
     <li> <p><strong>使用者小時加固定：</strong>為特定使用者設定的收費率，乘以該使用者針對工作記錄的時數。 此外，在任務上指定的固定金額會新增到使用者費率中。 </p> <p>Workfront會使用以下公式：</p> <p><code>User Hourly Plus Fixed Actual Revenue = [SUM(Actual Hours from role on all tasks) * User Billing Rate] + SUM(Cap or Fixed Amount of the task / User Hours of the task)</code> </p> </li> 
   </ul> <p><b>角色或使用者小時，上限</b> </p> <p><b>重要</b>
   <p>不像其他區域的Workfront，如果計畫收入超過上限，超過上限的金額會被視為固定收入。 「計畫收入」的計算方式為將「固定收入」平均除以任務上的「計畫時數」數目，然後加上上限金額與角色或使用者每小時收入。 <br></p> <p>以下是一些案例：</p></p>
    <ul> 
     <li> <p><strong>角色小時（上限：</strong>個任務）會以角色小時為單位每小時計費，但其上限金額是您可以指定的。 Workfront會使用以下公式：</p> <p><code>Role Hourly w/ Cap Actual Revenue = [SUM(Actual Hours from role on all tasks and issues) * Role Billing Rate] + Cap Amount of the task + SUM(Amount over the Cap Amount / Actual Hours of the task)</code></p> </li>
     <li> <p><strong>受限使用者小時：</strong>任務會以使用者小時為單位每小時計費，但其上限金額由您指定。</p> <p> Workfront會使用以下公式：</p> <p><code>User Hourly w/ Cap Actual Revenue = [SUM(Actual Hours from role on all tasks and issues) * User Billing Rate] + Cap Amount of the task + SUM(Amount over the Cap Amount / Actual Hours of the task)</code> </p> </li> 
    </ul>
     <p><strong>專案收入</strong>：與專案記錄時數關聯的收入計算時，會考量記錄時間之使用者主要工作角色的每小時計費金額。 我們不建議在此專案上記錄時間。 </p> 
     <p><b>注意</b>如果使用者未與工作角色相關聯，或如果主要角色的每小時計費為零，則Workfront會使用使用者的每小時計費金額來計算實際收入。 如果使用者設定檔中沒有每小時計費金額，則實際收入為零。 </p> 
   </td> 
  </tr> 
  <tr> 
...
1. 根據您檢視的是個別專案、多個專案、方案或投資組合的使用率資訊，執行下列任一作業：
   ...
   * 若要檢視單一專案的使用率資訊，請執行下列步驟：
      ...
      1. 前往您要檢視使用率資訊的專案，然後按一下左側面板中的&#x200B;**使用率**。
...
1. 指定下列資訊以建立或修改篩選器：
   ...
   * **投資組合：**&#x200B;開始輸入包含您要包含在「使用率」報表中之資訊的投資組合名稱，然後在名稱出現在下拉式清單中時按一下該名稱\
...
「使用率報表」僅包含您指定之角色的資訊。例如，任務包含10個實際小時。其中6個小時來自Designer角色，4個小時來自開發人員角色。如果您依Designer的角色篩選「使用率報表」 ，則報表會排除來自開發人員角色的四個小時。
   ...
   * **新增篩選規則：**&#x200B;按一下&#x200B;**新增篩選規則**，按一下文字欄位，然後開始輸入您要篩選的欄位名稱。 如果欄位可供使用，則會針對可與其關聯的每個物件填入欄位。 按一下欄位名稱以將其新增至篩選器。
     ...
     >[!IMPORTANT]
     >
     >您必須輸入欄位名稱，而不是欄位標籤。 欄位標籤會顯示在附加至物件的自訂表單上。 如需標籤與自訂欄位名稱之間差異的詳細資訊，請參閱[建立自訂表單](/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/form-designer/design-a-form/design-a-form.md)。
...
1. （選擇性）從&#x200B;**顯示**&#x200B;下拉式選單中選取適當的選項，設定圖表以顯示專案、程式或投資組合。
