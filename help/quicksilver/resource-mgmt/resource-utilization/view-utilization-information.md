...
如需有關存取及使用「使用率」報告的資訊，請參閱 [使用「使用率」報告追蹤進度、成本和收入](#track-progress-cost-and-revenue-with-the-utilization-report) 一節。
...
追蹤專案、方案或投資組合的進度時，針對任務和問題的進度都會包含在使用率報表中。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td scope="col"> <p>包含的專案的總預算時數。 您可以檢視包含專案之整個生命週期的預算時數總計，或僅檢視指定日期範圍的預算時數總計（您可以指定個別周或月）。 </p> <p>預算時數是從業務案例或資源規劃工具的新資源預算區域可用的資訊填入<em>.</em></p> <p>預算時數會出現在下列任何一列的「使用率」報表中：</p> 
    <ul> 
     <li> 預算時數在「使用率」報表中依職務角色與個別使用者進行彙總，如下所示：<br><strong>個別使用者：</strong> 預算時數在使用情況報告中彙總給每個使用者。 這些預算時數與在內含專案中指派給使用者的任務和問題相關聯。 （您可以展開對應工作角色的列，以檢視具有該工作角色的使用者清單。）<br><strong>工作角色：</strong> 已編列預算時數會依使用率報表中的工作角色進行彙總。<br>預算時數會因下列任一情況出現在特定工作角色中： 
    ...
    <ul> 
     <li> <p><strong>未分配時數</strong>：當預算時數與任務或問題相關聯，且沒有使用者或角色指派給任務或問題時，預算時數會顯示在未分配時數區段的使用率報告中。<br>僅當專案上存在符合此說明的小時數時，以及當按專案或從專案檢視「使用率」報告時，才會顯示此區段。 </p> <p>僅當專案上存在符合此說明的小時數時，以及當按專案或從專案檢視「使用率」報告時，才會顯示此區段。 </p> </li> 
    </ul> <p>如需有關預算時數的詳細資訊，請參閱 <a href="../../manage-work/projects/project-finances/budgeted-labor-cost.md" class="MCXref xref">瞭解專案的預算勞力成本和預算時數</a> 文章。</p> </td> 
  </tr> 
  <tr> 
...
<p>
如需有關修改使用者配置的資訊，請參閱 <a href="../workload-balancer/manage-user-allocations-workload-balancer.md">在工作負載平衡器中管理使用者配置</a>.
...
<li><strong>個別使用者</strong>：使用率報告中會摘要說明每個使用者的計畫時數。 這些計畫小時與使用者在包含的專案中受指派的任務和問題相關聯。 （您可以展開對應工作角色的列，以檢視具有該工作角色的使用者清單。）

<li><strong>工作角色</strong>：計畫時數依工作角色在單一專案的利用報告中進行摘要。<br>計畫時數會因下列任一情況而出現在特定工作角色中：  
<ul>
...
<li><strong>未分配時數</strong>：當計畫時數與任務或問題相關聯，且沒有使用者或角色指派給任務或問題時，計畫時數會顯示在未分配時數區段的使用率報告中。僅當專案上有符合此說明的時數時，以及檢視單一專案的使用率報告時，才會顯示此區段。 <br>如需計畫時數的詳細資訊，請參閱 <a href="../../manage-work/tasks/task-information/planned-hours.md">計畫時數概觀</a>.
</li> 
</ul>
</li> 
</ul> </td> 
  ...
  <tr> 
   ...
   <td> <p> 任務、問題和 <span>在專案上和</span> 針對包含的專案。 您可以檢視所包含專案整個生命週期的實際小時總計，或僅檢視指定日期範圍內的實際小時總計（您可以指定個別周或月）。 </p> <p>警告：使用率報表包含記錄至專案的時數、子系任務、問題以及至少具有一個指派的父系任務。 它不包括記錄到沒有指派的父系任務的小時數。 我們建議您不要將父系任務用作工作任務，並且只將子系任務指派給您的資源。 </p> <p>實際時數會顯示在任何下列資料列的使用率報表中：</p> 
    <ul> 
     <li> 實際時數在專案的使用率報表中依職務角色與個別使用者進行彙總，如下所示：<br><strong>個別使用者：</strong> 實際時數會顯示在使用率報告中記錄時數之使用者的列中的位置。 （您可以展開對應工作角色的列，以檢視具有該工作角色且已記錄時數的使用者清單。）<br><strong>工作角色：</strong> 與這些角色相關聯的使用者所記錄的實際時數，會摘要至對應工作角色列中的使用率報告。<br>由於下列任一情況，實際時數會出現在特定工作角色中： 
      <ul> 
     ...
     <li><strong>其他時數：</strong> 實際時數會顯示在其他時數區段的使用率報告中，該區段位於記錄時數之使用者的列中。<br>當記錄時數的使用者沒有在系統中定義工作角色時，時數會顯示在此區段中。<br>僅當專案上有符合此說明的小時時，才會顯示此區段。 </li> 
    </ul> </td> 
  </tr> 
  <tr> 
...
追蹤專案、方案或投資組合的成本時，「使用率」報表中的資訊來自任務。 作業的成本資訊一律可在「使用率」報表中取得。 根據任務的成本型別計算任務的成本。 如需有關作業成本型別的資訊，請參閱下列的「修改個別作業的成本型別」： [追蹤成本](../../manage-work/projects/project-finances/track-costs.md).
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td scope="col"> <p>包含的專案上的總計畫成本。 您可以檢視包含專案之整個生命週期的計畫成本總計，或僅檢視指定日期範圍的計畫成本總計（您可以指定個別周或月）。</p> <p>請注意，對於周、月和每季檢視，當工作角色或使用者的成本費率生效時，計畫成本會計算為所選期間的平均值。</p><p>如需有關如何計算專案計畫成本的資訊，請參閱本文中的「Workfront如何計算計畫、預算與實際成本」一節 <a href="../../manage-work/projects/project-finances/track-costs.md" class="MCXref xref">追蹤成本</a>.</p> </td> 
  </tr> 
  <tr> 
   <td scope="col"><strong>實際成本</strong> </td> 
   <td scope="col"> <p>包含專案的總實際成本。 您可以檢視包含專案之整個生命週期的實際成本總計，或僅檢視指定日期範圍的實際成本總計（您可以指定個別周或月）。</p> <p>如需有關如何計算專案實際成本的資訊，請參閱文章中的「Workfront如何計算計畫、預算與實際成本」一節 <a href="../../manage-work/projects/project-finances/track-costs.md" class="MCXref xref">追蹤成本</a>.</p> </td> 
  </tr> 
  <tr> 
...
如需特定欄位以及Workfront計算欄位方式的相關資訊，另請參閱下列文章：
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <p>「使用率」報表中顯示的專案計畫收入，與「專案詳細資訊」區域及專案報表中顯示的計畫收入不同。 </p> <p>專案詳細資訊區域中的計畫收入反映任務收入和專案的固定收入。 「使用率報表」中的「計畫收入」僅顯示與專案中任務相關的計畫收入。 </p> 
     ...
     </div> <p>如需「使用率」報表之外之任務與專案「計畫收入」的詳細資訊，請參閱 <a href="../../manage-work/projects/project-finances/billing-and-revenue-overview.md" class="MCXref xref">帳單與收入概要</a>.</p> </p> <p>「使用率」報表計算與顯示所包含專案之「計畫收入」的方式，會考量在作業上設定的「收入型態」。 </p> <p>根據專案中每個任務的「收入型態」，下列案例會存在： </p> <p><strong>固定收入：</strong> 無論任務指派為何，系統一律會使用任務上指定的「固定金額」來計算任務收入。</p> <p><b>重要</b>
...
如果您有「固定收入」作業，而且該作業沒有「計畫時數」，則「使用率報表」中不會顯示「收入」，因為無法將其分配至時數。 如果您有固定收入且沒有指派之任務的計畫時數，則收入會顯示為「未配置收入」。 </p> <p><strong>角色小時：</strong> 任務的收入是使用為特定角色設定的收費率，乘以該角色關聯的計畫時數來計算。 Workfront會使用以下公式：</p> <p><code>角色每小時計畫收入= SUM（所有任務上角色的計畫時數） *角色收費率</code></p><p><b>注意：</b> 公式中的帳單小時費率會考量費率的任何日期有效變更。</p>   <p><strong>使用者小時：</strong> 任務的收入是使用為特定使用者設定的收費率，乘以與該使用者關聯的計畫時數來計算。 Workfront會使用以下公式：</p> <p><code>使用者小時計畫收入= SUM（來自所有任務中使用者的計畫時數） *使用者收費率</code> </p> <p><b>注意：</b> 公式中的帳單小時費率會考量費率的任何日期有效變更。</p> <p><b>角色小時或使用者小時加固定</b> </p> <p><b>重要</b>

與Workfront的其他區域不同，「使用率」報表會計算「計畫收入」，方法是將「固定收入」平均除以任務上的「計畫時數」數目。 </p> <p>以下是一些案例： </p>
<ul>
<li> <p><strong>角色小時加固定：</strong> 任務的收入是使用為特定角色設定的收費率，乘以與角色關聯的計畫時數來計算。 此外，在任務上指定的固定金額會新增至角色比率。 Workfront會使用以下公式：</p> <p><code>角色小時加固定計畫收入= [SUM（所有任務上角色的計畫時數） *角色收費率] + SUM（任務的上限或固定數量/任務的計畫時數）</code> </p> </li>
</ul>
<ul>
<li> <p><strong>使用者小時加固定：</strong> 為特定使用者設定的收費率，乘以該使用者在該任務上的計畫時數。 此外，在任務上指定的固定金額會新增到使用者費率中。 Workfront會使用以下公式：</p> <p><code>使用者小時加固定計畫收入= [SUM（使用者在所有任務上的計畫時數） *使用者收費率] + SUM（任務的上限或固定數量/任務的計畫時數）</code> </p> </li>
...
<ul>
<li> <p><strong>受限角色小時：</strong> 任務會以每小時角色的方式計費，但上限金額是您可以指定的上限。 Workfront會使用以下公式：</p> <p><code>角色小時與計畫收入上限= [SUM（所有任務和問題的角色計畫時數） *角色收費率] +任務的上限金額+ SUM（超過任務的上限金額/計畫時數的金額）</code> </p> </li>
</ul>
<ul>
<li> <p><strong>受限使用者小時：</strong> 任務會以每小時的使用者小時計費，但具有您可指定的上限金額。 Workfront會使用以下公式： </p> <p><code>計畫收入上限的使用者小時= [SUM（使用者在所有任務上的計畫時數） *使用者收費率] +任務的上限金額+ SUM（超過任務的上限金額/計畫時數的金額）</code> </p> </li>
</ul> <p>有關計算計畫收入時考慮哪些角色或使用者的詳細資訊，請參閱 <a href="../../manage-work/projects/project-finances/billing-and-revenue-overview.md" class="MCXref xref">帳單與收入概要</a>.</p> </td>
</tr> 
  <tr> 
   <td><strong>實際收入</strong> </td> 
   <td> <p>實際收入是與任務實際時數關聯的收入 <span>和專案</span>. 如需實際收入的詳細資訊，請參閱文章中的「追蹤收入金額」一節 <a href="../../manage-work/projects/project-finances/billing-and-revenue-overview.md" class="MCXref xref">帳單與收入概要</a>.</p>

<p>「使用率」報表計算所包含專案之「實際收入」的方式，會根據作業上設定的「收入型態」而有所不同，如下所示：</p> <p><strong>固定收入：</strong> 無論任務指派為何，系統一律會使用任務上指定的「固定金額」來計算任務收入。</p> <p><b>重要</b>
...
如果您有「固定收入」作業，且作業上沒有「實際時數」，則「使用率報表」中不會顯示「實際收入」，因為無法分配時數。 </p> <p><strong>角色小時：</strong> 任務的收入是使用特定角色所設定的收費率，乘以實際時數來計算。</p> <p>Workfront會使用以下公式：</p> <p><code>角色每小時實際收入= SUM（所有任務上角色的實際時數） *角色記帳費率</code> </p> <p><b>注意：</b> 公式中的帳單小時費率會考量費率的任何日期有效變更。</p> <p><strong>使用者小時：</strong> 任務的收入是使用為特定使用者設定的收費率，乘以針對該使用者所記錄之任務的小時數來計算。 Workfront會使用以下公式：</p> <p><code>使用者小時實際收入= SUM（使用者在所有任務上的實際時數） *使用者收費率</code></p> <p><b>注意：</b> 公式中的帳單小時費率會考量費率的任何日期有效變更。</p> <p><b>角色或使用者小時加固定</b> </p> <p><b>重要</b>

與Workfront的其他區域不同，「使用率」報表會計算「實際收入」，方法是將「固定收入」平均除以任務上記錄的時數。 </p> <p>以下是一些案例： </p>
<ul>
<li> <p><strong>角色小時加固定：</strong> 為特定角色設定的收費率，乘以來自該角色之使用者的任務記錄時數。 此外，在任務上指定的固定金額會新增至角色比率。 </p> <p>Workfront會使用以下公式：</p> <p><code>角色小時加固定實際收入= [SUM（所有任務上角色的實際時數） *角色收費率] + SUM（任務的上限或固定數量/任務的實際時數）</code> </p> </li>
</ul>
<ul>
<li> <p><strong>使用者小時加固定：</strong> 為特定使用者設定的收費率，乘以該使用者針對該任務記錄的小時數。 此外，在任務上指定的固定金額會新增到使用者費率中。 </p> <p>Workfront會使用以下公式：</p> <p><code>使用者小時加固定實際收入= [SUM（所有任務上角色的實際時數） *使用者收費率] + SUM（任務的上限或固定數量/任務的使用者時數）</code> </p> </li>
</ul> <p><b>受限角色或使用者小時</b> </p> <p><b>重要</b>

不像其他區域的Workfront，如果計畫收入超過上限，超過上限的金額會被視為固定收入。 「計畫收入」的計算方式為將「固定收入」平均除以任務上的「計畫時數」數目，然後加上上限金額與角色或使用者每小時收入。 <br></p> <p>存在下列情況：</p>
<ul>
<li> <p><strong>受限角色小時：</strong> 任務會以每小時角色的方式計費，但上限金額是您可以指定的上限。 Workfront會使用以下公式：</p> <p><code>角色小時，具有實際收入上限= [SUM（所有任務和問題的角色實際時數） *角色收費率] +任務的上限金額+ SUM（超過任務的上限金額/實際時數的金額）</code></p> </li>
</ul>
<ul>
<li> <p><strong>受限使用者小時：</strong> 任務會以每小時的使用者小時計費，但具有您可指定的上限金額。</p> <p> Workfront會使用以下公式：</p> <p><code>具有實際收入上限的使用者小時= [SUM（所有任務和問題的角色實際時數） *使用者收費率] +任務的上限金額+ SUM（超過任務的上限金額/實際時數的金額）</code> </p> </li>
</ul>
<div>
<p><strong>專案收入</strong>：計算與專案記錄時數相關聯的收入，會考慮記錄時間之使用者的主要工作角色的每小時計費金額。 我們不建議在此專案上記錄時間。 </p>
<p><b>附註</b>
...
如果使用者未與工作角色相關聯，或如果主要角色的每小時計費為零，則Workfront會使用使用者的每小時計費金額來計算實際收入。 如果使用者設定檔中沒有每小時計費金額，則實際收入為零。 </p>
  ...
  <tr> 
   <td><strong>預算差額（適用於收入）</strong> </td> 
   <td> <p>包含專案的總實際收入減去預算收入。<br>您可以檢視所包含專案整體生命週期的預算差異總計，或者僅檢視指定日期範圍的預算差異總計（您可以指定個別周或月）。</p> <p>如果值為正數，則會以綠色顯示。 這表示總預算收入大於實際收入。</p> <p>如果值為負數，則會以紅色顯示。 這表示總預算收入小於實際收入。</p> </td> 
  </tr> 
  <tr> 
   <td><strong>計畫差額（適用於收入）</strong> </td> 
   <td> <p>實際收入總計減去包含專案中的計畫收入總計。<br>您可以檢視所包含專案整個生命週期的計畫差異總計，或僅檢視指定日期範圍的計畫差異總計（您可以指定個別周或月）。 </p> <p>如果值為正數，則會以綠色顯示。 這表示總計畫收入大於實際收入。</p> <p>如果值為負數，則會以紅色顯示。 這表示總計畫收入小於實際收入。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td scope="col"><strong>計畫收入</strong> </td> 
   <td scope="col"> <p>計畫收入是與任務的計畫時數相關聯的收入。 </p> <p>「使用率」報表計算與顯示內含專案之「計畫收入」的方式，會根據作業上設定的「收入型態」而有所不同，如 <a href="#track-revenue" class="MCXref xref">追蹤收入</a> 一節。</p> </td> 
  </tr> 
  <tr> 
...
如果「計畫收入」等於0，則「利潤」會顯示為0。 </p> </td>
  ...
  <tr> 
   ...
   <td> <p>實際收入是與任務實際時數關聯的收入。</p> <p>僅當任務標籤為「完成」或「完成」（或等同於「完成」的狀態）後，「實際收入」才會顯示在「使用率」報表中。</p> <p>「使用率」報表計算所包含專案之實際收入的方式會根據作業上設定的「收入型態」而有所不同，如以下所述 <a href="#track-revenue" class="MCXref xref">追蹤收入</a> 一節。 </p> </td> 
  </tr> 
  <tr> 
   <td scope="col"><strong>邊界</strong> </td> 
...
1. 根據您檢視的是個別專案、多個專案、方案或投資組合的使用率資訊，執行下列任一作業：
   ...
   * 若要檢視單一專案的使用率資訊，請執行下列步驟：
      ...
      1. 移至您要檢視使用率資訊的專案，然後按一下 **顯示更多>使用率**.
   ...
   * 若要檢視多重專案的使用率資訊，請執行下列步驟：
      ...
      1. 按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。
      1. 將篩選器套用至使用率報表，然後按一下 **執行**.\
   ...
   * 若要檢視方案的使用率資訊，請執行下列步驟：
      ...
      1. 按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中按一下 **顯示**>**計畫**.
      1. 將篩選器套用至使用率報表，然後按一下 **執行**.\
   ...
   * 若要檢視投資組合的使用率資訊，請執行下列步驟：
      ...
      1. 按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中並按一下 **顯示**>**Portfolio**.
      1. 將篩選器套用至使用率報表，然後按一下 **執行**.\
...
1. 在「使用率」報告的右上角，按一下 **檢視** 下拉式功能表，然後選取下列專案：
   ...
   * **收入vs成本（實際）**.
   ...
   您選取的選項決定報告中可用的欄和資訊。 如需每一欄可用資訊的詳細資訊，請參閱步驟5中的表格。\
   ![](assets/utilization-view-dropdown.png)

1. （選擇性）選取顯示使用率資訊的日期範圍。 您可以在左側顯示指定周或月的資訊 **整體** 欄。 整體專案、方案或投資組合的資訊一律顯示在 **整體** 欄。\
...
1. 使用區段中的資訊 [使用率報表概要](#overview-of-the-utilization-report) 本文章以瞭解使用情況報告中的每一欄。

## 篩選器使用率資訊 {#filter-utilization-information}

您可以篩選在專案使用率報告中顯示的內容。 您可以按任務、問題、角色和自訂資料進行篩選。 當您套用篩選條件至「使用率」報表時，「使用率」報表會包含根據您選取之條件的資訊。
...
1. 若要篩選單一專案的使用率資訊，請移至您要篩選使用率資訊的專案，然後按一下 **顯示更多>使用率** 在左側面板中。

   或

   若要篩選多個專案、方案或投資組合的使用率資訊，請按一下主要功能表圖示 ![](assets/main-menu-icon.png) 在Workfront的右上角， **資源**， **使用率** 在左側面板中，然後 **顯示**>**計畫** 或 **Portfolio** 或&#x200B;**專案**.

1. 按一下 **篩選** 圖示來顯示篩選選項。

1. （視條件而定）若要修改現有篩選，請按一下 **篩選** 下拉式功能表，然後選取您要修改的篩選器。
1. 指定下列資訊以建立或修改篩選器：

   * **Portfolio：** 開始輸入包含您要納入使用率報告之資訊的投資組合名稱，然後按一下出現在下拉式功能表中的名稱。\
     重複此程式，將多個產品組合的資訊加入使用率報告中。\
     若要將您系統中的所有產品組合納入您的篩選器，請按一下 **全部新增**. （只有在系統中擁有少於10個投資組合時，才可使用此選項。）

   * **程式：** 開始輸入程式名稱，其中包含您要納入使用率報告的資訊，然後在下拉式功能表中出現時按一下該名稱。\
     重複此程式，在「使用率」報表中包含多個工作的資訊。\
     如果您已在篩選器中指定任何投資組合，則您指定的計畫必須來自已包含在篩選中的投資組合。 如果沒有，則程式的資料不包括在使用率報告中。\
     若要將您系統中的所有程式都包含在您的篩選器中，請按一下 **全部新增**. （只有在系統中擁有少於20個程式時，才能使用此選項。）

   * **專案：** 開始輸入專案名稱，該專案包含您要納入使用率報告的資訊，然後在下拉式功能表中出現時按一下該名稱。\
     重複此程式，將多個專案的資訊納入使用率報告中。\
     如果您已在篩選器中指定任何投資組合或計畫，則您指定的專案必須來自已包含在篩選中的投資組合或計畫之一。 如果沒有，則來自專案的資料不會包含在使用率報表中。\
     若要將您系統中的所有專案包含在您的篩選器中，請按一下 **全部新增**. （只有在系統中擁有少於250個專案時，才可使用此選項。）

   * **工作：** 開始輸入工作名稱，其中包含您要納入使用率報告的資訊，然後在下拉式功能表中出現時按一下該名稱。\
     重複此程式，在「使用率」報表中包含多個工作的資訊。\
     如果您已在篩選器中指定任何投資組合、計畫或專案，則您指定的任務必須來自已包含在篩選中的投資組合、計畫或專案之一。 如果沒有，則來自「 」任務的資料不會包含在「 」使用情況報表中。

   * **問題：** 開始輸入問題名稱，該問題包含您要納入使用情況報告的資訊，然後在下拉式選單中出現時按一下名稱。\
     重複此程式，以在使用率報告中包含多個問題的資訊。\
     如果您已在篩選器中指定任何投資組合、計畫或專案，則您指定的問題必須來自已包含在篩選中的投資組合、計畫或專案之一。 如果沒有，問題的資料不會包含在使用率報告中。\
     問題的成本資訊並不一定包含在「使用率」報告中。 如需問題的成本資訊何時包含在「使用率」報表中的詳細資訊，請參閱區段 [使用「使用率」報告追蹤進度、成本和收入](#track-progress-cost-and-revenue-with-the-utilization-report) 本文章內容。

   * **角色：** 開始輸入您要在「使用率報告」中顯示的角色名稱，然後按一下該名稱（當它出現在下拉式功能表中時）。 重複此過程以包含其他角色。\
     「使用率報表」僅包含您指定之角色的資訊。 例如，任務包含10個實際小時。 其中6個小時來自設計師角色，4個小時來自開發人員角色。 如果您依「設計工具」的角色來篩選「使用率報表」，則報表會排除來自「開發人員」角色的4小時。

   * **新增篩選器規則：** 按一下 **新增篩選器規則**，按一下第一個欄位，然後開始輸入您要篩選的欄位名稱。 如果欄位可供使用，則會針對可與其關聯的每個物件填入欄位。 按一下欄位名稱以將其新增至篩選器。

     >[!IMPORTANT]
     >
     >您必須輸入欄位名稱，而不是欄位標籤。 欄位標籤會顯示在附加至物件的自訂表單上。 如需有關標籤與自訂欄位名稱之間差異的資訊，請參閱  [建立或編輯自訂表單](../../administration-and-setup/customize-workfront/create-manage-custom-forms/create-or-edit-a-custom-form.md).
     ...
     選擇篩選的篩選和條件修飾詞。 有關可用修飾元的說明，請參見 [篩選和條件修飾元](../../reports-and-dashboards/reports/reporting-elements/filter-condition-modifiers.md).

1. 若要建立新篩選器，請按一下 **儲存篩選器**.\
   或\
   若要修改現有篩選器，請按一下 **儲存篩選器** 按鈕，然後按一下 **儲存新的篩選器**.\
...
1. 若要在單一專案的「使用情況報表」上套用篩選，請前往您要篩選的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
   ...
   若要針對多個專案、方案或投資組合，在使用情況報表上套用篩選，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) 在Workfront的右上角， **資源**， **使用率** 在左側面板中，然後 **顯示**>**計畫** 或&#x200B;**Portfolio** 或&#x200B;**專案**.
...
1. 若要在單一專案的「使用情況報表」上複製篩選器，請前往您要複製篩選器的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
   ...
   若要針對多個專案、方案或投資組合，在利用報告上複製篩選器，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。
...
1. 在 **篩選器名稱** 欄位，指定新篩選的名稱，然後按一下 **儲存**.
...
1. 若要重新命名單一專案之使用報告上的篩選器，請前往您要重新命名篩選器的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
   ...
   若要重新命名多個專案、方案或投資組合之使用報告上的篩選器，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。
...
1. 若要刪除單一專案之使用報告上的篩選器，請前往您要刪除篩選器的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
   ...
   若要刪除多個專案、方案或投資組合之使用報告上的篩選器，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。
...
1. 若要調整單一專案之「使用率報表」的日期範圍，請移至您要調整日期範圍的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
   ...
   若要調整多個專案、方案或投資組合之使用率報表的日期範圍，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。
...
1. 從下列選項中選取：
   ...
   * **月：** 選取此選項可選取指定月份。
   ...
   Workfront會記住您是要檢視周或月檢視。 下次存取使用率報告時，將會根據您選取的選項顯示當週或當月。
...
1. 若要匯出單一專案的使用率資訊，請移至您要匯出其使用率資訊的專案，然後按一下 **使用率** 標籤(視您的版面設定而定，這可能位於 **更多** 標籤)。

   或

   若要匯出多個專案、方案或投資組合的使用率資訊，請按一下 **主要功能表** 圖示 ![](assets/main-menu-icon.png) (位於Workfront的右上角)，然後按一下 **資源**，然後 **使用率** 在左側面板中。

1. 按一下 **匯出**，位於 **使用率** 標籤。

1. 從下列選項中選取：

   * **PDF：** 以PDF格式匯出報表。 如果您計畫列印報表，則建議使用此格式。\
     選取 **字母 — 縱向**， **字母 — 橫向**，或 **其他大小** (提供在Legal (8.5&quot; x 14&quot;)、Ledger (11&quot; x 17&quot;)及A4中匯出的選項)。\
     視您使用的作業系統而定，您可以選擇開啟或儲存檔案。 使用關聯的應用程式開啟檔案，或將其儲存到硬碟。

   * **Excel：** 以XLSX格式匯出報表。 如果您打算進一步分析Excel中的資料，則建議使用此格式。\
     視您使用的作業系統而定，您可以選擇開啟或儲存檔案。 使用關聯的應用程式開啟檔案，或將其儲存到硬碟。

   * **定位字元分隔：** 以TSV格式匯出報告。 如果您打算將資料匯入協力廠商軟體以進行進一步分析，建議使用此格式。\
     視您使用的作業系統而定，您可以選擇開啟或儲存檔案。 使用關聯的應用程式開啟檔案，或將其儲存到硬碟。
...
1. 若要以圖表格式檢視單一專案的使用率報告，請移至您要檢視的專案，然後按一下 **顯示更多>使用率** 在左側面板中。
...
1. （可選）透過選取以下專案中的適當選項，設定此項以顯示專案、方案或Portfolio： **顯示** 下拉式功能表。
1. （可選）將滑鼠移至報告上的特定時間點上，即可檢視該時間點的資料。

   ![](assets/utilization-chart-hover-350x176.png)

1. （選擇性）調整篩選器以決定要在圖表中顯示哪些資訊。 如需調整篩選器的相關資訊，請參閱區段 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。
1. （可選）依照一節中的說明，設定圖表報表的時間範圍 [調整顯示資訊的日期範圍](#adjust-the-date-range-for-which-information-is-displayed) 本文章內容。
