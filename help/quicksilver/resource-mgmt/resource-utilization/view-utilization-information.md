...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td scope="col"><strong>已編列預算時數</strong> </td> 
   <td scope="col"> <p>包含的專案的總預算時數。 您可以檢視包含專案之整個生命週期的預算時數總計，或僅檢視指定日期範圍的預算時數總計（您可以指定個別周或月）。 </p> <p>預算時數會從業務案例或資源規劃工具資源預算區域中的可用資訊植入<em>.</em></p> <p>預算時數會出現在下列任何一列的「使用率」報表中：</p> 
    <ul> 
...
<li><strong>未分配時數</strong>：當計畫時數與任務或問題相關聯，並且沒有使用者或角色指派給任務或問題時，計畫時數會顯示在未分配時數區段的利用報告中。 僅當專案上有符合此說明的小時數以及檢視單一專案的「使用率」報告時，才會顯示此區段。 <br>如需計畫時數的詳細資訊，請參閱 <a href="../../manage-work/tasks/task-information/planned-hours.md">計畫時數概觀</a>.
</li> 
</ul>
</li> 
</ul> </td> 
  ...
  <tr> 
   <td><strong>實際小時</strong> </td> 
   <td> <p> 任務、問題和 <span>在專案上和</span> 針對包含的專案。 您可以檢視所包含專案整個生命週期的實際小時總計，或僅檢視指定日期範圍內的實際小時總計（您可以指定個別周或月）。 </p> <p><strong>警告：</strong> 使用情況報告包含記錄到專案的時數、子系任務、問題以及至少有一個指派的父系任務。 它不包括記錄到沒有指派的父系任務的小時數。 我們建議您不要將父系任務用作工作任務，並且只將子系任務指派給您的資源。 </p> <p>實際時數會顯示在任何下列資料列的使用率報表中：</p> 
    <ul> 
...
追蹤專案、方案或投資組合的成本時，「使用率」報表中的資訊來自任務。 作業的成本資訊一律可在「使用率」報表中取得。 根據任務的成本型別計算任務的成本。 如需有關作業成本型別的資訊，請參閱 [Workfront如何計算任務的成本型別](/help/quicksilver/manage-work/projects/project-finances/track-costs.md#how-workfront-calculates-cost-types-for-tasks) 在 [追蹤成本](/help/quicksilver/manage-work/projects/project-finances/track-costs.md).
...
如需特定欄位以及Workfront如何計算欄位的資訊，另請參閱 [追蹤成本](../../manage-work/projects/project-finances/track-costs.md) 和 [帳單與收入概要](../../manage-work/projects/project-finances/billing-and-revenue-overview.md).
...
與Workfront的其他區域不同，「使用率」報表會計算固定收入任務的計畫收入，方法是將固定收入平均除以任務上的計畫時數。 </p> <p>例如，任務的收入為$200。 如果任務有4個計畫時數，則每小時將為$50。 這會分散到使用者和角色層級。 此分配在「使用率」報表中是唯一的。</p> <p><b>注意</b>
...
<li> <p><strong>受限使用者小時：</strong> 任務會以每小時的使用者小時計費，但具有您可指定的上限金額。 Workfront會使用以下公式： </p> <p><code>計畫收入上限的使用者小時= [SUM（使用者在所有任務上的計畫時數） *使用者收費率] +任務的上限金額+ SUM（超過任務的上限金額/計畫時數的金額）</code> </p> </li>
  ...
  <tr> 
   ...
   <td> <p>實際收入是與任務及專案之實際時數關聯的收入。 如需實際收入的詳細資訊，請參閱 <a href="/help/quicksilver/manage-work/projects/project-finances/billing-and-revenue-overview.md#track-revenue-amounts">追蹤收入金額</a> 在 <a href="/help/quicksilver/manage-work/projects/project-finances/billing-and-revenue-overview.md">帳單與收入概要</a>.</p>
...
與Workfront的其他區域不同，「使用率」報表會計算「實際收入」，方法是將「固定收入」平均除以任務上記錄的時數。 </p> <p> </p> <p>例如，任務的實際收入為$200。 如果任務有4個實際時數，則每小時將為$50。 這會分散到使用者和角色層級。 此分配在「使用率」報表中是唯一的。</p> <p><b>注意</b>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td scope="col"><strong>邊距</strong> </td> 
...
1. 根據您檢視的是個別專案、多個專案、方案或投資組合的使用率資訊，執行下列任一作業：
   ...
   * 若要檢視單一專案的使用率資訊，請執行下列步驟：
      ...
      1. 檢視個別專案時，利用資訊會自動顯示，不需要套用篩選器。\
         ...
         如需有關如何篩選「使用率」報表的資訊，請參閱 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。\
...
執行「使用率」報告之前，您必須在篩選器中指定一或多個專案。 如需有關如何篩選「使用率」報表的資訊，請參閱 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。\
   ...
   * 若要檢視方案的使用率資訊，請執行下列步驟：
     ...
     {{step1-to-utilization-report}}
      ...
      1. 按一下 **顯示**>**計畫**.
      1. 將篩選器套用至使用率報表，然後按一下 **執行**.\
         執行「使用率」報告之前，您必須在篩選器中指定一或多個程式。 如需有關如何篩選「使用率」報表的資訊，請參閱 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。\
   ...
   * 若要檢視投資組合的使用率資訊，請執行下列步驟：
     ...
     {{step1-to-utilization-report}}
      ...
      1. 按一下 **顯示**>**Portfolio**.
      1. 將篩選器套用至使用率報表，然後按一下 **執行**.\
         執行「使用率」報告之前，您必須在篩選器中指定一或多個投資組合。 如需有關如何篩選「使用率」報表的資訊，請參閱 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。\
...
1. （選擇性）選取顯示使用率資訊的日期範圍。 您可以在左側顯示指定周或月的資訊 **整體** 欄。 整體專案、方案或投資組合的資訊一律顯示在 **整體** 欄。\
   如需詳細資訊，請參閱 [調整顯示資訊的日期範圍](#adjust-the-date-range-for-which-information-is-displayed) 本文章內容。
...
1. 開啟「使用率」報表。
另請參閱 [使用「使用率」報告追蹤進度、成本和收入](#track-progress-cost-and-revenue-with-the-utilization-report) 以執行此操作。

1. 按一下 **篩選** 圖示來顯示篩選選項。
1. （視條件而定）若要修改現有篩選，請按一下 **篩選** 下拉式功能表，然後選取您要修改的篩選器。
1. 指定下列資訊以建立或修改篩選器：

   * **Portfolio：** 開始輸入包含您要納入使用率報告之資訊的投資組合名稱，然後在名稱出現在下拉式清單中時按一下該名稱\
     重複此程式，將多個產品組合的資訊加入使用率報告中。\
     若要將您系統中的所有產品組合納入您的篩選器，請按一下 **全部新增**. （只有在系統中擁有少於10個投資組合時，才可使用此選項。）

   * **程式：** 開始輸入程式名稱，該程式包含您要納入使用率報告的資訊，然後按一下該名稱（當它出現在下拉式清單中時）。\
   ...
   * **專案：** 開始輸入專案名稱，該專案包含您要納入使用率報告的資訊，然後按一下該名稱（當它出現在下拉式清單中時）。\
   ...
   * **工作：** 開始輸入工作名稱，其中包含您要納入使用率報告的資訊，然後在名稱出現在下拉式清單中時按一下該名稱。\
     重複此程式，在「使用率」報表中包含多個工作的資訊。\
     如果您已在篩選器中指定任何投資組合、計畫或專案，則您指定的任務必須來自已包含在篩選中的投資組合、計畫或專案之一。 如果沒有，則來自「 」任務的資料不會包含在「 」使用情況報表中。

   * **問題：** 開始輸入問題名稱，該問題包含您要納入使用情況報告的資訊，然後按一下該名稱（當它出現在下拉式清單中時）。\
     重複此程式，以在使用率報告中包含多個問題的資訊。\
     如果您已在篩選器中指定任何投資組合、計畫或專案，則您指定的問題必須來自已包含在篩選中的投資組合、計畫或專案之一。 如果沒有，問題的資料不會包含在使用率報告中。\
     問題的成本資訊並不一定包含在「使用率」報告中。 有關問題的成本資訊何時包含在「使用率」報表中的詳細資訊，請參閱 [使用「使用率」報告追蹤進度、成本和收入](#track-progress-cost-and-revenue-with-the-utilization-report) 本文章內容。

   * **角色：** 開始輸入您要在「使用率報告」中顯示的角色名稱，然後按一下該名稱（當它出現在下拉式清單中時）。 重複此過程以包含其他角色。
「使用率報表」僅包含您指定之角色的資訊。 例如，任務包含10個實際小時。 其中6個小時來自設計師角色，4個小時來自開發人員角色。 如果您依「設計工具」的角色來篩選「使用率報表」，則報表會排除來自「開發人員」角色的四個小時。

   * **新增篩選器規則：** 按一下 **新增篩選器規則**，按一下文字欄位，然後開始輸入您要篩選的欄位名稱。 如果欄位可供使用，則會針對可與其關聯的每個物件填入欄位。 按一下欄位名稱以將其新增至篩選器。
...
1. 按一下 **儲存的篩選器**，將游標停留在您要複製的篩選器上，然後按一下 **複製** 圖示。

   ![「複製」圖示](assets/utilization-filter-duplicate.png)

   隨即顯示「複製篩選器」對話方塊。

1. 在 **篩選器名稱** 欄位，輸入新篩選器的名稱，然後按一下 **儲存**.
...
1. 按一下 **儲存的篩選器**，將游標停留在您要重新命名的篩選器上，然後按一下 **重新命名** 圖示。
...
1. 按一下 **儲存的篩選器**，將游標停留在您要刪除的篩選器上，然後按一下 **刪除** 圖示。

   ![「刪除」圖示](assets/utilization-filter-delete.png)
...
1. （可選）透過選取以下專案中的適當選項，設定圖表以顯示專案、方案或Portfolio： **顯示** 下拉式功能表。
1. （可選）將滑鼠移至報表上的特定時間點上，即可檢視該時間點的資料。

   ![暫留在資料點上](assets/utilization-chart-hover.png)

1. （選擇性）調整篩選條件以變更在圖表中顯示的資訊。 如需有關調整篩選的資訊，請參閱 [篩選器使用率資訊](#filter-utilization-information) 本文章內容。
1. （可選）設定圖表報表的時間範圍，如所述 [調整顯示資訊的日期範圍](#adjust-the-date-range-for-which-information-is-displayed) 本文章內容。
