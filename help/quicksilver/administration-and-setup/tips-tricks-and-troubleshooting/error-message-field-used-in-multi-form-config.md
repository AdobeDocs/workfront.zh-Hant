...
>[!NOTE]
>
>如果您看到錯誤但找不到多個包含自訂欄位的物件，請檢查「資源回收筒」。 資源回收筒中的物件可能包含欄位。 因為可隨時還原資源回收筒中的物件，所以在衝突解決之前，將會持續發生多形式衝突錯誤。
>
>若要新增具有不同運算式的欄位，您必須執行下列其中一項作業來解決此衝突：
>
>* 從衝突的自訂表單中移除欄位
>* 還原已刪除的物件、分離其中一個衝突的自訂表單，然後視需要再次刪除物件。
