...
* **使用者將使用Adobe身分識別登入。** Adobe將現有使用者移轉至Adobe Admin Console後，使用者將使用其新的Adobe身分識別(Adobe ID或Adobe Federated ID (SSO))登入其Workfront執行個體。
...
* **Adobe將會先自動移轉Workfront管理員。** Workfront管理員移轉至Adobe Identity時，會獲派Adobe產品設定檔管理員（Workfront系統管理員）角色。 使用者在移轉前可能擁有的現有角色將不會受到影響。
...
* **當使用者移轉時，他們會收到Adobe的電子郵件，通知他們已變更Workfront的登入方式。** 使用者將獲邀接受邀請，以使用Adobe身分識別首次登入，方法是使用現有的Adobe ID登入，或使用相同的電子郵件地址設定新的Adobe ID。
