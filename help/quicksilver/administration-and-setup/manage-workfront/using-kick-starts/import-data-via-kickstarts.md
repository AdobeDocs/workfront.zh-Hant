...
description: Kick-Start是特別格式化的Excel活頁簿，可填入您要匯入Workfront的資料。 Adobe Workfront提供您可用來執行此動作的Kick-Start範本，如Kick-Starts資料匯入工具中所述。
author: Caroline
...
Kick-Start是特別格式化的Excel活頁簿，可填入您要匯入Workfront的資料。 Adobe Workfront提供您可用來執行此動作的快速啟動範本，如中所述 [Kick-Starts資料匯入工具](../../../administration-and-setup/manage-workfront/using-kick-starts/kick-starts-data-importer.md).
...
您可以使用Kick-Start範本將大量物件匯入Workfront。 但是，請考量下列限制：
...
1. 按一下 **系統** > **匯出資料(Kick-Starts)**.

1. 按一下 **更多選項**，然後選取您要包含的資訊型別。

   您選取的每個選項都代表匯出試算表中多個索引標籤的集合。 例如，如果您選取「報表」選項，建立報表所需的所有物件都會包含在試算表中（檢視、篩選器、群組、報表）。

   您可以使用下列所有物件型別，將資料匯入Workfront。 (唯一的例外是「存取層級」選項。 匯出中的「存取層級」資料表僅供參考之用，可讓您依ID指派存取層級給新的使用者帳戶。)
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      <td scope="col"> <p>儀表板</p> <p>系統中所有儀表板都可匯出。 在單一匯出中，您最多可以選取100個特定儀表板。</p> </td> 
      <td scope="col">匯出為ZIP檔案</td> 
      <td scope="col"> <p>參數</p> <p>參數選項</p> <p>參數群組</p> <p>類別參數</p> <p>類別</p> <p>報告</p> <p>入口網站頁籤區段</p> <p>儀表板</p> <p>偏好設定</p> </td> 
     </tr> 
     <tr> 
      <td scope="col"> <p>報告</p> <p>系統中的所有報告都可匯出。 在單一匯出中，您最多可以選取100個特定報表。</p> </td> 
      <td scope="col">匯出為ZIP檔案 </td> 
...
>[!TIP]
>
>若要更清楚瞭解當您填入Kick-Start範本時如何需要格式化每個欄中的資訊，請考慮執行一個練習，使用您嘗試匯入的物件上的現有Workfront資料匯出Kick-Start。 如需指示，請參閱 [透過Kick-Starts從Adobe Workfront匯出資料](../../../administration-and-setup/manage-workfront/using-kick-starts/export-data-from-wf-via-kick-starts.md).

當您開啟空白的Kick-Starts範本時，會有許多標籤（資料表）可供使用。 視您選取要下載的物件而定。 每一個都代表應用程式中的物件，例如專案、任務、小時、控制面板和使用者：

當您開啟其中一個標籤時，第2列會顯示可在匯入期間設定的每個物件的欄位。 在欄標題中，在「set」一詞之後，欄位名稱會依顯示在資料庫中的方式顯示。 這些欄位會作為欄標題。
...
>
>  例如，在 **專案專案** 標籤， **setCondition** 和 **setConditionType** 欄位可以留空，但 **setGroupID** 和 **setName** 欄不能。
>
...
1. 完成中的儲存格 **isNew** 欄：
   ...
   * 如果物件已存在於Workfront中，請鍵入 **FALSE** 以忽略該列。

1. 完成中的儲存格 **ID** 欄的下列其中一種方式：

   * 如果您要匯入的物件是新的(而且您鍵入了 **TRUE** 在 **isNew** 欄)，為ID指定任何數字。 此數字在試算表中必須是唯一的。

   * 如果您要匯入的物件已存在於Workfront系統中(且您輸入 **FALSE** 在 **isNew** 欄)，ID必須是該物件在Workfront中存在的英數字元GUID。

     **範例：** 若為專案，值會顯示在 **setGroupID** 欄必須為下列其中一項：

      * Workfront執行個體中現有群組的GUID
...
1. （選用）若要新增自訂資料：
   ...
   * 為對應自訂欄位命名每個新欄，如下所示： **DE：[Workfront中顯示的自訂欄位名稱]**.
   * 在欄中 **setCategoryID**，輸入此自訂欄位所在之現有自訂表單的GUID。 匯入自訂資料時，此欄位是必要的。
   * 如果您需要在自訂欄位中新增多個資料值（例如選項按鈕、核取方塊或清單），請使用列於「偏好設定」索引標籤中的垂直列自訂資料分隔符號「|」來分隔值。

     **範例：** 在DE：Departments欄下輸入A|D，以填入自訂表單中的部門A和部門D。

### 包含日期  {#include-dates}

Workfront可以處理大多數的日期格式。 不過，您必須確保試算表中的日期欄會格式化為日期。 如果欄的格式為一般、數字或文字，匯入將會失敗。

>[!TIP]
>
>大部分人認為使用YYYY/MM/DD格式最簡單（例如：07/10/2022）。

Workfront也接受時間值作為日期的一部分（例如：07/10/2022 01:30或07/10/2022 1:00 PM）。

如果您忽略日期中的時間，Workfront會執行下列任一項作業：
...
>
>例如，如果您的時間戳記為7336899000，則會在儲存格中輸入7336899000000。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td> <p>$$TODAY</p> </td> 
   <td> <p>用於 <strong>setDate</strong> 欄位，此萬用字元會將日期設定為您匯入Kick-Start當天的午夜。</p> <p>您可以使用篩選器上萬用字元允許的標準語法來修改萬用字元。</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例： </b></span></span>如果您希望專案在匯入當週的星期一開始（無論您實際執行匯入的日期為何），可以使用 <strong>$$TODAYbw</strong>. 這會將您的專案計劃開始日期設定為星期日凌晨12:00。 由於專案排程此時可能不允許工作，因此將於星期一上午9:00開始。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>$$NOW</p> </td> 
   <td> <p>用於 <strong>setDate</strong> 欄位，此萬用字元會根據您在Kick-Start匯入期間建立記錄的時刻來設定日期。</p> <p>您可以使用篩選器上萬用字元允許的標準語法來修改萬用字元。</p> <p class="example" data-mc-autonum="<b>Example: </b>"><span class="autonumber"><span><b>範例： </b></span></span>如果您希望專案在匯入後3小時開始，您可使用 <strong>$$NOW+3h</strong>.</p> </td> 
  </tr> 
  <tr> 
   <td> <p>$$USER.ID</p> </td> 
   <td> <p>用於 <strong>setAssignedToID</strong> 或其他以使用者ID為基礎的欄位，此萬用字元會指派工作或以其他方式將記錄與執行匯入的個人建立關聯。</p> </td> 
  </tr> 
  <tr> 
   <td> <p>$$CUSTOMER</p> </td> 
   <td> <p>此萬用字元是專為Kick-Start使用者匯入新增的。 建立Workfront帳戶時，會建立具有系統管理員存取層級的使用者。 在帳戶中建立其他使用者時，指派給預設管理員的使用者名稱可以當做前置詞使用。</p> <p>由於使用者名稱在所有客戶中必須是唯一的，因此如果您有多個個人使用非常常見的使用者名稱，例如John Smith，他可能有使用者名稱「jsmith」，這個選項會很有用。 在使用者名稱指派前面加上預設的管理員使用者名稱，可以保證每個使用者名稱都是唯一的(例如： <strong>$$CUSTOMER.jsmith</strong>)。</p> <p>提示：若要確保使用者名稱在整個系統中是唯一的，更簡單的方式是在以下位置輸入個人的電子郵件地址： <strong>setusername</strong> 欄位。</p> </td> 
  </tr> 
 </tbody> 
</table>
...
雖然最佳做法是儘可能使用ID，但有時在設定 **setattributeid** 值。 只要變更欄標題，您就可以依名稱參照值。
...
* **（專案匯入）**
  ...
  僅處理幾個群組和專案時，這是可行的，但如果您處理每個群組和專案，則這樣做並不實際。

  若要對上述範例執行「屬性名稱替代」，請變更 **setGroupID** 欄標題至 **#setGroupID GROUP****name**. 然後，您可以依名稱參照每個專案的群組。

  >[!NOTE]
  >
  >使用「屬性名稱替代」的選項僅限於現有記錄的參照。 您無法在相同匯入中對建立的物件使用名稱替代。

* **（使用者匯入）**

  匯入使用者時，請填寫 **setRoleID** 從 **角色角色** 標籤。

  有些角色ID適用於帳戶中已存在的記錄，而其他角色ID則在匯入期間建立。

  對於指派給現有角色的新使用者記錄，您可以使用名稱替代。 對於指派給新匯入角色的新使用者記錄，您不能。

  以下說明如何在相同的匯入檔案中使用這兩種方法：

   * 在試算表左側新增一欄 **setRoleID** 欄。
   * 為新欄命名 **#setRoleID角色名稱**.
   * 若要將角色指派給現有記錄，請在下列位置輸入角色名稱： **#setRoleID角色名稱** 欄。

     對於新角色記錄的角色指派，請在setRoleID中輸入您在「角色角色」工作表中指派的ID。

     ![使用者的角色ID](assets/set-role-id.png)

## 將試算表資料匯入Workfront

使用您的資料填入Excel範本後，您可以將其資料上傳至Workfront。
