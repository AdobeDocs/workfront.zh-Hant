...
1. （選用）按一下齒輪圖示 ![設定圖示](assets/gear-icon-settings.png) 如果您要為欄位選擇全系統共用選項，請前往右上角。
...
1. 在左側面板中，按一下 **自訂Forms**.
1. 開啟自訂表單或建立新的自訂表單。
1. 在表單設計工具中，選取要設定共用的專案，然後按一下 **共用** 在右側的欄位編輯區域中。
1. 在顯示的方塊中，在 **授予自訂表單存取權**，開始輸入您要共用專案的使用者、團隊、工作角色、群組或公司名稱，然後按下 **輸入** 名稱顯示時。
   ...
   <table style="table-layout:auto"> 
    ...
    <col> 
      <tbody> 
       <tr> 
        <td role="rowheader">檢視它</td> 
        <td> <p>按一下 <strong>進階設定</strong> 以指定您是否希望使用者能夠將專案新增至自訂表單，或與其他使用者共用。</p> </td> 
       </tr> 
       <tr> 
        <td role="rowheader">管理它</td> 
        <td> <p>允許存取以編輯自訂欄位，並在欄位程式庫和表單設計工具中檢視它。</p> <p>按一下 <strong>進階設定</strong> 指定您是否希望使用者能夠刪除系統中的專案，或是與其他使用者共用專案。</p> </td> 
       </tr> 
     </tbody> 
    </table>
...
如果您需要移除對自訂欄位或共用之自訂表單中Widget的存取權，則需要取消共用表單。 如需指示，請參閱區段 [移除對自訂表單的存取權](/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/share-access-to-a-custom-form.md#remove-access-to-a-custom-form-from-the-list-of-forms) 在文章中 [共用自訂表格](/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/share-access-to-a-custom-form.md).
