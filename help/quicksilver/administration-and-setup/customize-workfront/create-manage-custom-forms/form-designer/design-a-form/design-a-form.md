...
### 欄位API名稱和標籤的附註 {#notes-on-field-names-and-labels}
...
* 建議您不要在自訂欄位API名稱中使用句號/點字元，以防止在Workfront的不同區域使用欄位時發生錯誤。
...
> 
>另請注意，如果環境促銷活動套件中包含預先輸入欄位，則促銷活動發生時，會自動將該欄位轉換為內部查詢。 如需有關環境升級的詳細資訊，請參閱[在Workfront環境之間移動物件的概述（環境升級）](/help/quicksilver/administration-and-setup/set-up-workfront/workfront-testing-environments/environment-promotion-in-wf.md)。

若要新增內部查詢：

1. 在畫面左側的&#x200B;**新欄位**&#x200B;索引標籤中，尋找&#x200B;**內部查詢**&#x200B;或&#x200B;**多重選取內部查詢**，並將其拖曳至畫布上的區段。
1. 在畫面右側，設定自訂欄位的選項：
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td><p>選取您要與欄位關聯的物件型別。</p> <p>按一下<strong>套用</strong>或<strong>儲存並關閉</strong>後，您就無法變更欄位的物件型別。</p>
      <p><strong>注意：</strong></p>
      <ul><li>如果您的Workfront管理員在Workfront使用者介面中為投資組合、程式或專案自訂名稱，則該物件的預設Workfront名稱會出現在下拉式清單中，而不是自訂名稱。 如果您需要這方面的協助，請洽詢Workfront管理員。</li>
     ...
     <tr>
      ...
      <td><p>為物件型別新增篩選條件，以限制使用者在使用欄位時可以選擇的物件。 </p> <p>例如，您可以限制欄位，讓使用者名稱只有在符合以下條件時才可選取：</p> 
       ...
       <p><b>提示：</b>您可以先建立報表以測試您的篩選器，然後再將篩選器直接新增到內部查詢欄位。 這可協助您驗證篩選器是否傳回正確的物件。 然後，您可以切換至報告中的文字模式、複製文字模式陳述式，並將其新增至內部查詢篩選器。</p>
       <p><b>附註</b>：
       <ul> 
        <li>如果您正在編輯現有的自訂表單，將篩選器新增至內部查詢欄位時，不會移除使用者已使用該欄位新增的任何物件（在篩選範圍之外）。</li> 
        <li>此篩選器不適用於行動裝置。 如果您將篩選器用於內部查詢欄位，該欄位將會顯示在不受篩選器影響之使用者的行動裝置上。</li> 
        </ul></p>
      </td>  
     </tr>
     <tr> 
...
此清單顯示自訂表單可用的原生欄位，以及您可以在哪些物件型別上使用每個欄位。
...
* **轉換的問題建立者**：專案
* **貨幣**：專案，範本
* **說明**：專案，任務，問題，範本，範本任務， Portfolio，方案，群組，檔案，費率卡，工作角色
* **工期**：任務、範本任務
* **期間型別**：任務、範本任務
* **工期單位**：任務、範本任務
* **輸入者**：專案、任務、問題、範本、範本任務、群組
* **專案日期**：專案、任務、問題、範本、範本任務、群組
* **匯率日期**：專案
* **固定成本**：專案，範本
* **固定收入**：專案、範本、記帳記錄
* **群組**：專案、範本、Portfolio、方案、公司、費率卡
* **問題型別**：問題
* **反複專案**：任務
* **上次更新者**：專案、任務、問題、範本、範本任務
* **上次更新日期**：專案、任務、問題、範本、範本任務
* **名稱**：專案，任務，問題，範本，範本任務， Portfolio，方案，群組，檔案，公司，費率卡，工作角色
* **所有者**：專案、範本、Portfolio、方案
* **績效指數方法**：專案，範本
* **計畫收益**：專案，範本
* **計畫完成日期**：專案、任務、問題
* **計畫持續時間**：專案、範本
...
>
>您必須有其他套件才能存取Workfront Planning。
>
