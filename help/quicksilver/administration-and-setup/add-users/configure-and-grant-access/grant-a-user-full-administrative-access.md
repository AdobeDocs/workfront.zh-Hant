...
>[!NOTE]
>
>這不同於使用存取層級來授與使用者對系統特定區域的管理存取權。 如需詳細資訊，請參閱以下頁面:
>
>* [授與使用者對特定區域的管理存取權](../../../administration-and-setup/add-users/configure-and-grant-access/grant-users-admin-access-certain-areas.md)
>* [Workfront管理員的存取權與具有系統管理許可權之Standard或Plan使用者的存取權](#access-of-a-workfront-administrator-vs-access-of-a-plan-user-with-administrative-rights) （本文章）
>
...
## Workfront管理員的存取權與具有管理許可權的標準或計畫使用者的存取權  {#access-of-a-workfront-administrator-vs-access-of-a-plan-user-with-administrative-rights}

以下兩個表格顯示具有Workfront系統管理員存取層級的使用者存取許可權，與具有某些管理許可權之標準或計畫授權的使用者存取許可權之間的差異。

Workfront管理員可以檢視系統中的所有物件（不論物件的建立者為何）、建立新物件，以及修改或刪除現有物件。 使用者擁有系統中所有物件的完整存取權。

擁有標準或計畫授權且可以在一個區域中編輯功能的使用者，可完整存取該區域中的功能。

>[!NOTE]
>
>具有指定作為群組管理員的Standard或Plan授權的使用者可以執行Workfront管理員允許的某些動作。 他們只能為其管理的群組、其子群組以及這些群組和子群組中的使用者執行這些動作。 如需詳細資訊，請參閱[群組管理員](../../../administration-and-setup/manage-groups/group-roles/group-administrators.md)。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>登入身份</td> 
   <td>完整存取權 </td> 
   <td> <p>如果他們的群組管理存取權在其存取層級上已啟用，並且他們被指定為群組管理員，則他們可以以其所管理群組及其子群組中的使用者身分登入。 他們無法以系統管理員身分登入。<br>如需為使用者啟用群組管理存取權的詳細資訊，請參閱<a href="../../../administration-and-setup/add-users/configure-and-grant-access/grant-access-other-users.md" class="MCXref xref">授予使用者存取權</a>。</p> </td> 
  </tr> 
  <tr> 
   <td>排程</td> 
   <td>完整存取權</td> 
