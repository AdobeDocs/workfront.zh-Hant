...
在開始使用Workfront目標之前，我們建議您先閱讀我們對於有效管理目標的最佳實務建議和准則。 如需建立和管理目標的准則詳細資訊，請參閱 [Adobe Workfront目標總覽](../../workfront-goals/goal-management/wf-goals-overview.md).
...
1. 在下列欄位中輸入資訊：
   * **目標名稱**：輸入目標的名稱。 這是必填欄位。
   * **期間**：選取預先定義的季度或年度，從 **期間** 下拉式欄位

     或

     選取 **啟用自訂日期** 選項，然後選取 **開始** 和 **結束日期** 以達成目標。

     先前年度、目前年度及後續年度及其各自的季度會在「期間」下拉式欄位中列為預先定義的選項。

     目標的期間會指出您預期目標完成的時間範圍。

   * **目標所有者**：開始輸入使用者、團隊、群組或組織的名稱，以指出誰是目標的擁有者。 系統預設會選取您作為目標的擁有者。
   * **說明**：輸入有關目標的其他資訊。
1. 按一下 **建立目標**.

   新目標會列在「目標」清單中，而且其狀態為 **草稿**.
   ...
   * 新增結果
     ...
     如需新增結果的資訊，請參閱 [在Adobe Workfront目標中新增結果](../results-and-activities/add-results-to-goals.md).
   * 新增活動

     如需新增活動的相關資訊，請參閱 [將活動新增至Adobe Workfront目標中的目標](../results-and-activities/add-activities-to-goals.md).
   * 對齊其他目標

     如需對齊目標的資訊，請參閱 [透過在Adobe Workfront目標中連線目標來對齊目標](../goal-alignment/align-goals-by-connecting-them.md).
...
如需有關複製目標的資訊，請參閱 [在Adobe Workfront目標中複製目標](../../workfront-goals/goal-management/copy-goals.md).

### 將結果或活動轉換為目標 {#convert-a-result-or-activity-to-a-goal}

您可以將現有目標的結果或活動轉換為目標，以建立目標。 新目標會對齊原始目標。

如需將結果和活動轉換為目標的相關資訊，請參閱 [透過將結果和活動轉換為目標來校準目標](../../workfront-goals/goal-alignment/align-goals-by-converting-results-activities.md).
